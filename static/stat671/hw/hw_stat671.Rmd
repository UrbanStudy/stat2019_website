...
$$=\sum_{i=1}^n\alpha_i\langle x_i,x\rangle +b$$
 ...
 where $\alpha_i=\begin{cases}\frac1{n_{+}}&y_i=+1\\-\frac1{n_{-}}&y_i=-1\end{cases}$; $b=\frac1{2n_{-}^2}\sum\limits_{(i,j)\in I_{-}}\langle x_i,x_j\rangle -\frac1{2n_{+}^2}\sum\limits_{(i,j)\in I_{+}}\langle x_i,x_j\rangle$
...
```{r, echo=T,message=F,warning=F,collapse=F}
n=100
library(mvtnorm)
...
k = function(X,Y)  return(sum(X*Y))
f=function(X){
  k.x=outer(1:n,1,Vectorize(function(i,j) k(x[i],X[j])))
  return(t(k.x)%*%alpha)
}
...
k.t=outer(1:n,1:n,Vectorize(function(i,j) k(x[i],test.x[j])))
test.y.hat=(t(k.t)%*%alpha)

t(cbind(test.y,test.y.hat))

```

```{r,eval=F}
k<- function(x,xp){return(sum(x*xp))}

kernel_machine <- function(x, X, alpha, k){
    return (sign(sum(alpha * k(X,x))))
}

train_kernalized <- function(k, X, Y) {
 alpha = rep(0, n)
    for (j in 0:2) {
        for (i in 0:n) {
...
$$\sum_{i=1}^2\sum_{j=1}^2\alpha_i\alpha_j\langle x_i,x_j\rangle_{R^d}=\sum_{i=1}^2\sum_{j=1}^2\alpha_i\alpha_j\left[\sum_{k=0}^d\binom{d}k\phi_i(x)^T\phi_j(x)\right]=\sum_{k=0}^d\binom{d}k\left[\sum_{i=1}^2\sum_{j=1}^2\alpha_i\alpha_j\langle\phi_i(x)^T\phi_j(x)\rangle_{R}\right]$$
...
\subsection{find a function $\phi$:$\mathbb{R}^2 \mapsto H$, where $H$ is an inner product space such that for any $(x,y)$, $\langle\phi(x),\phi(y)\rangle_H = x^Ty -1$}

$$k(x,y)=\langle \phi(x),\phi(y)\rangle _H = x^Ty -1=\begin{bmatrix} x_1\\x_2 \end{bmatrix}_{2\times1}\begin{bmatrix} y_1&y_2 \end{bmatrix}_{1\times2}-1=x_1y_1+x_2y_2-1$$
...
Therefore, there isn't a function $\phi$:$\mathbb{R}^2 \mapsto H$ that can make $\langle\phi(x),\phi(y)\rangle_H = x^Ty -1$. $\qquad\blacksquare$
