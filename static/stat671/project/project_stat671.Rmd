...
One particular choice of loss function (<span class="red2">hinge loss </span>).
...
If we replace the loss function with hinge loss function, we can ensure that the solution is sparse, so that predictions only depend on a subset of the training data, known as \textbf{support vectors}. This combination of the kernel trick plus a modified loss function is known as a \textbf{support vector machine} or \textbf{SVM}. 
...
n <- 600 #set number of data points. 
...
df$y <- factor(ifelse(df$x2 - 1.4*df$x1 < 0, -1, 1), 
    levels = c(-1, 1))
delta <- 0.07 #set margin
df1 <- df[abs(1.4*df$x1 - df$x2) > delta, ] # retain only those points that lie outside the margin
plot_margins <- ggplot(data = df1, aes(x = x1, y = x2, color = y)) + geom_point() + #build plot
    scale_color_manual(values = c("red", "blue")) + 
    geom_abline(slope = 1.4, intercept = 0)+
    geom_abline(slope = 1.4, intercept = delta, linetype = "dashed") +
    geom_abline(slope = 1.4, intercept = -delta, linetype = "dashed")
plot_margins #display plot 
```


## Support vectors {.smaller}
...
p <- p +geom_path(data = boundary_1,aes(x = x1c, y = x2c),inherit.aes = FALSE) 
