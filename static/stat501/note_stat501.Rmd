...
mode<- fish$idx$alt
fishing <- data.frame(mode,income,price,catch)
```
...
```{r}
multi.fish  <-  nnet::multinom(mode~price + income + catch , data=fishing)
summary(multi.fish)
```
...
```{r,results=F,eval=F }
brm.mlt.fish <- brm(mode~price + income + catch , data=fishing, family=categorical("logit"),silent = T, refresh=0)
```
...
|             |         |Manual       |`glm`        |`brms`      |`nimble`    |`mlogit`    |`glmer`     |
|$\{0,1\}$    | $\Phi$  |$\checkmark$ |$\checkmark$ |$\checkmark$|$\checkmark$|            |$\checkmark$|
|             |  Logit  |$\checkmark$ |$\checkmark$ |$\checkmark$|$\checkmark$|$\checkmark$|$\checkmark$|
|             |         |             |`polr`       |            |            |            |            |   
