...
```{r,echo=T,collapse=T}
(g1.beta <- colMeans(gibbs1,na.rm = T))
(g2.beta<-apply(gibbs2,2,mean))
```
...
```{r}
nimble.probit.beta <- mcmc.out$summary$all.chains[,1]
nimble.probit.beta.sd <- mcmc.out$summary$all.chains[,3]
```
...
```{r,echo=F,collapse=T}
par<- cbind(g1.beta,g2.beta,rl.beta,glm.beta,nimble.probit.beta)
rownames(par) <- c("Beta0","Beta1","Beta2")
pander::pander(round((par),4))
```

```{r,eval=T,echo=F}
g1.beta.sd<- apply(gibbs1,2,sd,na.rm = T)
g2.beta.sd<-apply(gibbs2,2,sd)
par<- rbind(g1.beta.sd,g2.beta.sd,rl.beta.sd,glm.beta.sd, nimble.probit.beta.sd)
...
```{r}
nimble.beta <- mcmc.out$summary$all.chains[,1]
nimble.beta.sd <- mcmc.out$summary$all.chains[,3]
```
...
```{r,eval=T,echo=T,collapse=T}
fit.logit.glm <- glm(Resp~Vol+Rate,family=binomial(link="logit"))
glm.logit.beta<- coef(fit.logit.glm)
glm.logit.beta.sd <- summary(fit.logit.glm)$coef[,2] # sqrt(diag(vcov(fit.probit.glm)))
```
...
mlogit.beta<- coef(fit.mlogit)
mlogit.beta.sd <- sqrt(diag(vcov(fit.mlogit)))
...
```{r,echo=F,collapse=T}
par<- cbind(brm.beta,nimble.beta,glm.logit.beta,mlogit.beta)
rownames(par) <- c("Beta0","Beta1","Beta2")
pander::pander(round((par/2),4))
```

```{r,eval=T,echo=F}
par<- rbind(brm.beta.sd,nimble.beta.sd,glm.logit.beta.sd,mlogit.beta.sd)
colnames(par) <- c("Beta0","Beta1","Beta2")
pander::pander(round(t(par/2),4))
```
...
# Update beta 1
beta <- mvrnorm(1, t(solve(t(X)%*%w%*%X)%*%t(X)%*%w%*%z), solve(t(X)%*%w%*%X)) # without n/(n+1)
# Update beta 2
# beta <- mvrnorm(1, V%*%(t(X)%*%z), V) # draw beta from Normal
# Update beta 3
# beta<- V%*%(t(X)%*%z)+cholV%*%rnorm(p)
