...
\(\beta_{marg}=\phi(\bar p)\boldsymbol{\beta}\)
...
```{r}
nimble.sample<- rbind(mcmc.out$samples$chain1[901:1000,],mcmc.out$samples$chain2[901:1000,],
                      mcmc.out$samples$chain3[901:1000,],mcmc.out$samples$chain4[901:1000,])
(nimble.p.marg <- mean(dnorm(X %*% t(nimble.sample)))*nimble.p)
```
...
```{r,echo=T,collapse=T}
par.p<- cbind(g1,g2,g3,rl,glm.p,nimble.p)
...
```{r,eval=T,echo=T}
par.p.sd<- rbind(g1.sd,g2.sd,g3.sd,rl.sd,glm.p.sd, nimble.p.sd)
...
summary(fit.brms2)
summary(fit.brms3)
brm.l <- fixef(fit.brms1)[,1]
brm.l.sd <- fixef(fit.brms1)[,2]
```
...
for (i in 1:4) {
brm.sample <- rbind(brm.sample,data.frame(fit.brms1$fit@sim$samples[[i]])[1901:2000,1:3])
}
...
(nimble.l.marg <- mean(dlogis(X %*% t(nimble.sample)))*nimble.l)
```
...
\(\beta_{marg}=\bar\phi(p_i)\boldsymbol{\beta}\)

\(\beta_{marg}=\bar logit(p_i)\boldsymbol{\beta}\)

\(\beta_{marg}=\bar p(1-\bar p))\boldsymbol{\beta}\)

```{r,echo=F,collapse=T}
par.marg<- rbind(g1.marg,g2.marg,g3.marg,rl.marg,nimble.p.marg,nimble.l.marg,glm.p.marg,glm.l.marg,brm.l.marg,mlogit.marg)
