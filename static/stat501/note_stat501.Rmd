...
glm<- coef(fit.probit.glm)
glm.sd <- summary(fit.probit.glm)$coef[,2] # sqrt(diag(vcov(fit.probit.glm)))
...
```{r}
nimble.probit <- mcmc.out$summary$all.chains[,1]
nimble.probit.sd <- mcmc.out$summary$all.chains[,3]
```
...
```{r,echo=F,collapse=T}
par<- cbind(g1,g2,g3,rl,glm,nimble.probit)
...
par<- rbind(g1.sd,g2.sd,g3.sd,rl.sd,glm.sd, nimble.probit.sd)
