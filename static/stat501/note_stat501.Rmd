...
```{r setup, include=F}
knitr::opts_chunk$set(message=FALSE, warning=F, echo=TRUE,cache = T)
options(width = 2000)
...
```{r}
fit.brms1 <- brm(Resp ~ Vol + Rate, data =finney, family = bernoulli(),refresh = 0)
...
fit.brms2 <- brm(bf(Resp ~ inv_logit(eta), # 0.5 + 0.5*
                     eta ~ Vol + Rate, nl = TRUE),
  data = finney, family = bernoulli("identity"), 
  prior = prior(normal(0, 5), nlpar = "eta"),refresh = 0
)
...
fit.brms3 <- brm(
  bf(Resp ~ guess + (1 - guess) * inv_logit(eta), 
    eta ~ 0 + Vol + Rate, guess ~ 1, nl = TRUE),
  data = finney, family = bernoulli("identity"), 
  prior = c(
...
gibbs <- matrix(NA,ncol=p,nrow=100); Z<-matrix(NA,100,n) #For Sub Sample
...
if(m%%(M/100)==0 & m>burnin) { gibbs[m/(M/100),] <- t(beta); Z[m/(M/100),]<- z}   # subsamples of equal size
}
```
...
for(j in 1:3) {
acf(gibbs[-(1:20),j],ci.col="dodgerblue4",xlab=lab[j])} 
for(j in 1:3) {
...
for(j in 1:p){
plot(gibbs[-(1:20),j],  ylab=lab[j],main="",pch=1,cex=0.1,
     xlab="Gibbs iteration (k)",col="cornflowerblue")  
plot(cumsum(gibbs[-(1:20),j])/(1:80),  ylab=lab[j],main="",
...
plot(density(gibbs[-(1:20),j],adj=2),lwd=2,main="",col="cornflowerblue",
    xlab=lab[j],ylab="density")
abline(v=quantile(gibbs[-(1:20),j],c(0.025,0.975)),col="gray",lwd=1)
}
```
...
lines(density(gibbs[-(1:20),j],adj=2),col="cornflowerblue",lwd=2,lty=1)
