...
```{r,echo=T,collapse=T}
par.p<- cbind(g1,g2,g3,rl,glm.p,nimble.p,brm.p)
...
brm.ord <- brm(opinion ~ x1 + x2 + x3, data=Panel101, family=categorical("logit", refcat ="Str disag"),silent = T, refresh=0)
```
...
```{r,results=F,eval=F }
brm.mlt.fish <- brm(mode~price + income + catch , data=Fish, family=categorical("logit"),silent = T, refresh=0)
```
