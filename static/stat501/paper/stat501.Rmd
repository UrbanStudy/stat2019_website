...
Denote $W_i \sim PG(1,|\mathbf{x}^T_{i}\boldsymbol{\beta}|)$,i=1:n, then (no absolute value in polson2013)
...
\end{aligned}\]
where \(\mathbf{m_\omega=\underset{(p\cdot p)}{v_\omega}(\underset{(p\cdot n)}{x^T}(\mathbf{y_{1:n}}-\frac12)+B^{-1}b)}\); \(\mathbf{v_\omega=(x^T\Omega x+B^{-1})^{-1}}\).
