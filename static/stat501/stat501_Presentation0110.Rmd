...
```{r setup, include=F}
knitr::opts_chunk$set(message=F, warning=F, echo=T,cache = T,collapse = F)
options(width = 2000)
options(scipen=6)
options(digits=4)
...
$E[\frac{1}{m}\sum Y_k]=E(Y)$


## Monte Carlo Markov chain (MCMC)


\[\pi(\theta|x)=\frac{f(x|\theta)\pi(\theta)}{c=\int f(x|\theta)\pi(\theta) d\theta}\]

Prior $\pi(\theta)$, $\theta\in\Theta\subseteq\mathbb{R}^k$, where $k$ may be "high" dimention

When $c$ is hard to compute, 

define $X_1, X_2...$ stochastic processes with values in $E\subseteq \mathbb{Z}$, if


\(\scriptsize{P(X_n=i_n|X_1=i_1,..,X_{n-1}=i_{n-1})=P(X_n=i_n|X_{n-1}=i_{n-1})}\)


\(P(X_n\in A|X_1,...,X_{n-1})=P(X_n\in A|X_{n-1})\)
...
The unknown true $Z_i\sim t_{\nu=4,\mu,\sigma^2}$,i=1,..,m,
$f_Z(z)=\frac{\Gamma(\frac{\nu+1}2)}{\sqrt{\nu\sigma^2}\Gamma(\frac{\nu}2)}(1+\frac{(z-\mu)^2}{\nu\sigma^2})^{-\frac{\nu+1}2}$

We know $p(z,y|\mu,\sigma^2)=$
$\prod_{i=1}^m\underbrace{\frac{\sqrt{y_i}}{\sqrt{2\pi\sigma^2}}\exp\left[-\frac{y_i}{2\sigma^2}(z_i-\mu)^2\right]}_{N(\mu,\sigma^2/y_i)}\underbrace{\frac{(\frac{\nu}2)^{(\frac{\nu}2)}}{\Gamma(\frac{\nu}2)}y_i^{\frac{\nu}2-1}\exp\left[-\frac{\nu}{2}y_i\right]}_{Gamma(\nu/2,\nu/2)}$
...
beta.p.ci.marg <-cbind(ll=mean(dnorm(X %*% beta.p.ll))*beta.p.ll, ul= mean(dnorm(X %*% beta.p.ul))*beta.p.ul)
```
...
$H^{-1}(p_i)=\ln\frac{p_i}{1-p_i}$=$\mathbf{x}_i^T\boldsymbol{\beta}$ log odds of success
...
\(\mathbf{m_\omega=\underset{(p\cdot p)}{v_\omega}(\underset{(p\cdot n)}{x^T}(\mathbf{y_{1:n}}-\frac12)+B^{-1}b)}\); \(\mathbf{v_\omega=(x^T\Omega x+B^{-1})^{-1}}\).
...
```{r echo=F}
glm.l.marg <-mean(dlogis(X %*% glm.l))*glm.l
glm.l.ci.marg <-mean(dlogis(X %*% glm.l.ci))*glm.l.ci

beta.l.marg <-mean(dlogis(X %*% beta.l.mean))*beta.l.mean
beta.l.ci.marg <-cbind(ll=mean(dlogis(X %*% beta.l.ll))*beta.l.ll, ul=mean(dlogis(X %*% beta.l.ul))*beta.l.ul)
```
...
```{r,echo=F}
mean.all<- cbind(irls.p=glm.p.marg,gibbs.p=beta.p.marg,irls.l=glm.l.marg,gibbs.l=beta.l.marg)
...
```{r,echo=F}
sd.all<- cbind(irls.p=glm.p.ci.marg,gibbs.p=beta.p.ci.marg,irls.l=glm.l.ci.marg,gibbs.l=beta.l.ci.marg)
