---
title: 'Introduction to Data Augmentation and Applications'
author: "Shen Qu"
...
```{r setup, include=F}
knitr::opts_chunk$set(message=F, warning=F, echo=F,cache = F,collapse = F)
options(width = 2000)
options(scipen=6)
options(digits=4)
if (!require(pacman)) {install.packages("pacman"); library(pacman)}
p_load(mixtools,scales,TTR,BayesLogit,mvtnorm,pander,tidyverse) #,EnvStats
...
\(\begin{aligned}
p_i=\Phi(\eta_i)&\\
\Phi^{-1}(p_i)&=\eta_i=\mathbf{x}_i^T\boldsymbol{\beta}\\
\end{aligned}\)

$\Phi$ is the standard normal cdf.

<!-- =\frac12+\frac12\rm erf(\frac{\eta_i}{\sqrt {2}}) -->
<!-- =\sqrt {2}\rm erf^{-1}(2p_i-1) -->
...
Bayesian estimate $\widehat{g(\theta)}=E[\theta|y]$

Draw many $\theta_i^*\overset{iid}{\sim}\pi(\theta|y)$

$\widehat{g(\theta_n)}=\frac1n\sum_{i=1}^ng(\theta_i^*)\overset{a.s}{\to}\widehat{g(\theta)}$ by SLLN
...
Regardless of the distribution, if we have $X_1,X_2,\ldots,X_m \overset{iid}{\sim} f_X(x)$ 
...
Define $\{X_n\}\in E$, $A\in \mathcal{J}$. $\mathcal{J}$ is a $\sigma$-field on $E$, 
...
```{r,eval=T,echo=F, fig.height=12, out.width='100%'}
par(mfrow=c(2,1))

for(i in 1:2){
  cum.est <- cumean(g.t[[1]][,i])
  cum.sd <- cumSd(g.t[[1]][,i])
plot(cum.est,main=c(expression(mu),expression(sigma^2))[i],
  ...
  lines(cum.est+1.96*cum.sd,col="cornflowerblue",lty=3)
  if(i==1){legend(550,3.2,c("true value","estimated value","confidence interval"),
         col =c("red","cornflowerblue","cornflowerblue"),
         lty=rep(1,1,3),cex =1)}
}

```

```{r,echo=F,fig.height=12, out.width='100%'}
Z <- g.t[[2]] %>% data.frame() %>% gather(index,z)
...
# ggplot(Z, aes(x = z, y = index, fill = index)) +
#   geom_density_ridges() +
#   theme_ridges() + 
#   theme(legend.position = "none")

ggplot(Z, aes(x = z, y =index, fill = ..x..)) +
  xlim(-.1,2.2) +
  geom_density_ridges_gradient(scale = 3, rel_min_height = 0.1) +
  scale_fill_viridis(alpha = .5,direction=-1,name = "values") + #,option = "D"
  labs(title = 'Simulated Z') +
  theme_bw()
```
...
Assume $\boldsymbol{\beta}=\{2,-1,1\}$

$\begin{bmatrix}X_1\\X_2\end{bmatrix}\sim N_2(\begin{bmatrix}3\\2\end{bmatrix},\begin{bmatrix}1&0\\0&1\end{bmatrix})$
...
X.df <- data.frame(x1=3-1*rnorm(m),
                   x2=2+1*rnorm(m))
X <- model.matrix(~.,X.df)

# Y <- ifelse(X%*%beta.t+ rnorm(m) > 0, 1, 0)
Y <- rbinom(m,1,pnorm(X%*%beta.t + .1*rnorm(m)))  # 1/(1+exp(-X%*%beta.t - .1*rnorm(m))) 
head(cbind(Y,X),4)
data.frame(sd=c(sd(X[,2]),sd(X[,3])),mean=c(mean(X[,2]),mean(X[,3])),row.names = c("x1","x2"))
...
for(s in 1:p){
  ...
  plot(cum.est,ylim= range(c((cum.est-2*cum.sd),(cum.est+2*cum.sd))),
       ...
       main=bquote(beta[.(s-1)]),type="l",xlab="index",ylab="running average",lwd=2,col="cornflowerblue")
  abline(h=beta.t[s],col="red")
  abline(h=glm.p[s],col="green")
  ...
  lines(cum.est-1.96*cum.sd,col="cornflowerblue",lty=3)
  lines(cum.est+1.96*cum.sd,col="cornflowerblue",lty=3)
  if(s==3){legend(550,mean(cum.est-cum.sd),c("true value","DA's est.","DA's CI","IRLS's est."),
         col =c("red","cornflowerblue","cornflowerblue","green"),
         lty=c(1,1,3,1),lwd=c(1,2,1,1),cex =.8)}
}
```
...
X.df <- data.frame(x1= 3 - 1*rnorm(m),
                   x2= 2 + 1*rnorm(m))
X <- model.matrix(~.,X.df)
# Y <- ifelse(X%*%beta.t+ .1*rnorm(m)> 0, 1, 0) #
Y <- rbinom(m,1,1/(1+exp(-X%*%beta.t - .1*rnorm(m))))  # pnorm(X%*%beta.t+ rnorm(m))
# head(cbind(Y,X),4)
# colMeans(X)
# table(Y)
```
...
```{r,echo=F}
set.seed(123)
beta.l <- g.logit(N,X,Y) # *sqrt(3)/pi
```
...
glm.l<- coef(fit.l.glm) # *sqrt(3)/pi
