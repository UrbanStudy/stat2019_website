...
---|---|---|---|---|---|---
$\bar x=\frac{\sum_{i=1}^n(x_i)}n$|$S_{xx}=\sum_{i=1}^n(x_i-\bar x)^2$|$Var(x)=S_x^2$ | $S_x$ | | |  
$\bar y=\frac{\sum_{i=1}^n(y_i)}n$|$S_{yy}=\sum_{i=1}^n(y_i-\bar y)^2$|$Var(y)=S_y^2$ | $S_y$ | | |  
...
$S(\hat\beta_0,\hat\beta_1)=\sum_{i=1}^n\hat\varepsilon_i=\sum_{i=1}^n(y_i-\hat y)=\sum_{i=1}^n(y_i-\hat\beta_0-\hat\beta_1x_i)^2\quad(SSE)$
...
#### Covariance
   ...
   |x|y|x,y|$\beta$|$\varepsilon$|e|
---|---|---|---|---|---|---|---
Var()|$\sigma_x^2$ | | $ $ 
Cov()|$\sigma_x^2$ | | $\sigma_{xy}$ 
Cor()|$ $ | $ $
...
---|---|---|---|---|---|---
Total Sum of Squares    |SST|$\sum_{i=1}^n(y_i-\bar y)^2$     | $S_{yy}$ | |n-1| MST 
Explained sum of squares|SSR|$\sum_{i=1}^n(y_i-\hat y_i)^2$   | $\hat\beta_1S_{xy}=\hat\beta_1^2S_{xx}$ | |2-1| MSR
Residual sum of squares |SSE|$\sum_{i=1}^n(\hat y_i-\bar y)^2$| $S_{yy}-\hat\beta_1S_{xy}$| $Y'Y-\hatβ'X'Y$ |n-2| MSE 

$SST=S_{yy}=\sum_{i=1}^n(y_i-\bar y)^2=\sum_{i=1}^n[(\beta_0+\beta_1x_i+\varepsilon_i)-\frac1n\sum_{i=1}^n(\beta_0+\beta_1x_i+\varepsilon_i)]^2$
...
$SSE=\sum_{i=1}^ne_i^2=\sum_{i=1}^n(y_i-\hat y)=\sum_{i=1}^n(y_i-\hat\beta_0-\hat\beta_1x_i)^2=\sum_{i=1}^n(y_i-(\bar y-\hat{\beta_1}\bar x)-\hat\beta_1x_i)^2$
$=\sum_{i=1}^n[(y_i-\bar y)-\hat{\beta_1}(x_i-\bar x)]^2=\sum_{i=1}^n(y_i-\bar y)^2-2\hat{\beta_1}\sum_{i=1}^n(y_i-\bar y)(x_i-\bar x)+\sum_{i=1}^n(x_i-\bar x)^2=S_{yy}-2\hat\beta_1^2S_{xy}+\hat\beta_1^2S_{xx}=\mathbf{S_{yy}-\hat\beta_1^2S_{xx}}=S_{yy}-\hat\beta_1S_{xy}$
...
E(SSR)|$\sigma^2+\hat\beta_1^2S_{xx}$ | $k\sigma^2+β^{*'}X'_cX_cβ^{*}$ 
E(SSE)|$(n-2)\sigma^2$                | $ $
