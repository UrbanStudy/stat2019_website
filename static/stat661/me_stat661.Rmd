...
w_1to6 <- sum(y[2:7])
c<- sum(y[2:7]*c(1:6))
lambda0<-0.4; ksi0=0.75; delta = 1e-006; iteration <- 1
PHI<-matrix(nrow=S,ncol=3)
PHI[1,]<-phi<-c( ksi0, lambda0,iteration)
### iteration
repeat { 

# generate a new ksi value 
  phi[1] <- (w-w_0*exp(phi[2]))/(w*(1-exp(phi[2])))
# generate a new lambda value 
  phi[2] <- (sum(index*y[2:size]) * (phi[1]*exp(phi[2])+(1-phi[1])))/
    ((w - w_0) * phi[1] * exp(phi[2]) + w*(1-phi[1]))    

  

 conv <- dist(rbind(PHI[phi[3],1:2],phi[1:2])) 
  phi[3] <- phi[3]+1 
 if(conv < delta) break 
