...
Thus, $\sum_{i=1}^n X_i$ is a sufficient statistic for $\theta$. $P_\theta(\sum_{i=1}^n X_i=x)$ is a one-parameter exponential family.
...
In part (b), $P_\theta(\sum_{i=1}^n X_i=k)=(1-\theta)^k\theta^n$. 
...
\begin{align*}
   ...
   \cdots\\
   \frac{d^k}{d\omega^k} (1-\omega)^{-n}&=(-n-k+1)(-1)\cdots(n+1)n(1-\omega)^{-n-k+1}=[\prod_{i=0}^{k-1}(n+i)](1-\omega)^{-n-k+1}\\
   \left.\frac{d^k}{d\omega^k} (1-\omega)^{-n}\right|_{\omega=0} & =\prod_{i=0}^{k-1}(n+i)
\end{align*}
...
Let $N_j$ be the number of plants of genotype $j$ in a sample of n independent plants, $1\le j\le 6$ and let$\hat p_j = N_j/n$. 
Show that 

\begin{align*}
\hat\theta_1 = \hat p_1 + \frac12\hat p_4 + \frac12\hat p_5 \\
\hat\theta_2 = \hat p_2 + \frac12\hat p_4 + \frac12\hat p_6 \\
\hat\theta_3 = \hat p_3 + \frac12\hat p_5 + \frac12\hat p_6
\end{align*}
