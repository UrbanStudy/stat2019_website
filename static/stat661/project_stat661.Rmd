...
fit0 <- lm(y~nu) 
fit_c <- lm(y[I_c]~nu[I_c])
fit_u <- lm(y[I_u]~nu[I_u])
...
H_i <- dnorm((y_0[I_c]-mu_i)/sigma)/(1-pnorm((y_0[I_c]-mu_i)/sigma))
# H_i <-dnorm(0)/(1-pnorm(0))
D_j <- y_0[I_u]-mu_j  # difference between uncensored y and mean_j
...
# M step    
mu_i <- beta0+beta1*nu_i
# H_i<-   dnorm((y[index_censored]-mu_i)/sigma)/(1-pnorm((y[index_censored]-mu_i)/sigma))

SS_nu <- sum((nu_j)^2)+sum((nu_i)^2)
MS_nu <- sum(nu)^2/n

beta0_star <- sum(y_0[I_u])/n+
                 (sum(nu_j*y_0[I_u])-sum(nu)*sum(y_0[I_u])/n)/
                   (MS_nu-SS_nu)*sum(nu)/n-
                sum(nu)/(MS_nu-SS_nu)/n*
                 sum((mu_i+sigma*H_i)*(sum(nu)/n-1/(sum(nu)/(MS_nu-SS_nu))-nu_i))
               
beta1_star <- -(sum(nu_j*y_0[I_u])-sum(nu)*sum(y_0[I_u])/n)/
                 (MS_nu-SS_nu)+
              1/(MS_nu-SS_nu)*sum((mu_i+sigma*H_i)*(sum(nu)/n-nu_i))

mu_j <- beta0_star+beta1_star*nu_j
 D_j <- y_0[I_u]-mu_j  

mu_i_star <- beta0_star+beta1_star*nu_i 
# H_i_star<-   dnorm((y[index_censored]-mu_i_star)/sigma)/(1-pnorm((y[index_censored]-mu_i_star)/sigma))
sigma_star <- (sum((y_0[I_u]-beta0_star-beta1_star*nu_j)^2)+
                 sum(sigma^2+H_i*sigma*2*(mu_i-mu_i_star)+(mu_i-mu_i_star)^2)
               )/n
...
Q[k] <-  
 -n*log(2*pi)/2-
  n*log(sigma_star)-
 (1/(2*sigma_star^2))*sum((D_j)^2)-          # 1/2sigma^2*sum(j uncensored) -
 (1/(2*sigma_star^2))*sum(                   # 1/2sigma^2*sum(i censored  
                     mu_i^2+       # mu_i_star^2+
                     sigma^2+       # sigma_star^2+                         
                     sigma*(        # sigma_star*(
  y_0[I_c]+mu_i-2*mu_i_star #      w_i+mu_i_star-2mu_i
                            )*H_i-   #            )*H)-
                     2*mu_i*mu_i_star+    # 2*mu_i*mu_i_star+
                     mu_i_star^2               # mu_i^2
                    )                   #                   )
# Update THETA
...
THETA[k,5:8]<-unique(mu_i_star+ sigma_star*H_i)
THETA[k,1] <- k
...
```{r,echo=F}
plot(nu,y_0,main="EM Method with full data",xlab="reciprocal of the absolute temperature",ylab="log10 of failure time",
     xlim=c(2,2.5),ylim=c(2.5,5),
     panel.first=abline(h=c(3.577492,3.906551),v=c(2.027575,2.158895,2.256318,2.362949),lty=3,col="gray"))
...
temp <- c(170,190,220) #temperature levels 
trec <- 1000/(temp+273.2) #reciprocal of the absolute temperature T 
...
# Update THETA
 ...
 THETA[k,3] <- beta1_star
 THETA[k,4] <- sigma_star
THETA[k,5:7]<-unique(mu_i_star+ sigma_star*H_i)
THETA[k,1] <- k
...
```{r,echo=F}
plot(nu,y_0,main="EM Method without 150c",xlab="reciprocal of the absolute temperature",ylab="log10 of failure time",
     xlim=c(2,2.5),ylim=c(2.5,5),
     panel.first=abline(h=c(3.577492,3.906551),v=c(2.027575,2.158895,2.256318,2.362949),lty=3,col="gray"))
for (i in (1:max(THETA[,1],na.rm =T)))
abline(THETA[i,2:3],lwd=0.1,lty=1,col=gray(1/max(THETA[,1],na.rm =T)))
