...
(a) Create a matrix of scatterplots for all the variables in the data file.
...
(b) Compute the matrix of correlation coefficients all the variables in the data file.

```{r, include = F}
library(GGally)
ggpairs(data=table_b4[c(1:10)])
...
(c) Based on scatterplots and correlation coefficients, explain and provide order of the predictors according to the significance of their relationship to the selling price of these houses. 

x1, x2, x4, x3, x6, x5, x8, x7, and x9. The scatterplots of y versus x1, x3, and x4 show a medium to strong positive linear relationship and it is confirmed by the correlation coefficients (more than 0.6) in part (b). Predictor x8 has weak to medium negative linear relationship with y.

### 2. 

(a) Fit a multiple linear regression model relating selling price (y) to all nine predictors. Write the fitted regression model with estimated coefficients
...
(b) Test the overall significance of the full model at 10% significance level. 

The fitted model is statistically significant at 10% significance level (p value = 0.000 or 0.00185 or 0.0002)

(c) Explain which predictor is least significant based on partial F test. 

The predictor x6 which is number of rooms is the least significant predictor because p value for partial F test (or the t test) is 0.866 which is the highest p value and it is more than 10%.

(d) Explain whether there is a problem of multicollinearity. The predictor x6 has the highest VIF which is 11.71 and it is the only VIF more than 10. Further, scatterplot of x6 versus x7 and correlation coefficient between x6 and x7 (which is 0.870) show a strong positive linear relationship between the two variables. 

Predictor x6 is the number of rooms while x7 is the number of bedrooms in a house. A high correlation is expected between these two variables. 

There is a problem of multicollinearity. 

(e) Overall, explain which predictor you would eliminate first to simplify the model. 

It will be important to solve multicollinearity first, and hence predictor x6 is the first to remove. However, according to the variable names of x6 and x7, predictor x6 may contain same information contains in x7. The bedrooms are rooms in a house. 
...
(f) Check the adequacy of model assumptions and suggest any corrections if necessary. 

Plot of standardized residuals versus predicted values does not show a specific pattern and hence the fitted linear model is good for the data. The residuals are symmetrically distributed about zero and hence the zero mean assumption of errors is not violated. The vertical deviations of residuals form zero are not significantly different at different values of predictions. Therefore, constant variance assumption is not violated. 

The q-q plot does not show that data points deviated from the straight line much and assumption of normal distribution about errors is not violated. 

Plot of studentized residuals versus observation number does not show positive or negative autocorrelation. Therefore, assumption of independence in errors is not violated.
...
(a) Fit the multiple linear regression model again after removing least significant predictor. Check the significance of the remaining predictors and continue removing the least significant predictor at each stage and refit the model. Stop when all the predictors in the model are significant at 10% significance level. Write the final fitted regression model with estimated coefficients.

```{r, eval=F, include = T}

k <- ols_step_all_possible(model_b4_full)
k %>%slice(511:n()) %>% arrange(desc("Adj. R-Square"))
plot(k)
...
4. 
(a) Use forward selection method to find the best model for these data (use $\alpha=0.25$) and report the final fitted model with estimated coefficients here.
...
(b) Use backward elimination method to find the best model for these data (use $\alpha=0.1$) and report the final fitted model with estimated coefficients here. Does the backward elimination method give the same model as forward selection method?
...
(c) Use stepwise selection method to find the best model for these data (use to enter and stay or remove) and report the final fitted model with estimated coefficients here. Does the stepwise selection method give the same model as forward selection method and backward elimination method?
...
(d) Use best subsets method to find the best model for these data and report the final fitted model with estimated coefficients here. Does this method give the same model as in the previous three methods?
...
(e) If the final models in the previous 4 methods are different, compare their model adequacy and suggest one best model.
...
-If you had the two different models as above from the four methods, then your answer will be:
...
- If you had the same models from the above 4 methods, then your answer will be: 

The model does not have a problem of multicollinearity (VIF <10), and violation of assumptions about the errors (no pattern on residual plots and points follow approximately straight line on the qq plot). Both predictors are significant at 10% significance level and the model explains about 78.4% variation in the sale price of houses. The partial regression plots do not show nonlinear patterns and hence first-order terms are good enough. 

Therefore, the best model will be
...
(f) Provide complete ANOVA table for the best model. Provide partial sum of squares, estimated coefficients, standard errors, p-values, 90% Bonferroni joint confidence intervals for the coefficients of the best model. Provide in a tabular form clearly.
...
(f) Conduct a lack-of-fit test for the best model and report conclusion along with p-value here.

There are no enough repeating values for at least one possible combination of predictors. Therefore, a lack of fit cannot be performed.
...
(g) Check the adequacy of model assumptions and suggest necessary corrections. 

The model does not have a problem of multicollinearity (VIF <10). 
...
(h) Interpret each estimated regression coefficient using question context. 
...
(i) According the question context, do the predictors in the best model make sense? Explain.

(j) Compute and report the prediction power of the best model.

72.93% About 72.93% of variation in predicting the sale price of houses in Erie, Pennsylvania.
...
1-((MPV::PRESS(model_b4_1))/(var(table_b4$y)*(nrow(table_b4)-1)))
# prediction power of backward
1-((MPV::PRESS(model_b4_backward))/(var(table_b4$y)*(nrow(table_b4)-1)))
