...
**12.2** Show that the extrema of are given by
...
$$=\frac{2(S_{xx}-S_{yy})}{(1+b^2)^2}-\frac{8b^2(S_{xx}-S_{yy})}{(1+b^2)^3}-\frac{4bS_{xy}}{(1+b^2)^2}+\frac{16bS_{xy}}{(1+b^2)^3}=\frac{2(S_{xx}-S_{yy}-2bS_{xy})}{(1+b^2)^2}-\frac{8b(bS_{xx}-bS_{yy}-2S_{xy})}{(1+b^2)^3}$$
...
$$=\frac{[2(S_{xx}-S_{yy})+4bS_{xy}](1+b^2)^2-[2(S_{xx}-S_{yy})b+2S_{xy}b^2-2S_{xy}][2(1+b^2)(2b)]}{(1+b^2)^4}=\frac{2}{(1+b^2)^3}[(S_{xx}-S_{yy})(1-3b^2)+2bS_{xy}(3-b^2)]=c$$
...
(a) Show that $\lim\limits_{\lambda\to0}\hat\beta(\lambda)=S_{xy}/S_{xx}$, the slope of the ordinary regression of y on.

(b) Show that  $\lim\limits_{\lambda\to\infty}\hat\beta(\lambda)=S_{yy}/S_{xy}$ the reciprocal of the slope of the ordinary r gression of x on y.

(c) Show that $\hat\beta(\lambda)$ is, in fact, monotone in $\lambda$ and is increasing if $S_{xy}>0$ and de reasing if $S_{xy}<O$.

(d) Show that the orthogonal least squares line $\lambda=1$ is always between the lines gi en by the ordinary regressions of y on x and of x on y.
...
Species
Arctic fox, Owl monkey, Mountain beaver, Guinea pig, Chinchilla, Ground squirrel, Tree hyrax, Big brown bat

Body weight (kg)
(x)
3.385, .480, 1 .350, 1 .040, .425, .101, 2.000, .023

Brain weight (g)
(y)
44.50, 15.50, 8.10, 5.50, 6.40, 4.00, 12.30, .30
