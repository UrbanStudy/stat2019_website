...
Therefore, $MSE(\hat\theta_{MOM})−MSE(\hat\theta_{MLE})\ge0$, $MSE(\hat\theta_{MOM})\ge MSE(\hat\theta_{MLE})$
...
$V[\hat p_{MLE}]=V[\bar X]=\frac{p(1-p)}{p}$
...
$\frac{\partial^2}{\partial{p}}\ln f(x|p)=-\frac{x}{p^2}-\frac{1-x}{(1-p)^2}=-\frac{x-2px+xp^2+p^2-xp^2}{p^2(1-p)^2}$

$I(p)=-E[\frac{\partial^2}{\partial{p}}\ln f(x|p)]=\frac{1}{p(1-p)}$
...
$E(x_1x_2x_3x_4)=\prod_{i=1}^n EX_i=p^4$ which is a unbiased estimator.
