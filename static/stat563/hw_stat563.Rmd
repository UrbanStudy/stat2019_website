...
$$=\sigma^2[\sum_{i=1}^{n-1}n\rho^{i}-\sum_{i=1}^{n-1}i\rho^{i}]=\sigma^2[n\frac{1-\rho^{n}}{1-\rho}-\frac{n(-1+\rho)\rho^{n}+\rho(1-\rho^{n})}{(1-\rho)^2}]=\frac{\sigma^2[n\rho(1-\rho)-\rho(1-\rho^{n})]}{(1-\rho)^2}=\frac{\rho\sigma^2}{1-\rho}(n-\frac{1-\rho^n}{1-\rho})$$
...
For $\delta_i\sim iid\ n(0,1)$, $E[\delta_i]=0$, $Var[\delta_i]=1$,

$$E[X_i]=E[E(X_i|X_{i−1})]=E[E(\rho X_{i-1}+\delta_{i-1}|X_{i−1})]= E[\rho X_{i−1}]=\rho^2E[ X_{i−2}]=\cdots=\rho^{i−1}E[X_1]$$

For $X_1\sim n(\theta,\sigma^2)$,$Var[X_1]=\sigma^2$, $X_i,\delta_i$ are independent, and $\sigma^2-\rho^2\sigma^2=1$,

$Var[X_i]=Var[E(X_i|X_{i−1})]+ E[Var(X_i|X_{i−1})]=Var[E(\rho X_{i-1}+\delta_{i-1}|X_{i−1})]+ E[Var(\rho X_{i-1}+\delta_{i-1})|X_{i−1})]$
 
$$=Var[\rho^{i−1}EX_1|X_{i−1}]+E(\rho^2 Var[X_{i-1}]+Var[\delta_{i-1}]+2\rho Cov[X_{i-1},\delta_{i-1}]|X_{i−1})$$
$Var[X_{i−1}]=\cdots$

$\cdots$

$Var[X_{2}]=Var[\rho EX_1|X_{1}]+E(\rho^2 Var[X_{1}]+Var[\delta_{1}]+2\rho Cov[X_{1},\delta_{1}]|X_{1})=0+\rho^2\sigma^2+1+0=\sigma^2$

$\cdots$

$Var[X_{i−1}]=0+\rho^2\sigma^2+1+0=\sigma^2$

$Var[X_i]=0+\rho^2\sigma^2+1+0=\sigma^2$

$X_1$ and $X_i$ are independent conditional on $X_{i−1}$, then
