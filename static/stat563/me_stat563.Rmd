...
1. \textcolor[rgb]{0.7,0.7,0.7}{\textcolor[rgb]{0.7,0.7,0.7}{Assume that $X_1,X_2,..X_{10}$ is a random sample from a distribution having a p.d.f. of the form $f(x)=\begin{cases}\lambda x^{\lambda-1}& 0<x<1\\0&\text{otherwise}\end{cases}$.}
...
So $Y_i=-\sum_{i=1}^{10}\ln x_i\sim Gamma(\alpha=10,\beta=\frac1{\lambda})$, then,
...
5. \textcolor[rgb]{0.7,0.7,0.7}{Find the MUVE of $\lambda$.} 
...
6. \textcolor[rgb]{0.7,0.7,0.7}{Show that the MUVE of $\lambda$ is asymptotically efficient.} 



 ----
 
 b. \textcolor[rgb]{0.7,0.7,0.7}{Find the expected value of $\hat\lambda_{MLE}$.}
...
$$E[\hat\lambda]=E\left[\frac{n}{\sum_{i=1}^n\ln x_i}\right]=nE\left[(\sum_{i=1}^n\ln x_i)^{-1}\right]=\frac{n\lambda}{n-1}$$
 ...
 c. \textcolor[rgb]{0.7,0.7,0.7}{Find the variance of $\hat\lambda_{MLE}$.}
...
$$Var[\hat\lambda^2]=\frac{n^2\lambda^2}{(n-1)(n-2)}-\frac{n^2\lambda^2}{(n-1)^2}=\frac{n^2\lambda^2}{(n-1)}[\frac1{n-2}-\frac1{n-1}]=\frac{n^2\lambda^2}{(n-1)^2(n-2)}$$
 ...
 d. \textcolor[rgb]{0.7,0.7,0.7}{Using $\hat\lambda_{MLE}$, create an unbiased estimator $\hat\lambda_{U}$.}
...
$$\hat\lambda_{U}=\frac{n-1}{n}\hat\lambda_{MLE}\quad \text{is an unbiased estimator.}$$
 ...
 e. \textcolor[rgb]{0.7,0.7,0.7}{Find the variance of $\hat\lambda_{U}$.`2019-3-5p12`}
