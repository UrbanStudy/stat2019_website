...
$$L(\lambda)=\lambda^ne^{-(\lambda-1)(-\sum^n_{i=1} \ln x_i)}=h(x)c(\lambda)e^{W_i(\lambda)t_i(\vec x)}$$

For pdf $f(x)>0$ and $x^{\lambda-1}>0$, $\lambda>0$. $W_i(\lambda)=\lambda-1$ contains an open interval in $\Bbb R$, so $T=-\sum^n_{i=1} \ln x_i$ is a complete sufficient statistic for $\lambda$.
