...
### 2003S3
[2009FA2][] [2010SA4][] [2016S5][] [2019SA1][]
...
### 2003S9
[2003S9][] [2007F5B][] [2015S4B][] [2018S3B][] [2019SB3][] [SPower](#SPower) [power](#power) [UMP][8.3.2]
...
### 2007F5B
[2003S9][] [2004F9][] [2015S4B][] [2018S3B][] [2019SB3][] [SPower](#SPower) [power](#power) [UMP][8.3.2]
...
### 2013FB1
[2007F1A] [2008S4A]  [MLE](#MLE) [suff][8.2.1] [consi][10.1.1] [unbias][7.3.1]
...
$\pi(\theta)\sim Unif(0,1)$; $f(x|\theta)={n\choose{x}}\theta^x(1-\theta)^{n-x}$

$\pi(\theta|x)=\frac{f(x|\theta)\pi(\theta)}{f(x)}\propto\frac{{n\choose{x}}\theta^x(1-\theta)^{n-x}}{\int_0^1f(x|\theta)d\theta}\propto\frac{(n+1)!}{x!(n-x)!}\theta^x(1-\theta)^{n-x}\sim Beta(x+1,n-x+1)$
...
$E[(Z^2)^2]=V[Z^2]+E[Z^2]^2=V[Z^2]+1\ge1$
...
Given $\theta\ge x$, $\hat\theta=X_{(n)}$ 

(b) Find $E[\hat\theta]$.

$EX_{(n)}=\frac{2n}{2n+1}\theta$

(c) Prove that $\hat\theta$ is consistent for $\theta$. [Cheb](#Cheb)

$\lim\to0$, Bias$\to0$

$\lim\limits_{n\to\infty}E[T]=\mu$, $\lim\limits_{n\to\infty}V[T]=0$
...
### 2015S3B
[2004F8][] [2007F4B][] [2013FB4][] [2018S1B][] [2019SB2][] [Laplace](#Laplace) [MLE](#MLE) [median](#median)  [7.E.13][]
...
### 2015S4B
[2003S9][] [2004F9][] [2007F5B][] [2018S3B][] [2019SB3][] [SPower](#SPower) [power](#power) [UMP][8.3.2]
[7.E.11][] [563-me]
...
$E[\frac{n+12}{n}Y_n^2]=\theta^2$

### 2015F2
[2003F8][] [2014SA2][] [2017FA3][]  [Expo](#Expo) [Basu](#Basu)
...
(d) (5 pts) What is the limiting distribution of $W_1=nY_1$? [MaxU 5.5.11](#Max-Unif)




(e) (5 pts) What is the limiting distribution of $W_n=n(1-Y_n)$? [DeltaM](#DeltaM)
...
### 2016S1
[7.3.3][] [Lehm-Sche](#Lehm-Sche) [Pois]
...
$E[g(t)]=\sum g(t)\frac{(n\lambda)^te^{-n\lambda}}{t!}$. $\forall\lambda$ iff $g(t)=0$, $E[g(t)]=0$, T is complete sufficient stat.
...
$E(\delta|t)=\frac{P(x_i=0)P(\sum_{i=2}^n x_i=t)}{P(\sum_{i=1}^n x_i=t)}=(\frac{n-1}{n})^t \sim Bino(t,1/n),x=0$

(e) Find the MVUE of $P(X_i=1)=\lambda^1e^{-\lambda}/1!$

Let $\delta(x_i)=\begin{cases}1&X_1=1\\0&X_1\neq1\end{cases}$, 
$E(\delta|t)=\frac{P(x_i=1)P(\sum_{i=2}^n x_i=t-1)}{P(\sum_{i=1}^n x_i=t)}=\frac{t(n-1)^{t-1}}{n^t} \sim Bino(t,1/n),x=1$

(f) Find the MVUE of $P(X_i=k)=\lambda^ke^{-\lambda}/k!$:

Let $\delta(x_i)=\begin{cases}1&X_1=k\\0&X_1\neq k\end{cases}$, 
$E(\delta|t)=\frac{P(x_i=1)P(\sum_{i=2}^n x_i=t-k)}{P(\sum_{i=1}^n x_i=t)}=\frac{t!}{k!(n-k)!}\frac{(n-1)^{t-k}}{n^t} \sim Bino(t,1/n),x=k$
...
$\frac{y}n-\frac{y^2-y}{n^2-n}=\frac{Y(n-Y)}{n(n-1)}$
...
### 2016S5
[2003S3][] [2009FA2][] [2010SA4][] [2019SA1 ][]
...
$W=S_k^2=\frac{n-1}{k}S^2$, $E[W]$, $Var[W]$, $Bias$, $MSE$

$\frac{\partial MSE}{\partial k}\overset{set}=0$
...
### 2016F6
[2008F7][] [Expo](#Expo) [BayesE](#BayesE) [562-fe3]

Suppose that $X_1,X_2,..,X_n$ i.i.d. $Exponential(\theta)$, i.e. $f(x;\theta)=\theta e^{-\theta x},x>0$. Also assume that the prior distribution of $\theta$ is $h(\theta)=\lambda e^{-\lambda\theta},\theta>0$. Find the Bayes estimator of $\theta$, assuming squared error loss.

$\pi(\theta|\vec x)=\frac{g(\vec x|\theta)}{m(\vec x)}=\frac{f(\vec x|\theta)h(\theta)}{\int g(\vec x|\theta)d\theta}$

$\pi(\theta|\vec x)\propto f(\vec x|\theta)h(\theta)\sim Beta(n+1,\frac{1}{\lambda+\sum x_i})$

$\hat\theta_{Bayes}=E[\pi(\theta|\vec x)]=\alpha\beta=\frac{n+1}{\lambda+\sum x_i}$
...
(g) Suppose you've taken a sample of size $n=10$. Determine the UMP test of the null hypothesis $\theta=.5$ vs. the alternative $\theta>0.5$. [Neym-Pear][8.3.2]
...
### 2017S3
[Expo](#Expo) [5.2.7 mgf of the sample mean]
...
Suppose that $(P,X)$ is a pair of random variables with $P\sim Beta(\frac12,\frac12)$ and then $X_{|P=p}\sim bin(n,p)$. Find the variance of X. $n(n+1)/8$
...
When $Y>X$,$0<x<y<1$, $U=Y-X$, $V=X$, $|J|=1$, $g(u,v)=1$, $0<v<u+v<1$, $0<v<1-u$,
$g(u)=1-u$, $0<u<1$; Same with $Y<X$;
$E[U]=\int_0^1u(1-u)du=1/6$

### 2017FA2
[2010SA3][] [2011S4][] [2018FA4][] [DeltaM](#DeltaM) [LimD](#LimD)
...
$Exp(1)=Gamma(1,1)$, $X/(X+Y)=1\sim Beta(1,1)=Unif(0,1)$

$U=X/(X+Y)$, $V=X+Y$, $0<u<1$, $0<v$, $|J|=V$, $g(u,v)=ve^{-v}$,
$g(u)=\int_0^1ve^{-v}dv=E[V]=1$
...
$F_X(x)=\begin{cases}\int_1^{\sqrt y}f(x)dx=[\frac{x^2}{9}+\frac{2x}{9}]_1^{\sqrt y}=\frac{y}{9}+\frac{2\sqrt y}{9}-\frac13&1\le y\le4\\\int_{-\sqrt y}^{\sqrt y}f(x)dx=[\frac{x^2}{9}+\frac{2x}{9}]_{-\sqrt y}^{\sqrt y}=\frac{4\sqrt y}{9}& 0\le y\le1\end{cases}$
...
$F_U(u)=P(U\le u)=1-P(U\ge u)=1-\int_1^{2-u}\int_{x+u}^21dydx=\frac12+u-\frac{u^2}2$, $0<u<1$

$f_U(u)=1-u$; $E[U]=\int_0^1u(1-u)du=[\frac{u^2}2-\frac{u^3}3]_0^1=\frac16$
...
(b) Find the distribution of each of them. [indep-1](#indep-1)

$U\sim n(\mu+\gamma,2\sigma^2)$, $V\sim n(\mu-\gamma,2\sigma^2)$

### 2018S1B
[2004F8][] [2007F4B][] [2013FB4][] [2015S3B][] [2019SB2][] [Laplace](#Laplace) [MLE](#MLE) [median](#median)  [7.E.13][]
...
### 2018S3B
[2003S9][] [2004F9][] [2007F5B][] **[2015S4B][]** [2019SB3][] [SPower](#SPower) [power](#power) [UMP][8.3.2] [HypoT](#HypoT)
...
$T=\sum x_i$, $\frac{2T}{\mu}\sim\chi^2_{20}$;
$P(\frac{2T}{5}\le \chi^2_{20,0.95}=10.85)=0.05$;
Power function $P(reject H_0|\mu_1)=P(\frac{2T}{\mu_1}\le \frac{5\chi^2_{20,0.95}}{\mu_1})$
...
(d) Find $P(Y_1\le3/4|Y_1>1/3)$ 55/64
...
(b) Find the expected value of the sample range. [Example 5.4.5](#range)
...
### 2018FA4
[2010SA3][] [2011S4][] [2017FA2][] [DeltaM](#DeltaM) [LimD](#LimD)
...
### 2018FB4
[2013FB5] [HypoT](#HypoT) [Min-Suff](#Min-Suff)
[6.E.10] [8.E.33]
...
### 2019SB2 
[2004F8][] [2007F4B][] [2013FB4][] [2015S3B][] [2018S1B][] [Laplace](#Laplace) [median](#median) [7.E.13][]
...
### 2019SB3 
[2003S9][] [2004F9][] [2007F5B][] [2015S4B][] [2018S3B][]

Suppose $X_1,X_2,..,X_n$ is a random sample from a distribution with pdf
$f(x,\theta)=\begin{cases}\theta x^{\theta-1}& 0<x<1\\0& o.w.\end{cases}$
Suppose that the value of $\theta$ is unknown and it is desired to test the following hypotheses:
$H_0:\theta=1\quad H_1 :\theta>1$
Derive the UMP test of size $\alpha$ and obtain the null distribution of your test statistic. [UMP][8.3.2]
...
n trials until r(successes)  $P(X=n)={n-1\choose r-1}p^r(1-p)^{n-r}$;$P(X\le n)=\sum_{i=r}^n{i-1\choose r-1}p^r(1-p)^{i-r}$

x(failures) before r(successes) $P(X=x)={{x+r-1}\choose{x}}p^rq^{x}$;$P(X\le x)=\sum_{i=0}^x{{i+r-1}\choose{i}}p^rq^{i}$; $M_X(t)=(\frac{p}{1-qe^t})^r,t<\ln q$$
...
### 5.5.3 Convergence in Distribution {#Max-Unif} {#CLT}
...
### 6.2.2 Minimal Sufficient Statistics {#Min-Suff}
...
$f(x|\vec\theta)=h(x)c(\vec \theta)e^{\sum^k_{j=1}W(\vec \theta_j)t_j(x)}$,
where $\vec\theta=\theta_1,..\theta_k$. Then the statistic $T(X)=(\sum_{i=1}^nt_1(X_i),..,\sum_{i=1}^nt_k(X_i))$ is complete as long as the parameter space $\Theta$ contains an open set in in $\Bbb R^k$.
...
$(\ln x)'=\frac1x$, $\int\frac1{ax+b}=\frac{1}a\ln|ax+b|$;

$\lim\limits_{n\to\infty}(1\pm\frac{a}n)^n=e^{\pm a}$, $\sum_{n=0}^\infty\frac{(tx)^n}{n!}=e^{tx}$;

$\Gamma(n)=(n-1)!=(n-1)\Gamma(n-1)$, $\Gamma(1/2)=\sqrt\pi$, $\Gamma(-1/2)=-2\Gamma(1/2)$, $\Gamma(0)=\Gamma(-1)=\infty$;
...
|   $D$   |$E;EX^2;V$|$f(x);F(x),P(X\le x)$|$MLE;T;I$|$M_x(t);M'(t);M''(t);M^n(t)$|
|   -    |     ---  |   ---   |    --   | ---- |
|$Bern(p)$|$p$;$p$;$pq$|$p^xq^{1-x},x=1,0$|$\bar X$;$\sum x_i\sim Bino(n,p)$;$1/pq$|$pe^t+q$|
|$Bino(n,p)$|$np$;$np(np+q)$;$npq$|$\binom{n}{x}p^x q^{n-x}$,$x=0,1..n$;|$\bar X$or$k\ge X_{(n)}$;$\sum x_i\sim Bino(n,p)$;$1/pq$|$(pe^t+q)^n$|
|$Geom(p)$|$1/p$;${(p+2q)}/{p^2}$;$q/p^2$|$pq^{x-1}$,$x=1,2,..$;$1-q^x$|$1/\bar X$;$\sum x_i$;|$\frac{pe^t}{1-qe^t},t<-\ln{q}$;$\frac{pqe^t}{(1-qe^t)^2}$;$\frac{2pqe^t}{(1-qe^t)^3}-M'(t)$|
|$NBino(r,p)$|$r/p$; ;$rq/p^2$|$\binom{x-1}{r-1}p^rq^{x-r}$,$x=r,r+1..$|$\bar X$;$\sum x_i$|$(\frac{pe^t}{1-qe^t})^r,t<\ln q$|
|$HGeom(N,m,k)$|$\frac{km}N$; ;$\mu\frac{(N-m)(N-k)}{N(N-1)}$|${\binom{m}{x}\binom{N-m}{k-x}}/{\binom{N}{k}}$|
|$Pois(\mu)$|$\mu$;$\mu$;$\mu^2+\mu$|$\frac{\mu^x}{x!}e^{-\mu}$,$x=0,1..$;$e^{-\mu}\sum_{i=0}^x\frac{\mu^i}{i!}$|$\bar X$;$\sum x_i$;$1/\mu$|$e^{\mu(e^t-1)}$;$\mu e^tM(t)$;$\mu e^t(1+\mu e^t)M(t)$|
|$Unif(n)$|$\frac{n+1}2$;$\frac{(n+1)(2n+1)}{6}$;$\frac{n^2-1}{12}$|$\frac{1}n$,$x=1,2..n$,$n=b-a+1$;$\frac{x-a+1}n$|;$X_n$Comp|$\frac1n\sum_{i=1}^n{e^{ti}}$,$\frac{e^{at}-e^{(b+1)t}}{n(1-e^t)}$|
|$Unif(a,b)$|$\frac{a+b}{2}$; ;$\frac{(b-a)^2}{12}$|$\frac{1}{b-a}$,$x\in(a,b)$;$\frac{x-a}{b-a}$|$\min{x_{(1)},x_{(n)}}$;R ancillary|$\frac{e^{tb}-e^{ta}}{t(b-a)}$|
|$Norm(\mu,\sigma^2)$|$\mu$;$\mu^2+\sigma^2$;$\sigma^2$|$\frac{1}{\sigma\sqrt{2\pi}} e^{-\frac{(x-\mu)^2}{2\sigma^2}}$||$e^{\mu t +\frac{\sigma^2t^2}2}$; $(\mu+\sigma^2t)M(t)$;$[(\mu+\sigma^2t)^2+\sigma^2]M(t)$|
|$SNorm(0,1)$|$0$;$1$;$1$|$\frac{1}{\sqrt{2\pi}}e^{-\frac{x^2}2}$||$e^{\frac{t^2}2}$|
|$LNorm(\mu,\sigma^2)$|$e^{\mu+\frac{\sigma^2}2}$;$e^{2\mu+2\sigma^2}$;$E^2X(e^{\sigma^2}-1)$|$\frac{1}{x\sigma \sqrt{2\pi}}e^{\frac{-(\ln x-\mu)^2}{2\sigma^2}}$|$\hat\mu=\frac1n\sum\ln x_i$,$\hat\sigma^2=\frac1n\sum\ln(x_i-\hat\mu)^2$|$EX^n=e^{n\mu+n^2\sigma^2/2}$|
|$Cauchy(\theta,\sigma^2)$||$\frac{1}{\pi\sigma}\frac1{1+(\frac{x-\theta}{\sigma})^2}$|||
|$DExpo(\mu,\sigma^2)$|$\mu$;$\mu^2+2\sigma^2$;$2\sigma^2$|$\frac{1}{2\sigma} e^{-|\frac{x-\mu}{\sigma}|}$|$\hat\mu=median$,$\hat\sigma=\frac1n\sum|x_i|$|$\frac{e^{\mu t}}{1-\sigma^2t^2}$|
|$Expo(\beta)$|$\beta$; ;$\beta^2$|$\frac1{\beta} e^{-\frac{x}\beta},x \in (0,\infty)$;$1-e^{-\lambda x}$|$1/\bar X$;$\sum x_i$;|$\frac{\lambda}{\lambda-t},t<\lambda$,$\frac{1}{1-\beta t}$;$\beta(1-\beta t)^{-2}$;$2\beta^2(1-\beta t)^{-3}$|
|$Gamm(\alpha,\beta)$|$\alpha\beta$; ;$\alpha\beta^2$;|$\frac{1}{\Gamma(a)\beta^{\alpha}}x^{a-1}e^{-x/\beta}$|$\prod x_i$,$\sum x_i$|$(\frac{1}{1-\beta t})^a, t <\frac1\beta$;$EX^n=\frac{\beta^n\Gamma(\alpha+n)}{\Gamma\alpha}$|
|$Beta(a,b)$|$\frac{a}{a+b}$;$\frac{a(a+1)}{(a+b)(a+b+1)}$;$\frac{ab}{(a+b)^2(a+b+1)}$|$\frac{\Gamma(a+b)}{\Gamma(a)\Gamma(b)}x^{a-1}(1-x)^{b-1}$||$B(\alpha,\beta)=\frac{\Gamma(\alpha)\Gamma(\beta)}{\Gamma(\alpha+\beta)}$,$x\in(0,1)$;$\frac{\Gamma(\alpha+n)\Gamma(\alpha+\beta)}{\Gamma(\alpha+\beta+n)\Gamma(\alpha)}$|
|$\chi_p^2$|$p$;$2p+p^2$;$2p$|$\frac{1}{\Gamma \frac{p}2 2^{\frac{p}2}}x^{\frac{p}2-1}e^{-\frac{x}2}$|;$\sum\ln x_i$;|$(1-2t)^{-p/2}, t<\frac12$|
|$t_p$|$0,p>1$; ;$\frac{p}{p-2},p>2$|$\frac{\Gamma(\frac{p+1}2)}{\Gamma(\frac{p}2)\sqrt{p\pi}}(1+\frac{x^2}{p})^{-\frac{p+1}2}$
|$F$|$\frac{q}{q-2},q>2$;;$2(\frac{q}{q-2})^2\frac{p+q-2}{p(q-4)},q>4$|$\frac{\Gamma(\frac{p+q}2)}{\Gamma(\frac{p}2)\Gamma (\frac{q}2)}(\frac{p}q)^{\frac{p}2}\frac{x^{\frac{p}2-1}}{(1+\frac{p}qx)^{\frac{p+q}2}}$,$x>0$|||
|$Arcsine$|$\frac{1}2$; ;$\frac{1}8$|$\frac{1}{\pi\sqrt{x(1-x)}}$,$x\in[0,1]$;$\frac{1}{\pi\arcsin\sqrt x}$||$Beta(\frac{1}2,\frac{1}2)$|
|$Dirichlet$|$\frac{a_i}{\sum_ka_k}$$\sum_{i=1}^kx_i=1$;;$\frac{a_i(a_0-a_i)}{a_0^2(a_0+1)}$|$\frac1{B(a)}\Pi_{i=1}^kx_i^{a_i-1}$,$x\in(0,1)$,$B(a)=\frac{\Pi_{i=1}^k\Gamma(a_i)}{\Gamma(\sum_{i=1}^ka_i)}$||$Cov(X_i,X_j)=\frac{-a_ia_j}{a_0^2(a_0+1)}$,$a_0=\sum_{i=1}^ka_i$|
|$Weibull(\gamma,\beta)$|$\beta^{1/\gamma}\Gamma(1+1/\gamma)$;;$\beta^{2/\gamma}[\Gamma(1+2/\gamma)-\Gamma^2(1+1/\gamma)]$|$\frac\gamma\beta x^{\gamma-1}e^{-\frac{x^\gamma}\beta}$;$1-e^{-(\frac{x}\beta)^\gamma}$|$x^\gamma$,$\sum x^\gamma$,$\sum\ln x$|;;$\beta^{\frac{n}\gamma}\Gamma(1+\frac{n}\gamma)$|
|$Pareto(\alpha,\beta)$|$\frac{\beta\alpha}{\beta-1}$,$\beta>1$;;$\frac{\beta\alpha^2}{(\beta-1)^2(\beta-2)}$,$\beta>2$|$\frac{\beta\alpha^\beta}{x^{\beta+1}}$;$1-(\frac{\alpha}{x})^\beta$,$x\ge\alpha$|$\hat\alpha=\min x_i$,$\hat\beta=\frac{n}{\sum\ln(x_i/x_{(1)})}$,$\sum x_i$comp/suff;$1/\beta^2$||
