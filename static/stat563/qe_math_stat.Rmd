...
(e) Find the MVUE of $P(X_i=1)=\lambda^1e^{-\lambda}/1!$: [Lehm-Sche](#Lehm-Sche)
...
$F_u(u)=\begin{cases}\int_0^u=u^2/2&0\le u\le1\\F_U(1)+\int_{1}^u=1/2+2u-u^2/2-3/2=2u-u^2/2-1&1\le u\le2\\0&o.w.\end{cases}$

### 2016F5
[2003S10][] [2008F6][] [Basu](#Basu) [562-me2] [Ancillary](#ancillary)
...
$\pi(\theta|\vec X)\propto f(\vec X|\theta)h(\theta)$
...
(f) Identify the sufficient statistic for $\theta$. [suff][8.2.1]

Expo family

$T(x)=\prod x_i$ is suff (by Neymann)
...
$P(Y_1\le1/2, Y_2\le3/4)=\int_0^{1/2}\int_0^{y_2}f(y_1,y_2)dy_1dy_2+\int_{1/2}^{3/4}\int_0^{1/2}f(y_1,y_2)dy_1dy_2=25/32$
...
$E(X-\mu)^3/\sigma^3=\frac{x^3-3x^2\mu+3x\mu^2+\mu^3}{(EX^2-E^2X)^{3/2}}=\frac{e^{9/2}-3e^{5/2}+2e^{3/2}}{(e^2-e)^{3/2}}=(e+2)(e-1)^{1/2}$
...
$E[\hat\sigma^2]=\frac{E[\sum(x_i-\mu)^2]}{n}=\frac{n-1}{n}E[S^2]=\frac{n-1}{n}\sigma^2$

(b) Compute the correlation of $\bar X$ and $X_n$. (2 pts)

$Var[X_n]=\sigma^2$, $Var[\bar X]=\sum Var[X_i]/n^2=\sigma^2/n$

$Cov[\bar X,X_n]=Cov[\sum X_i,X_n]/n=\frac{n-1}{n}\sigma^2$
...
### 2017FA3
[2003F8][] [2014SA2][] [2015F2][] [Expo](#Expo) [Basu](#Basu)

If X and Y are independent $Exp(1)$ random variables, and the density of the ratio $X/(X+Y)$. $=1\sim Beta(1,1)$
...
### 2018FB3
[UMVUE][7.3.2] [FishI][7.3.2] [perc](#perc) [7.3.14] [7.3.15]
...
### 6.2.1 Sufficient Statistics {#suff} {#Facto} 
...
### 6.2.4 Sufficient, Ancillary, and Complete Statistics {#comp} {#Basu} 

**Definition 6.2.21** the (n-1) values $X_1/X_n, . . . , X_{n-1}/X_n$ is an ancillary statistic.
...
### 7.3.3 Sufficiency and Unbiasedness {#Rao-Bla} {#Lehm-Sche} 
...
|$LNorm(\mu,\sigma^2)$|$e^{\mu+\frac{\sigma^2}2}$;$e^{2\mu+2\sigma^2}$;$E^2X(e^{\sigma^2}-1)$|$\frac{1}{x\sigma \sqrt{2\pi}}e^{\frac{-(\ln x-\mu)^2}{2\sigma^2}}$|$\hat\mu=\frac1n\sum\ln x_i$,$\hat\sigma^2=\frac1n\sum\ln(x_i-\hat\mu)^2$|$e^{n\mu+n^2\sigma^2/2}$|
