...
$0.9=\int_0^{g(\theta)}f(x,\theta)$, $g(\theta)=\frac{\ln10}\theta$
...
$1-\alpha=P_{\theta_1}(Y<0.5)=1-\int_0^{0.5}\theta y^{\theta-1}dy=1-\left.y^\theta\right|_0^{0.5}$

(b) Based on the single observation $Y$ , find the uniformly most powerful test of size $\alpha$ for testing $H_0:\theta=1$ against $H_A :\theta>1$.

$\Lambda=\frac{L(\theta_0)}{L(\theta_1)}=\theta_1^{-1}e^{-(\theta_1-1)\ln y}\le C$ 
$L(\theta)$ has MLR for the property of Expo family.
$T=-\ln y$ free of $\theta$ is Sufficient stat. $y\sim Beta(\theta,1)$, $-\ln y\sim Expo(\theta)$; (Under $\theta_0=1$,$-\sum\ln y\sim Gamma(n,1)$,$-2\sum\ln y\sim Gamma(n,1),\chi^2_{2n}$)
...
$S(\theta)={5\choose x}(\frac25)^x(\frac35)^{5-x}\sim Bino(5,2/5)$;
$E[S]=np=2$

(b) Sketch the graph of $S(\theta)$ as a function of $\theta$. Then describe the graph in words.
$\max S(\theta)={5\choose 2}(\frac25)^2(\frac35)^{3}=0.3456$
...
$f(x)=\frac1\theta$, $F(x)=\frac{x}\theta$, $f_{Y_{n}}(x)=\frac{n}{\theta}(\frac{x}\theta)^{n-1}$
...
By property of exponential familiy, $f(x|\vec\theta)=h(x)c(\vec \theta)e^{\sum^k_{j=1}W(\vec \theta_j)t_j(x)}=\frac{1}{\prod(x!)}e^{\sum x_i\ln(n\lambda)-n\lambda}$. 
Then the statistic $T(x)=\sum x_i$ is complete as long as the parameter space $\lambda$ contains an open set in in $\Bbb R^k$.
...
$E(\delta|t)=\frac{P(x_i=1)P(\sum_{i=2}^n x_i=t-k)}{P(\sum_{i=1}^n x_i=t)}=\frac{t!}{k!(n-k)!}\frac{(n-1)^{t-k}}{n^t} \sim Bino(t,\frac1n),x=k$
...
$f(y)={n\choose y}p^x(1-p)^{n-y}=\frac{1}{y!(n-y)!}\cdot n!(1-p)^n\cdot e^{y\ln(\frac{p}{1-p})}=h(y)c(p)e^{w(p)t(y)}$ is Expo familiy.
...
$E[\frac{Y}n-\frac{Y^2-Y}{n^2-n}]=\frac{Y(n-Y)}{n(n-1)}$
...
Ancillary, $g_a(\vec y)=(y_1+a,..,y_n+a,)$, $[Y_n-\bar Y](g_a(\vec y))=Y_n+a-\frac\sum n(Y_i+a)=Y_n-\bar Y$
...
$\pi(\theta|\vec x)\propto L(\theta)h(\theta)\sim Beta(n+1,\frac{1}{\lambda+\sum x_i})$
...
MGF= $\frac1{1-\beta t}=\frac1{1-t/n}$ or $E[e^{tX_{(1)}}]=\int_0^\infty e^{tx}ne^{-nx}dx=\frac{n}{n-t}\int_0^\infty(n-t)e^{-(n-t)x}dx$
...
$\Lambda=\frac{L(\lambda_0)}{L(\lambda_1)}=(\frac{\lambda_0}{\lambda_1})^{\sum x_i}e^{n(\lambda_1-\lambda_)0}$

Expo family $T=\sum x_i$is Suffi; $L(\lambda)$ has MLR; $\Lambda$ is a monotone function of T

$T>C$ is UMP test at $\alpha=P_{\theta_0}(T>C)=0.05$

Under $H_0$, $T=\sum x_i\sim Pois(n)$
...
$E[X]=\frac{n\alpha}{\alpha+\beta}=\frac{n}2$; $V[X]=\frac{n\alpha\beta(\alpha+\beta+n)}{(\alpha+\beta)^2(\alpha+\beta+1)}=n(n+1)/8$
...
$U\sim n(\mu+\gamma,2\sigma^2)$, $V\sim n(\mu-\gamma,2\sigma^2)$; Or $M_U(t)=M_X(t)\cdot M_Y(t)$, $M_V(t)=M_X(t)\cdot M_Y(-t)$
...
$T=\sum x_i$, $\frac{2}{\mu}T\sim\chi^2_{20}$;
$P_{H_0}(\frac{2}{5}T\le \chi^2_{20,0.95}=10.85)=0.05$;
Power function $P(reject H_0|\mu_1)=P_{H_1}(\frac{2T}{\mu_1}\le \frac{5\chi^2_{20,0.95}}{\mu_1})$
...
https://math.stackexchange.com/questions/246202/sample-range-and-order-statistics

(a) Find the CDF of the sample range.

$Y_{(n)}=n e^{-y_n}(1 - e^{y_n})^{n-1}$
...
Example 4.4.6 (Beta-binomial hierarchy) [2019.2.28][7.2.3]
...
where $\vec\theta=\theta_1,..\theta_k$. Then the statistic $T(X)=(\sum_{i=1}^nt_1(X_i),..,\sum_{i=1}^nt_k(X_i))$ is complete as long as the parameter space $\Theta$ contains an open set in $\Bbb R^k$.
...
$Expo(\beta)$&$\beta$; ;$\beta^2$&$\frac1{\beta} e^{-\frac{x}\beta},x \in (0,\infty)$;$1-e^{-\lambda x}$&$1/\bar X$;$\sum x_i$;&$\frac{\lambda}{\lambda-t},t<\lambda$,$\frac{1}{1-\beta t}$;$\beta(1-\beta t)^{-2}$;$2\beta^2(1-\beta t)^{-3}$\\\hline
