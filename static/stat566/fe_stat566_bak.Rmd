...
fe_model_chem1 <- aov(formula = Purity ~ Lab_f+Source_f+Technician_r%in%Lab_f+Source_f:Lab_f+Source_f:Technician_r%in%Lab_f, data=fe_table_chem)
...
```{r echo=F, message=FALSE}
library(lme4)
fe_model_chem2 <- lmer(formula = Purity ~ Lab+Source+(1|Lab:Technician)+Source:Lab+Source:(1|Lab:Technician), data=fe_table_chem, REML = TRUE)
...
For B nested in the random factor A, $\beta_j(i)\sim iid\ N(0,\sigma_\beta^2)$, $E[\beta_{j(i)}]=0$, $V[\beta_{j(i)}]=\sigma_\beta^2$

For $\tau_{i}$ and $\varepsilon_{(ij)k}$ are independent, $\beta_{j(i)}$ and $\varepsilon_{(ij)k}$ are independent.
$Cov[\beta_{j(i)},\frac1{n}\sum_{k=1}^n\varepsilon_{(ij)k}-\frac1{bn}\sum_{j=1}^b\sum_{k=1}^n\varepsilon_{ijk}]=0$
