...
In the multiple regression model with $p-1$ independent variables $X_j$, let the $n\times p$ matrix X denote
the design matrix which contains the column of 1’s to fit the intercept term and has full rank. Let H
denote the hat matrix. Let $h_{ii}$ denote the $i_{th}$ diagonal element of H. Prove that $0\le h_{ii}\le1$.
...
(f) Obtain MS(Pure Error). Hint: Pure Error can be found exactly the same way as we did for simple linear regression model. That is, group according to different combinations of levels from X1 and X2. First compute SS(Pure Error), and then divide it by degrees of freedom.
...
Given a educational material evaluation experiment where there are three possible blocking factors [R,C,G], each with six levels [R1::6;C1::6;G1::6]:
