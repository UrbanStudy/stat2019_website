...
p_load(MASS, ggplot2,GGally,tidyverse,kableExtra,lme4,GAD) ##,robcbi
```
...
You are asked to design an experiment to study the effect of three different types of 32-ounce bottles (A) and three different shelf types (B)—smooth permanent shelves, end-aisle displays with grilled shelves, and beverage coolers—on the time it takes to stock ten 12-bottle cases on the shelves. Three workers (factor C) will be employed in the experiment, and a $3^3$ factorial design is desired. Unfortunately, due to time constraints, you find that only a one-third fraction of this design can be run. Construct the design with I = ABC and determine the complete alias structure. Then, assuming that none of the interaction terms are of interest, analyze the data from the 9 runs that you select. The data set appears in the file, “Bottles.xlsx”.
...
```{r, out.width='25%',fig.show='hold'}
boxcox_2015f1_1 <- MASS::boxcox(model_2015f1_1)
boxcox_2015f1_1$x[which.max(boxcox_2015f1_1$y)]
model_2015f1_2 <- update(model_2015f1_1,Y^2~.)
summary(model_2015f1_2)
plot(model_2015f1_2)

model_2015f1_3 <- update(model_2015f1_2,~X1+I(X1^2)+X2+I(X2^2)+X1:X2)
summary(model_2015f1_3)
anova(model_2015f1_3)
plot(model_2015f1_3)
car::vif(model_2015f1_3)

aic_2015f1 <-  MASS::stepAIC(model_2015f1_3,~.)
summary(aic_2015f1)
plot(aic_2015f1)
...
```{r}
sqrt(predict(aic_2015f1, newdata=data.frame(X1 = 20 ,X2 =1500), interval="prediction", level=0.95 ))
...
# gather(table_2015f2,c(6,7,8),key = "any",value = "height" )[,-6]
# library(reshape2)
# melt(table_2015f2, id=c("A","B","C","D","E"),variable.name = "any",value.name = "Y")[,-6]
GGally::ggmatrix(table_2015f2[,1:5])
str(table_2015f2)
...
library(daewr)
halfnorm(coef(model_2015f2_1)[2:16],alpha=0.2)
...
table_2016s1$age <- as.numeric(table_2016s1$age)
...
ggplot(table_2016s1,aes(height,weight,col=age,group=male))+geom_point()+geom_smooth(method=lm)
```
...
```{r}
car::vif(lm(weight~height+age+male,table_2016s1))
```
No VIF problem
...
```{r, eval=F, include=T}
boxcox_2016s1 <-  MASS::boxcox(lm(weight~height+age+male,table_2016s1))
boxcox_2016s1$x[which.max(boxcox_2016s1$y)]
```
...
```{r, eval=F, include=T}
model_2016s1 <- lm(weight^(-0.5)~((height+age)^2+I(height^2)+I(age^2))*male,table_2016s1)
# model_2016s1 <- lm(weight^(-0.5)~(height+age)^2+I(height^2)+I(age^2)+male,table_2016s1)
summary(model_2016s1)
plot(model_2016s1)
```
...
```{r, eval=F, include=T}
aic_2016s1<- MASS::stepAIC(model_2016s1,~.,direction = "both") # ^2, "both"_sq.inv
```
...
```{r, eval=F, include=T}
p<- ols_step_best_subset(model_2016s1)
```
...
```{r, eval=F, include=T}
model_2016s1_subset <-  lm((weight)^(-0.5)~male+height:male+age:male+I(height^2):male+height:age:male,table_2016s1)#^(-2)
summary(model_2016s1_subset)
plot(model_2016s1_subset)

(predict(model_2016s1_subset,data.frame(height=70,age=26,male="male"),interval = "prediction", level = 0.95))^{-2}

boxcox(lm(weight~height+I(height^2)+I(age^2)+male,table_2016s1))
model_2016s1_subset2 <- lm(weight^(-1.5)~height+I(height^2)+I(age^2)+male,table_2016s1) # height+age+male+height:male
summary(model_2016s1_subset2)
plot(model_2016s1_subset2)
(predict(model_2016s1_subset2,data.frame(height=70,age=26,male="male"),interval = "prediction", level = 0.95))^{-2/3}
```
...
If we don't consider quadratic terms, the proper transform is $y^{-0.5}$

inverse-sqrt transform

```{r, eval=F, include=T}
model_2016s1_sqrt.inv <- lm(weight^(-0.5)~height+age+male,table_2016s1)
summary(model_2016s1_sqrt.inv)
plot(model_2016s1_sqrt.inv)
```



```{r, eval=F, include=T}
boxcox_2016s1 <-  MASS::boxcox(model_2016s1)
boxcox_2016s1$x[which.max(boxcox_2016s1$y)]
```

If we do consider quadratic terms, the proper transform is $y^{-2}$
inverse-sq transform

```{r, eval=F, include=T}
model_2016s1_sq.inv <- lm(weight^(-2)~height+age+male+height:male,table_2016s1)
summary(model_2016s1_sq.inv)
plot(model_2016s1_sq.inv)

model_2016s1_sq.inv2 <- lm(weight^(-2)~age+male+I(height^2)+height:male+age:male,table_2016s1)
summary(model_2016s1_sq.inv2)
plot(model_2016s1_sq.inv2)
```
...
```{r, eval=F, include=T}
model_2016s1_log <- lm(log(weight)~height+age+male,table_2016s1)
summary(model_2016s1_log)
plot(model_2016s1_log)
```
...
```{r, eval=F, include=T}
(predict(model_2016s1_sq.inv,data.frame(height=70,age=26,male="male"),interval = "prediction", level = 0.95))^{-0.5}
(predict(model_2016s1_sqrt.inv,data.frame(height=70,age=26,male="male"),interval = "prediction", level = 0.95))^{-2}
```

- Analysis by gender groups

```{r, eval=F, include=T}
t.test(weight~male,alternative="less",table_2016s1)
t.test(height~male,alternative="less",table_2016s1)
data_male <- table_2016s1[table_2016s1$male=="male",]
data_female <- table_2016s1[table_2016s1$male=="female",]
```


```{r, eval=F, include=T}
model_2016s1_1 <- lm(weight~(height+age)^2+I(height^2)+I(age^2),data_male)
summary(model_2016s1_1)
plot(model_2016s1_1)

boxcox_2016s1_1 <- MASS::boxcox(lm(weight~I(age^2),data_male))
boxcox_2016s1_1$x[which.max(boxcox_2016s1_1$y)]

aic_2016s1_1<- MASS::stepAIC(model_2016s1_1)
summary(aic_2016s1_1)
olsrr::ols_step_best_subset(model_2016s1_1)


model_2016s1_3 <- update(model_2016s1_1,weight^1.5~I(age^2))
summary(model_2016s1_3)
plot(model_2016s1_3)

predict(model_2016s1_1,data.frame(height=70,age=26),interval = "prediction", level = 0.95)
(predict(model_2016s1_3,data.frame(height=70,age=26),interval = "prediction", level = 0.95))^{2/3}
```



```{r, eval=F, include=T}
model_2016s1_2 <- lm(weight~(height+age)^2+I(height^2)+I(age^2),data_female)
summary(model_2016s1_2)
plot(model_2016s1_2)

aic_2016s1_2<- MASS::stepAIC(model_2016s1_2)
summary(aic_2016s1_2)
olsrr::ols_step_best_subset(model_2016s1_2)

boxcox_2016s1_2 <-  MASS::boxcox(lm(weight~age+height:age,data_female))
boxcox_2016s1_2$x[which.max(boxcox_2016s1_2$y)]
model_2016s1_4 <- update(model_2016s1_2,weight^0.5~age+height:age)
summary(model_2016s1_4)
plot(model_2016s1_4)
```
...
plot(model_2016s2)
# anova(model_2016s2)
boxcox_2016s2<- MASS::boxcox(model_2016s2)
boxcox_2016s2$x[which.max(boxcox_2016s2$y)]
model_2016s2_1 <- update(model_2016s2,y^2~.)
summary(model_2016s2_1)
plot(model_2016s2_1)
car::vif(model_2016s2_1)
```

```{r,eval=F,message=F}
library(lme4)
model_2016s2_3 <-lmer(y^2~machine/(1|power)*station,table_2016s2)
model_2016s2_4 <-lmer(y^2~machine*station+(machine|power),table_2016s2)  # machine*station+machine/(1|power)*station
summary(model_2016s2_3)$varcor
summary(model_2016s2_4)$varcor
confint(model_2016s2_3)
...
model_2016s2_5 <- aov(y^2~power_r%in%machine_f+machine_f*station_f+power_r%in%machine_f*station_f, table_2016s2)
gad(model_2016s2_5)
estimates(model_2016s2_5)
model_2016s2_6 <- aov(y^2~power_r%in%machine_f+power_r%in%machine_f*station_f, table_2016s2)
gad(model_2016s2_6)
```
...
summary(model_2016f1)
anova(model_2016f1)
plot(model_2016f1)
car::vif(model_2016f1)
ols_correlations(model_2016f1)


aic_2016f1 <- MASS::stepAIC(lm(Scig~(price+perfem+perblack+Income+Education+Age)^2, table_2016f1),direction = "both")
summary(aic_2016f1)

library(olsrr)
ols_step_both_aic(lm(Scig~(price+perfem+perblack+Income+Education+Age)^2, table_2016f1))

# ols_step_best_subset(model_2016f1)
```
...
Part correlations indicates how much rsquare will decrease if that variable is removed from the model and 
...
```{r}
boxcox_2016f1 <- MASS::boxcox(lm(Scig~price+Income*Age, table_2016f1))
boxcox_2016f1$x[which.max(boxcox_2016f1$y)]
```


```{r,message=F, echo=FALSE, out.width='25%',fig.show='hold'}
model_2016f1_1 <- lm(Scig^(-1)~price+Income+Age, table_2016f1) #  # perfem:Income:Age+price:perfem:Income:Age  #  Age+Income+perblack+Age:price # price + perfem+price:perfem
summary(model_2016f1_1)
anova(model_2016f1_1)
plot(model_2016f1_1)
table_2016f1[c(12,45),]
```

```{r,message=F, echo=FALSE, out.width='25%',fig.show='hold'}
model_2016f1_2 <- lm(log(Scig)~(Income*Age)+log(price), table_2016f1)
...
library(lme4)
model_2016f2_2 <- lmer(oxygen~creek+(1|creek:sample),table_2016f2) #creek+(1|creek:sample)
...
```{r}
exp(predict(model_2017f1_2, newdata=data.frame(age= 26, height= 70, male= "male"),interval = "prediction",level = 0.95))
```
...
table_2018s1$smoker <- factor(table_2018s1$smoker, labels = c("not regu","regularly"))
str(table_2018s1)
...
model_2018s1<- lm(volume ~ height*age*male*smoker, data = table_2018s1, subset=(age >= 6))
summary(model_2018s1)
```
...
```{r}
model_2018f1_1 <- lm(Y^2~X1+X2+X1:X2+I(X1^2)+I(X2^2), table_2018f1)
MASS::boxcox(model_2018f1_1)
summary(model_2018f1_1)
anova(model_2018f1_1)
plot(model_2018f1_1)
ols_plot_diagnostics(model_2018f1_1)
plot(x=fitted(model_2018f1_1), y=residuals(model_2018f1_1),panel.last = abline(h=0, lty=2))

olsrr::ols_regress(model_2018f1_1)
...
```{r}
ols_step_best_subset(model_2018f1_1)
aic_2018f1_1<- MASS::stepAIC(model_2018f1_1,direction = "both")
summary(aic_2018f1_1)
```


```{r}
model_2018f1_2 <- lm(Y^2~X1*X2+I(X1^2), table_2018f1)
MASS::boxcox(model_2018f1_2)
summary(model_2018f1_2)
anova(model_2018f1_2)
plot(model_2018f1_2)
ols_plot_diagnostics(model_2018f1_2)
plot(x=fitted(model_2018f1_2), y=residuals(model_2018f1_2),panel.last = abline(h=0, lty=2))

olsrr::ols_regress(model_2018f1_2)
car::Anova(model_2018f1_2)
car::vif(model_2018f1_2)
```

```{r}
sqrt(predict(model_2018f1_1, newdata = data.frame(X1=20,X2=1900), interval = "prediction", level=0.95))
```
...
table_2018f2$Blk <- rep(0:1,each=24)
table_2018f2$A <- as.factor(table_2018f2$A)
table_2018f2$B <- as.factor(table_2018f2$B)
table_2018f2$C <- as.factor(table_2018f2$C)
table_2018f2$D <- as.factor(table_2018f2$D)
table_2018f2$Blk <- as.factor(table_2018f2$Blk)
str(table_2018f2)

```

```{r, out.width='50%',fig.show='hold'}
model_2018f2_1 <- aov(Heights~Blk+(A+B+C+D)^3, table_2018f2)
anova(model_2018f2_1)

DanielPlot(model_2018f2_1,half = T, alpha=0.2)
library(daewr)
...
table_2019s1$A <- as.factor(table_2019s1$A)
table_2019s1$B <- as.factor(table_2019s1$B)

str(table_2019s1)
glimpse(table_2019s1)
...
library(GGally)
ggpairs(table_2019s1_250, mapping = aes(color = A,alpha=0.3))+theme_light()
ggpairs(table_2019s1_500, mapping = aes(color = A,alpha=0.3))+theme_light()
hist(as.numeric(table_2019s1$x1))
```

```{r, out.width='30%'}
# Prepare Data
data <- na.omit(table_2019s1_250) # listwise deletion of missing
# data <- scale(table_2019s1_250[,1:4]) # standardize variables
...
fit_2019s1 <- kmeans(table_2019s1_250[,1:4], 3) 
# get cluster means
aggregate(table_2019s1_250[,1:4],by=list(fit_2019s1$cluster),FUN=mean)

# append cluster assignment
clusted_2019s1 <- data.frame(table_2019s1_250, fit_2019s1$cluster)



# vary parameters for most readable graph
library(cluster)
clusplot(table_2019s1_250[,1:4], fit_2019s1$cluster, color=TRUE, shade=TRUE,
   labels=2, lines=0)

# Model Based Clustering
library(mclust)
fit_2019s1_1 <- Mclust(table_2019s1_250[,1:4])
# plot(fit_2019s1_1) # plot results
summary(fit_2019s1_1) # display the best model
...
model_2019s1 <- lm((y)~ (x1+x2+x3)^2+A*B,table_2019s1_250)
car::vif(model_2019s1)
summary(model_2019s1)
library(olsrr)
# ols_plot_diagnostics(model_2019s1_1)
ols_step_both_aic(model_2019s1)
aic_2019s1 <- MASS::stepAIC(model_2019s1,direction = "both")
summary(aic_2019s1)
```

```{r}
MASS::boxcox(model_2019s1_1)
model_2019s1_1 <- lm((1/y)~ (x1+x3)+A*B,table_2019s1_250)
summary(model_2019s1_1)
car::vif(model_2019s1_1)
model_2019s1_2 <- lm((1/y)~ x2+A*B,table_2019s1_250)
summary(model_2019s1_2)

model_2019s1_0 <- lm((1/y)~ (x1+x2+x3+A+B)^2,table_2019s1_250)


yhat500<- predict(model_2019s1_1,table_2019s1_500,interval="prediction",level = 0.95)

sq.predict.error_1 <- (yhat500[1]-table_2019s1_500$y)^2
sqrt(mean(sq.predict.error_1))

sq.predict.error_2 <- (predict(model_2019s1_2,table_2019s1_500)-table_2019s1_500$y)^2
sqrt(mean(sq.predict.error_2))
```

make three models by groups

```{r}
colnames(clusted_2019s1)[7] <- "group"
clusted_2019s1$group<- as.factor(clusted_2019s1$group)
model_2019s1_3 <- lm((1/y)~ x1+A*B+group,clusted_2019s1)
summary(model_2019s1_3)

model_2019s1_4 <- lm((1/y)~ x1+A+B,clusted_2019s1[which(clusted_2019s1$group==1),])
summary(model_2019s1_4)

model_2019s1_5 <- lm((1/y)~ A+B,clusted_2019s1[which(clusted_2019s1$group==2),])
summary(model_2019s1_5)

model_2019s1_6 <- lm((1/y)~ x2,clusted_2019s1[which(clusted_2019s1$group==3),])
summary(model_2019s1_6)
```

```{r}
table_2019s1_500 <- table_2019s1_500%>%mutate(group=ifelse(x3>0,1,ifelse(x1>0,2,3)))

test_group1 <- table_2019s1_500[which(table_2019s1_500$group==1),]
test_group2 <- table_2019s1_500[which(table_2019s1_500$group==2),]
test_group3 <- table_2019s1_500[which(table_2019s1_500$group==3),]

sq.predict.error1 <- (predict(model_2019s1_4,test_group1)-test_group1$y)^2
sq.predict.error2 <- (predict(model_2019s1_5,test_group2)-test_group2$y)^2
sq.predict.error3 <- (predict(model_2019s1_6,test_group3)-test_group3$y)^2

sqrt(mean(sq.predict.error1))
sqrt(mean(sq.predict.error2))
sqrt(mean(sq.predict.error3))
sqrt(mean(c(sq.predict.error1,sq.predict.error2,sq.predict.error3)))
```
...
model_2019s1_2 <- lm(table_2019s1_250,formula=(1/y)~ x2+A*B)
summary(model_2019s1_2)
...
model_2019s2_1 <- lm(formula = Shrink ~ Run_r+Trt_f+ Trt_f%in%Run_r+Rev_f + Rev_f%in%Run_r+Trt_f:Rev_f, data=table_2019s2)
gad(model_2019s2_1) # same to anova(model_2019s2_1)
estimates(model_2019s2_1) # see the structure of EMS
```
...
summary(model_2019s2_2)
anova(model_2019s2_2)
ranova(model_2019s2_2)
...
```{r}
model_2019s2_3 <- aov(formula = Shrink ~ Run_r + Trt_f*Rev_f+ Trt_f%in%Run_r, data=table_2019s2)
gad(model_2019s2_3)

model_2019s2_4<- lmer(Shrink ~ (1|Run)+(1|Run:Trt)+Trt*Rev, data=table_2019s2)
summary(model_2019s2_4)
anova(model_2019s2_4)
...
test(rbind(Trt_Run,Run_Trt),adjust="tukey")
test(rbind(Rev_Trt,Trt_Rev),adjust="tukey")
```
...
library(car)
library(agricolae)
with(table_2019s2,xyplot(Shrink~Rev_f|Trt_f,groups=Run_r, aspect="xy",type="o"))
with(table_2019s2,xyplot(Shrink~Trt_f|Rev_f,groups=Run_r, aspect="xy",type="a"))
with(table_2019s2, interaction.plot(x.factor = Trt_f, trace.factor = Rev_f,response = Shrink))
```
...
table_2020s1[!complete.cases(table_2020s1), ]
library(mice)
