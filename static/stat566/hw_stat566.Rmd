...
```{r echo=FALSE, results='hold', out.width='30%'}
...
library(FrF2)
hw1_3_design <- FrF2(nruns=16, nfactors=4, randomize = FALSE)
hw1_3_response <- c(-1.57,76.95,-67.52,-51.32,-7.84,11.69,20.78,-2.82,-18.73,9.78,14.74,-6.5,1.27,10.2,-7.98,-6.25) 
# Here the data are effect, should not be used as response variable.
hw1_3_table<- add.response(hw1_3_design, response =hw1_3_response)
hw1_3_table
hw1_3_model <-lm( hw1_3_response ~ A * B * C * D, data =hw1_3_table) 
hw1_3_effects <- coef(hw1_3_model)[2:16]
library(daewr) 
halfnorm(hw1_3_effects, names(hw1_3_effects), alpha = 0.05)
library(AlgDesign)
...
The $2^4$ factorial model with fixed effects and replications is:

$$y_{ijk}=\mu+\tau_i+\beta_j+(\tau\beta)_{ijk}+\varepsilon_{ijk}$$
for $i=1,2$ ; $j=1,2$; $k=1,2..,n$, where 
...
$\varepsilon_{jik}$ is random error for the $k^{th}$ Exerimental Unit when $i^{th}$ level of Factor A and $j^{th}$ level of Factor A are applied.

$y_{ijk}$ is response for the $k^{th}$ EU when  $i^{th}$ level of Factor A and  $j^{th}$ level of Factor B are applied.
