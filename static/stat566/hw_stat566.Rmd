...
kableExtra::kable(t(hw2_1_table)) %>%kableExtra::kable_styling("striped", full_width = F)
library(ggpubr)
...
```
Based on ANOVA table, factor C, AC, BC have significant effects at 0.05 significance level. Their P-values are 0.008516, 0.0004176, 0.003728 respectivly. Although the effect of factor AB is not significant at 0.05 significance level (P-value=0.005602), keeping it in the model is recommended.
...
The histogram of residuals does not satisfied the normality assumption. This could be caused by the response values being the mean values in each group receiving each treatment combination with 1000 customers. The QQ plot didn't show obvious violation of normality.
...
The result shows the interaction effects of ABC is negligible.
