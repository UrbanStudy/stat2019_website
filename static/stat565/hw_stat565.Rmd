...
The normality test and histogram of residuals show a violation of normality. The QQ plot of residuals shows a sharp upward and downward curve at both extremes, indicating that the tails of this distribution are too light for it to be considered normal. The variance of $y$ seems be proportional to $x$ which implies $y$ may be a Poisson random variable in a simple linear regression model and its variance is equal to the mean. We could regress $y′=\sqrt y$ against $x$ since the variance of the square root of a Poisson random variable is independent of the mean. 
...
and for $\hatμ$ is constant, $\hatτ_i,\hatα_{j(l)},\hatβ_k,\hatδ_l$ are constants for summations on other parameters,
...
```
 ...
 (f) Now, adjust the degrees of freedom for error in the ANOVA table for one missing value. (See the Chapter 4 note). And use adjusted df of error to compute the correct p-value for testing significance of treatments. Write your conclusion based on your p value and report the p value here.

$$F_{adj}=\frac{MS_{Trt}}{SSE/dfE_{adj}}=\frac{162.08}{921.5/(6-1)}=0.8794357$$
The adjusted P-value is 0.5111382.

Both of the ANOVA for RCBD of original data and adjusted data show that 4 treatments have not significant effects on the average values of cracking in the pavement at 0.05 significant level. The P-value for treatment with missing observation is 0.435 while adjusted P-value is 0.5111 and 0.5141 in two methods. This means we should use adjusted methods to avoid the type I error (the rejection of a true null hypothesis) for RCBD with one missing observation.

 (g) The second method of analyzing data with missing value is extra (partial) sum of squares method. Fit a multiple linear regression model and obtain the extra (partial) sum of squares for treatments. If you forgot how to do this, look at regression analysis labs and notes. Test whether treatment is a significant factor using extra (partial) SS F test. Report the p-value from extra (partial) SS F test.

$$F_0=\frac{\frac{SSE_{reduced}-SSE_{full}}{dfE_{reduced}-dfE_{full}}}{MSE_{full}}=\frac{1403.7-921.5}{184.30*(a-1)}=0.8721288$$

The adjusted P-value is 0.5141271. the result shows the same conculsion with apporximation method.

We find the P-value in the Apporximation method is smaller than the Exact method because the first method is that minimizing only $SSE$ may increasing $SS_{Trt}$ and hence it is more likely to conclude effects are significant when they are actually not.
...
```{r echo=FALSE, warning=FALSE, out.width='25%'}
...
table_ingredients%>%ggplot(aes(Batch,y))+geom_point(aes(shape=Day,col=Trt),alpha=0)+geom_text(aes(label=Day))+facet_grid(.~Trt)+theme_light()
table_ingredients%>%ggplot(aes(Batch,y))+geom_point(aes(shape=Day,col=Trt),alpha=0)+geom_text(aes(label=Trt))+facet_grid(.~Day)+theme_light()
qplot(reorder(Trt,y),y,data=table_ingredients)+geom_line(aes(group=Day,lty=Day))+theme_light()
qplot(reorder(Trt,y),y,data=table_ingredients)+geom_line(aes(group=Batch,lty=Batch))+theme_light()
qplot(reorder(Day,y),y,data=table_ingredients)+geom_line(aes(group=Trt,lty=Trt))+theme_light()
qplot(reorder(Batch,y),y,data=table_ingredients)+geom_line(aes(group=Trt,lty=Trt))+theme_light()
qplot(reorder(Day,y),y,data=table_ingredients)+geom_line(aes(group=Batch,lty=Batch))+theme_light()
qplot(reorder(Batch,y),y,data=table_ingredients)+geom_line(aes(group=Day,lty=Day))+theme_light()
```
...
```
 ...
 (c) Test whether ingredients have a significant effect on the reaction time at α=0.05. Report your conclusion and p value here.

Set the overall hypotheses are $H_0: \mu_1=\mu_2=\mu_3=\mu_4=\mu_5$ or $\tau_1=\tau_2=\tau_3=\tau_4=\tau_5$ versus $H_1$: At least two of $\mu_i$'s are different or At least one $\tau_i\neq0$

The ANOVA for Latin-square shows that the P-value of 0.000488 for treatment is small enough. We can reject the $H_0$ and conclude that the ingredients have significant effects on the average values of reaction time at 0.05 significant level. 

 (d) Test the significance of different batches of material at α=0.05. Report your conclusion and p value here.

Set the hypotheses are $H_0: \beta_1=\beta_2=\beta_3=\beta_4=\beta_5$ versus $H_1$: At least two of $\beta_k$'s are different.

The reaults of ANOVA show that the P-value of overall batches effects is 0.347618. TukeyLSD test show that the P-value of all the paired comparing among batches is large enough. All of the confident intervals include zero. We cannot reject $H_0$. There is not significant difference on the average values of reaction time among the batches at 0.05 significant level.
...
The histogram of residuals does not detect obvious violation of normality assumption.
...
ols_plot_resid_hist(lm(y~Trt+Day+Batch,data =table_ingredients))
...
```{r,collapse=T, out.width='25%',fig.show='hold'}
# (a)
table_ingredients <- read_xlsx("Exercise4_22.xlsx");  table_ingredients$Day <- as.factor(table_ingredients$Day)
table_ingredients$Batch <- as.factor(table_ingredients$Batch);  table_ingredients$Trt <- as.factor(table_ingredients$Trt)
glimpse(table_ingredients)
table_ingredients%>%ggplot(aes(Batch,y))+geom_point(aes(shape=Day,col=Trt),alpha=0)+geom_text(aes(label=Day))+facet_grid(.~Trt)+theme_light()
table_ingredients%>%ggplot(aes(Batch,y))+geom_point(aes(shape=Day,col=Trt),alpha=0)+geom_text(aes(label=Trt))+facet_grid(.~Day)+theme_light()
qplot(reorder(Trt,y),y,data=table_ingredients)+geom_line(aes(group=Day,lty=Day))+theme_light()
...
```{r echo=FALSE, fig.asp=0.5, warning=FALSE, out.width='50%'}
...
```{r echo=FALSE}
model_LSD2 <- aov(y~Trt+Row+Replicate/Col,data =table_LSD2)
summary(model_LSD2)
```

 (b) Test whether treatments are statistically significant at α=0.01. Report your conclusion and p value here.

Set the overall hypotheses are $H_0: \mu_1=\mu_2=\mu_3=\mu_4$ or $\tau_1=\tau_2=\tau_3=\tau_4$ versus $H_1$: At least two of $\mu_i$'s are different or At least one $\tau_i\neq0$.

The ANOVA for replicated Latin-square shows that the P-value of 0.437 for treatment is larger than 0.01. We cannot reject $H_0$. The treatments have not significant effects on the average values of resopnse variable at 0.01 significant level. 

### Problem 4:

The BIBD model with fixed effect is:$y_{ij}=μ+τ_i+β_j+ε_{ij}$,for i=1,2,…,a$;$j=1,2,…,b; $N=kb=ra≠ab$.
$ε_{ij} ~ iidN(0,σ^2)$;$\sum_{i=1}^aτ_i=0$ and $\sum_{j=1}^bβ_j =0$; $y_{i.}=\sum_jy_{ij}$,$\bar y_{.j}=\frac1k\sum_iy_{ij}=\frac{y_{.j}}k$.
...
$y_{ijk}=μ+τ_i+β_j+(τβ)_{ij}+ε_{ijk}$, for $i=1,2,…,a$; $j=1,2,…,b$; $k=1,2,…,n$
$ε_{ijk}\sim iid N(0,σ^2)$;$\sum_{i=1}^aτ_i=0$ and $\sum_{j=1}^bβ_j=0$; $\sum_{i=1}^a(τβ)_{ij}=0$   and $\sum_{j=1}^b(τβ)_{ij}=0$
$\bar y_{i..}=\frac1{bn}\sum_{j=1}^b\sum_{k=1}^ny_{ijk}$; $y_{...}=\sum_{i=1}^a\sum_{j=1}^b\sum_{k=1}^ny_{ijk}$; $\bar y_{...}=\frac{y_{...}}N$. Compute the $E(SS_A)$ where  $SS_A=bn\sum_{i=1}^a(\bar y_{i..}-\bar y_{...})^2$ 
 ...
 - For $i=1,2,…,a$; $j=1,2,…,b$; $k=1,2,…,n$, for $\sum_{i=1}^aτ_i=0$, $\sum_{j=1}^bβ_j=0$; $\sum_{i=1}^a(τβ)_{ij}=0$, and $\sum_{j=1}^b(τβ)_{ij}=0$,
...
$=τ_i+\frac1{bn}\sum_{j=1}^b\sum_{k=1}^nε_{ijk}-\frac1{abn}\sum_{i=1}^a\sum_{j=1}^b\sum_{k=1}^nε_{ijk}=τ_i+\bar ε_{i..}-\bar ε_{ijk}$
 ...
 - For $τ_i$ is a constant of the $i^th$ treatment, $E{τ_i}=τ_i,V{τ_i}=0$, for $ε_{ijk}\sim iidN(0,σ^2)$, $E[ε_{ijk}]=0$
...
$E[\bar y_{i..}-\bar y_{...}]=E[τ_i+\bar ε_{i..}-\bar ε_{ijk}]=τ_i+\frac1{bn}\sum_{j=1}^b\sum_{k=1}^nE[ε_{ijk}]-\frac1{abn}\sum_{i=1}^a\sum_{j=1}^b\sum_{k=1}^nE[ε_{ijk}]=\tau_i$
 ...
 - For $τ_i$ and $ε_{ijk}$ are independent. $Cov\left[τ_i,\bar ε_{i..}\right]=Cov[τ_i,\bar ε_{...}]=0$, for $ε_{ijk}\sim iidN(0,σ^2)$, $Cov\left[ε_{ijk},\sum_{i=1}^aε_{ijk}\right]=\sigma^2$
