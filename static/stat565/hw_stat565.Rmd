...
```{r echo=FALSE}
table_miss1 <- as.table(matrix(c(0,32,27,36,38,40,43,33,40,63,14,27),ncol=4,byrow=TRUE))
colnames(table_miss1) <- c("A","B","C","D")
rownames(table_miss1) <- c("Block1","Block2","Block3")
```
...
```{r echo=FALSE}
margin.table(table_miss1,2)/c(2,3,3,3)
```
...
```{r echo=FALSE}
margin.table(table_miss1,1)/c(3,4,4)
```
...
```{r include=FALSE}
addmargins(table_miss1, margin =c(1,2), FUN = mean, quiet =T)
(3*margin.table(table_miss1,1)[1]+4*margin.table(table_miss1,2)[1]-margin.table(table_miss1))/6
```
...
```{r include=FALSE}
table_complete <- tibble(y=c(34,32,27,36,38,40,43,33,40,63,14,27),trt=rep(c("A","B","C","D"),3),blk=rep(c("Block1","Block2","Block3"),4))
...
$$F_0=\frac{\frac{SSE_{reduced}-SSE_{full}}{dfE_{reduced}-dfE_{full}}}{MSE_{full}}=\frac{1025.7-539.5}{89.92*(a-1)}=1.802343$$

```{r}
1-pf(1.802343, 3, 6, lower.tail = TRUE, log.p = FALSE)
```
...
```{r}
summary(aov(y~blk,data=table_complete))
...
ggplot(table_ingredients,aes(Trt,y))+geom_point(aes(size=Day,shape=Batch))+theme_gray()
...
ggplot(table_LSD2,aes(Trt,y))+geom_point(aes(size=Col,shape=Row,col=Replicate))+theme_gray()
...
$$\sum_{i=1}^aQ_i=\sum_{i=1}^a[y_{i.}-\sum_{j=1}^bn_{ij}\bar y_{.j}]=\sum_{i=1}^a[\sum_{j=1}^by_{ij}-\sum_{j=1}^bn_{ij}\frac{\sum_{i=1}^ay_{ij}}k]=\sum_{i=1}^a\sum_{j=1}^b(μ+τ_i+β_j+ε_{ij})-\sum_{i=1}^a\sum_{j=1}^b\frac{n_{ij}}k\sum_{i=1}^a(μ+τ_i+β_j+ε_{ij})$$


$$=abμ+0+0+\sum_{i=1}^a\sum_{j=1}^bε_{ij}-\sum_{i=1}^a\sum_{j=1}^b\frac{n_{ij}}k(aμ+0+aβ_j+\sum_{i=1}^aε_{ij})=abμ+\sum_{i=1}^a\sum_{j=1}^bε_{ij}-\frac{ab^2\mu}{r}\sum_{i=1}^a\sum_{j=1}^bn_{ij}-0-\sum_{i=1}^a\sum_{j=1}^b\frac{n_{ij}}k\sum_{i=1}^aε_{ij}$$
...
$ε_{ij}  ~ iid N(0,σ^2 )$;$\sum_{i=1}^aτ_i=0$ and $\sum_{j=1}^bβ_j=0$; $\sum_{i=1}^a(τβ)_{ij}=0$   and $\sum_{j=1}^b(τβ)_{ij}=0$
...
$$SS_A=bn\sum_{i=1}^a(\bar y_{i..}-\bar y_{...})^2=bn\sum_{i=1}^a(τ_i-\frac1{bn}\sum_{j=1}^b\sum_{k=1}^nε_{ijk}-\frac1{abn}\sum_{i=1}^a\sum_{j=1}^b\sum_{k=1}^nε_{ijk})^2$$ 

$$E[SS_A]=bnE[\sum_{i=1}^aτ_i^2]+bnE[\frac1{bn}\sum_{j=1}^b\sum_{k=1}^nε_{ijk}]^2+bnE[\frac1{abn}\sum_{i=1}^a\sum_{j=1}^b\sum_{k=1}^nε_{ijk}]^2-2Cov[τ_i,ε_{ijk}]-2Cov[τ_i,ε_{ijk}]-Cov(\frac1{bn}\sum_{j=1}^b\sum_{k=1}^nε_{ijk},\frac1{abn}\sum_{i=1}^a\sum_{j=1}^b\sum_{k=1}^nε_{ijk})$$ 