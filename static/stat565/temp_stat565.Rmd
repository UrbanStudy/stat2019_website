...
For $ε_{ij}\sim iid\ N(0,σ^2)$, $Var[\varepsilon_{ij}]=Cov[\varepsilon_{ij},\varepsilon_{ij}]=\sigma^2$, $Cov[\varepsilon_{ij},\varepsilon_{ik}]=0,k=1,2,..n$ then
...
the results indicate that the mean of treatment 2 differs from the means of both treatments 1 and3, and that the means for treatments 1 and 3 are the same on 0.01 significent level.
...
$$C_1=y_{1.}-2y_{2.}+y_{3.}=54-2(111)+42=-126$$
$$C=\sum_{i=1}^ac_i\bar y_{i.}=\bar y_{1.}-2\bar y_{2.}+\bar y_{3.}=10.8-2(22.2)+8.4=-25.2$$

$$SS_{C_1}=\frac{(-126)^2}{5*6}=529.2;\quad F_{C_1}=\frac{529.2}{16.9}=31.31$$

$$F_{1,12}=\frac{SS_C}{MS_E}=\frac{C^2}{\frac{MS_E}n\sum_{i=1}^ac_i^2}=\frac{-25.2^2}{\frac{16.9}56}=31.31$$

The P-value of F-test ($0.0001169669$) is small enough to reject $H_0$, We can conclude treatment means of type 2 has a significant differce between treatment means of type 1 and type 3 on 0.01 significent level.
...
Type 1 and type 3 have lower response time and have not significant different on response time. The engineer could choose either one.
...
When $n\ge6,N\ge18$, The power is larger than 0.90.
...
power.anova.test(groups = 3, n =NULL,between.var =var(c(10.8,22.2,8.4)), within.var = 16.9, sig.level = 0.05, power = 0.9)

# pwr  type = c("two.sample", "one.sample", "paired"),alternative = c("two.sided", "less", "greater"))
pwr.t.test(n = , d =10/sqrt(16.9), sig.level =0.05, power =0.9, type ="one.sample", alternative =c("two.sided", "less", "greater"))
# biotools
...
Imposing the constraint,$\hatτ_3=0$ we get the following solution to the normal equations: $\hat\mu=8.40$, $\hatτ_1=2.40$, $\hatτ_2=13.8$, and $\hatτ_3=0$. These estimators are not the same as in part (a). However, $\hatτ_1-\hatτ_2=2.40-13.8=-11.40$, is the same as in part (a). The contrasts are estimable.
