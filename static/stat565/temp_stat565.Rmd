...
|RB-RCBD| |$\mu+\tau_{i}+\beta_j+ε_{ij}-\mu-\tau_{i}-\frac{\sum_{j=1}^b\beta_j}b -\frac{\sum_{j=1}^bε_{ij}}b$|$=\beta_j-\frac1b \sum_{j=1}^b\beta_j+$|$ε_{ij}-\frac{\sum_{j=1}^nε_{ij}}n$
...
|$Var[y_{ij}-\bar y_{i.}]$||||
| ---   |   ---  |---|---|
|FT-CRD |$Var[ε_{ij}-\frac{\sum_{j=1}^nε_{ij}}n]$|$Var[ε_{ij}]+\frac{\sum_{j=1}^nVar[ε_{ij}]}{n^2}-\frac{2Cov[ε_{ij},\sum_{j=1}^nε_{ij}]}n$|$\sigma^2+\frac{\sigma^2}n-\frac2n\sigma^2=\frac{n-1}n\sigma^2$
|RT-CRD | | | |
|FB-RCBD|$Var[\beta_{j}+ε_{ij}-\frac{\sum_{j=1}^nε_{ij}}n]$|$Var[\beta_{j}]+Var[ε_{ij}]+\frac{\sum_{j=1}^nVar[ε_{ij}]}{n^2}-\frac{2Cov[\beta_{j},ε_{ij}]}n-\frac{2Cov[\beta_{j},\sum_{j=1}^nε_{ij}]}n-\frac{2Cov[ε_{ij},\sum_{j=1}^nε_{ij}]}n$|
|RB-RCBD|$Var[\beta_{j}-\frac{\sum_{j=1}^b\beta_{j}}b+ε_{ij}-\frac{\sum_{j=1}^nε_{ij}}n]$|$Var[\beta_{j}]+\frac{\sum_{j=1}^bVar[\beta_{j}]}{b^2}+Var[ε_{ij}]+\frac{\sum_{j=1}^nVar[ε_{ij}]}{n^2}-\frac{2Cov[\beta_{j},\sum_{j=1}^b\beta_{j}]}b+\frac{2Cov[\beta_{j},ε_{ij}]}n-\frac{2Cov[\beta_{j},\sum_{j=1}^nε_{ij}]}n-\frac{2Cov[\sum_{j=1}^b\beta_{j},ε_{ij}]}b+\frac{2Cov[\sum_{j=1}^b\beta_{j},\sum_{j=1}^bε_{ij}]}{b^2}-\frac{2Cov[ε_{ij},\sum_{j=1}^nε_{ij}]}n$|
