...
The graph show that, when relative humidity is 45%, the average percentage of viability of seeds have obvious difference between different storage length. 
The longer storage length have lower average percentage of viability of seeds.

When humidity is 0 or 32%, the average percentage have not obivous difference between diffenent storage length.
...
The average percentage have not obivous difference between 0 and 32% humidity on any storage length.

The line plot shows that not all lines are parallel. Difference in average percentage between humidity is not same for different storage length. There could be an interaction effect.
...
h=relative humidity; d=storage length (days); h.d=interaction items

```{r echo=FALSE}
kable(favstats(Percent ~ h, data=table_sagebrush),format="latex",booktabs=T) %>%kable_styling("striped", full_width = F,font_size = 8)%>%footnote(symbol= "h=relative humidity")
kable(favstats(Percent ~ d, data=table_sagebrush),format="latex",booktabs=T) %>%kable_styling("striped", full_width = F,font_size = 8)%>%footnote(symbol= "d=storage length (days)")
kable(favstats(Percent ~ h+d, data=table_sagebrush),format="latex",booktabs=T) %>%kable_styling("striped", full_width = F,font_size = 8)%>%footnote(symbol= "h.d=interaction items")
```
...
sum((table_sagebrush$Percent- mean(table_sagebrush$Percent))^2)/62
```
 ...
 **Residuals**   42   204.4     4.867      NA         NA     
   ...
   **Total**     62   17623.78  284.2546   NA        NA    
 -------------------------------------------------------------
...
According to ANOVA table, there is a significant interaction effect from relative humidity and storage length on the average percentage around 5% significance level (P-value=$6.511\times e^{-24}$). That means, effect of humidity and effect of storage length on average percentage of viability is not independent. Therefore, simple effects examinations are recommended.
...
The pairwise test show the simple effects:

 - In comparisons of storage length Least Squares Means by humidity, 

When 0 humidity applied, **ONLY** 120 versus 300 days, 180 versus 300 days **have** significant difference on the average percentage around 5% significance level (P-value=$0.0290$, $0.0264$ but P-value=$0.0852$, $0.0780$ by Tukey method).

When 32% humidity applied, **NONE** of comparisons of storage length have significant difference on the average percentage around 5% significance level (all P-value>$0.05$).

When 45% humidity applied, **ONLY** 120 versus 180 days, 180 versus 240 days **have not** significant difference on the average percentage around 5% significance level (P-value=$0.0966$, $0.09977$). Moreover, by **Tukey method**, 60 versus 120 days, 120 versus 240 days also have not significant difference on the average percentage around 5% significance level (P-value=$0.0815$, $0.0747$)
...
When 60 to 360 day applied, **NONE** of 0 versus 32% have significant difference on the average percentage around 5% significance level (all P-value>$0.05$). **ALL** of 0 versus 45%, 32% versus 45% have significant difference on the average percentage around 5% significance level (all P-value<$0.05$). The **only exception** is, when 300 day applied, 0 versus 32% have significant difference on the average percentage around 5% significance level (P-value=$0.0209$ but P-value=$0.2538$ by Tukey method).
...
ols_plot_resid_hist(lm(Percent~h*d,data =table_sagebrush))
ols_plot_resid_qq(lm(Percent~h*d,data =table_sagebrush))
ols_plot_resid_stud_fit(lm(Percent~h*d,data =table_sagebrush))
...
where subscript "32" represents 45% humidity and 60-days storage; "12" represents 0 humidity and 60-days storage; "12" represents 32% humidity and 60-days storage.
...
```{r, eval=c(16:20,29,30), size="tiny", tidy = T,results="hold"}
# I(a)
table_sagebrush <- read_xlsx("Sagebrush.xlsx")
table_sagebrush$h <-  factor(table_sagebrush$Humidity, levels=c(0,32,45),labels=c("0%","32%","45%"))
table_sagebrush$d <-  factor(table_sagebrush$Days, levels=c(0,60,120,180,240,300,360),labels=c("0","60","120","180","240","300","360"))
...
kable(rbind(d_h,h_d),format="latex")%>%kable_styling("condensed",full_width=F,font_size = 6)%>%row_spec(c(43:63,68,69,71,72,74,75,77,78,80,81,83,84),bold =T)%>%row_spec(c(49,54,55,58),background ="#EAFAF1")
# I(h)
table_sagebrush$student_resid <- rstudent(model_sagebrush)
ols_plot_resid_hist(lm(Percent~h*d,data =table_sagebrush))
ols_plot_resid_qq(lm(Percent~h*d,data =table_sagebrush))
...
$\left.\frac{\partial SSE}{\partial \delta_k}\right|_{\hat\mu,\hat\tau_i,\hat\beta_j,\widehat{(\tau\beta)}_{ij},\hat\delta_k}=2\sum_i^a\sum_j^b(y_{ijk}-\hat\mu-\hat\tau_i-\widehat{(\tau\beta)}_{ij}-\hat\beta_k-\hat\delta_k)(-1)=0$
...
### Question 4 (Show main steps of your work to get full points)
 ...
 - Method 1: Using the estimation of model parameters 
...
For $\varepsilon_{ijk}\sim iid N(0,\sigma^2)$,$\sum_{i=1}^a\sum_{j=1}^b\sum_{k=1}^n\varepsilon_{ijk}=0$, $\sum_{i=1}^a\tau_i=0$ and $\sum_{j=1}^b\beta_j=0$; $\sum_{i=1}^a(\tau\beta)_{ij}=0$, $\sum_{j=1}^b(\tau\beta)_{ij}=0$
...
For $\tau_i,\beta_j,(\tau\beta)_{ij},\varepsilon_{ijk}$ are independent, all the cross product terms are zero. Then,

$$SS_A=bn\sum_{i=1}^a(\bar y_{i..}-\bar y_{...})^2=bn\sum_{i=1}^a(\mu+\tau_i+\bar\varepsilon_{i..}-\mu)^2=bn\sum_{i=1}^a(\tau_i+\bar\varepsilon_{i..})^2=n\sum_{i=1}^a\sum_{j=1}^b(\tau_i^2+\bar\varepsilon_{i..}^2)$$

$$SS_B=an\sum_{j=1}^b(\bar y_{.j.}-\bar y_{...})^2=an\sum_{j=1}^b(\mu+\beta_j+\bar\varepsilon_{.j.}-\mu)^2=n\sum_{i=1}^a\sum_{j=1}^b(\beta_j^2+\bar\varepsilon_{.j.}^2)$$



$$n\sum_{i=1}^a\sum_{j=1}^b(\bar y_{ij.}-\bar y_{...})^2=n\sum_{i=1}^a\sum_{j=1}^b(\mu+\tau_i+\beta_j+(\tau\beta)_{ij}+\bar\varepsilon_{ij.}-\mu)^2=n\sum_{i=1}^a\sum_{j=1}^b(\tau_i^2+\beta_j^2+(\tau\beta)^2_{ij}+\bar\varepsilon_{ij.}^2)$$

$n\sum_{i=1}^a\sum_{j=1}^b(\bar y_{ij.}-\bar y_{...})^2-SS_A-SS_B=n\sum_{i=1}^a\sum_{j=1}^b(\tau_i^2+\beta_j^2+(\tau\beta)^2_{ij}+\bar\varepsilon_{ij.}^2)-n\sum_{i=1}^a\sum_{j=1}^b(\tau_i^2+\bar\varepsilon_{i..}^2)-n\sum_{i=1}^a\sum_{j=1}^b(\beta_j^2+\bar\varepsilon_{.j.}^2)=n\sum_{i=1}^a\sum_{j=1}^b(\tau\beta)_{ij}^2+\bar\varepsilon_{ij.}^2-\bar\varepsilon_{i..}^2-\bar\varepsilon_{.j.}^2)$
...
$$SS_{AB}=n\sum_{i=1}^a\sum_{j=1}^b(\bar y_{ij.}-\bar y_{i..}-\bar y_{.j.}+\bar y_{...})^2=n\sum_{i=1}^a\sum_{j=1}^b\left[(\tau\beta)_{ij}+\bar\varepsilon_{ij.}-\bar\varepsilon_{i..}-\bar\varepsilon_{.j.}\right]^2=n\sum_{i=1}^a\sum_{j=1}^b(\tau\beta)_{ij}^2+\bar\varepsilon_{ij.}^2-\bar\varepsilon_{i..}^2-\bar\varepsilon_{.j.}^2)$$

Therefore,
$$SS_{AB}=n\sum_{i=1}^a\sum_{j=1}^b(\bar y_{ij.}-\bar y_{...})^2-SS_A-SS_B$$


 - Method 3: Using the  cross products of being zero 
 
 Proof three cross-product terms equal 0
 
$n\sum_{i=1}^a\sum_{j=1}^b(\bar y_{i..}-\bar y_{...})=\sum_{i=1}^ay_{i..}-y_{...}=y_{...}-y_{...}=0$
 
$n\sum_{i=1}^a\sum_{j=1}^b(\bar y_{.j.}-\bar y_{...})=\sum_{ji=1}^by_{.j.}-y_{...}=y_{...}-y_{...}=0$

$n\sum_{i=1}^a\sum_{j=1}^b(\bar y_{ij.}-\bar y_{i..}-\bar y_{.j.}+\bar y_{...})=\sum_{i=1}^a\sum_{j=1}^by_{ij.}-\sum_{i=1}^ay_{i..}-\sum_{j=1}^by_{.j.}+y_{...}=y_{...}-y_{...}-y_{...}+y_{...}=0$

$2n\sum_{i=1}^a\sum_{j=1}^b(\bar y_{ij.}-\bar y_{i..}-\bar y_{.j.}+\bar y_{...})(\bar y_{i..}-\bar y_{...})=2n\sum_{i=1}^a\sum_{j=1}^b(\bar y_{ij.}-\bar y_{i..}-\bar y_{.j.}+\bar y_{...})(\bar y_{.j.}-\bar y_{...})=2n\sum_{i=1}^a\sum_{j=1}^b(\bar y_{i..}-\bar y_{...})(\bar y_{.j.}-\bar y_{...})=0$

Expand the sum-of-squares terms

$n\sum_{i=1}^a\sum_{j=1}^b(\bar y_{ij.}-\bar y_{...})^2=n\sum_{i=1}^a\sum_{j=1}^b(\bar y_{ij.}-\bar y_{i..}-\bar y_{.j.}+\bar y_{...}+\bar y_{i..}-\bar y_{...}+\bar y_{.j.}-\bar y_{...})^2$

$=n\sum_{i=1}^a\sum_{j=1}^b(\bar y_{ij.}-\bar y_{i..}-\bar y_{.j.}+\bar y_{...})^2+n\sum_{i=1}^a\sum_{j=1}^b(\bar y_{i..}-\bar y_{...})^2+n\sum_{i=1}^a\sum_{j=1}^b(\bar y_{.j.}-\bar y_{...})^2+0+0+0$

$$=SS_{AB}+bn\sum_{i=1}^a(\bar y_{i..}-\bar y_{...})^2+an\sum_{j=1}^b(\bar y_{.j.}-\bar y_{...})^2=SS_{AB}+SS_A+SS_B$$
