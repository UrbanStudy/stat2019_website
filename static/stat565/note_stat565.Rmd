...
|      |      |  SS | df  | MS  | E(SS) | E(MS) |F test|P-value
| ---  | ---  | --- | --- | --- |  ---  |  ---  |  --- |
|Treatment(Between or Model)|$SS_{Trt}$|$n\sum^a_{i=1}(\bar y_{i.}−\bar y_{..})^2$           |$a-1$|$\frac{SS_{Trt}}{a-1}$|$(a-1)\sigma^2+n\sum^a_{i=1}\tau_i^2$|$\sigma^2+\frac{n}{a-1}\sum^a_{i=1}\tau_i^2$|$\frac{MS_{Trt}}{MS_E}$|$P(F>F_0)$|
|Error(Within)|$SS_E$    |$\sum^a_{i=1}\sum^n_{j=1}(y_{ij}−\bar y_{i.})^2$     |$N-a=a(n-1)$| $\frac{SS_E}{N-a}$   | $(N-a)\sigma^2$ | $\sigma^2$
Total|$SS_T$    |$\sum^a_{i=1}\sum^n_{j=1}(y_{ij}−\bar y_{..})^2$     |$N-1=na-1$|
