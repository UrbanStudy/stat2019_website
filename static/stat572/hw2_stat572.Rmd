...
\begin{align*}
   ...
   =\exp[\underbrace{\ln(\theta)}_{\varphi(\theta)(\theta)}\underbrace{\sum_i^n y_i}_{t(y)}
...
\begin{align*}
p_{n_o,t_0}(\theta)&\propto \exp[n_0 t_0 \ln(\theta)-n_0 \theta] \mathbf{1}_{\{\theta\in \Theta\}}
\propto \theta^{n_0 t_0+1-1}e^{n_0 \theta}\mathbf{1}_{\{\theta\in \Theta\}}\\
...
\begin{align*}
p(\theta| y)&\propto \exp[\ln(\theta)\sum_i^n y_i-n\theta]h(y)
                \cdot\exp[n_0 t_0 \ln(\theta)-n_0 \theta] \mathbf{1}_{\{\theta\in \Theta\}}\\
            &\propto \exp[(t(y) + n_0 t_0 )\ln(\theta) - (n+n_0)\theta] \mathbf{1}_{\{\theta\in \Theta\}}\\
            &\propto \theta^{t(y) + n_0 t_0+1-1}e^{(n+n_0)\theta}\mathbf{1}_{\{\theta\in \Theta\}}\\
...
When we have $n>1$ observation, $t(y)=\sum \tilde{t}(y_i)$,

\begin{align*}
p(\theta| y_{1:n})&\propto \exp[(t^\star n^\star\ln(\theta) - n^\star\theta] \mathbf{1}_{\{\theta\in \Theta\}}\\
            &\propto \theta^{t^\star n^\star+1-1}e^{n^\star\theta}\mathbf{1}_{\{\theta\in \Theta\}}\\
            &\sim \text{Gamma}(t^\star n^\star+1,n^\star)
\end{align*}

where $n^\star = n_0+n\quad\text{ and }\quad t^\star = \frac{n_0}{n_0 + n} t_0  + \frac{n}{n_0 + n}\frac{1}{n}\sum \tilde{t}(y_i),$
