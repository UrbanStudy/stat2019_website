...
\bit
\item \emph{Induction} is the process of reasoning from \textcolor{blue}{specific cases} to a \textcolor{orange}{ general principle}.
\item The statistical endeavor relies induction, where  \textcolor{blue}{\emph{the data}} (the specific case) is used to identify a plausible \textcolor{orange}{\emph{data-generating mechanism}} (the general principle).
\item Data is unknown and uncertain.  
\item In statistics, we assume that data is produced by a specific data-generating mechanism that depends on some unknown and uncertain parameters.
...
\bit
\item Is this event really random? 
...
\item If $\theta$ is known, this becomes a probabilistic data-generating mechanism characterizing the distribution of $Y_{1:n}=(Y_1,\ldots,Y_n)$, which allows us to study properties of $Y_{1:n}$ under this distribution.  This is a probabilistic problem.
...
\item We can estimate confidence intervals for the proportion, but
\eit
...
\bexa Suppose we choose $a = 1$ and $b = 1$ (so the prior for $\btheta$ is uniform). Whys is this a bad prior for this problem?
...
We won't touch on these for now, but if we have time we will come back to this topic.
