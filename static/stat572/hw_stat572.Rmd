...
- A method may be not right, but has a same value.
...
legend("topright",lty=c(2,3),col=c("red","forestgreen"),
       legend=c("p(quant CI)=0.0.945","p(HPD CI)=0.954"))
```
...
$$= \frac{(a+n)(b+\sum_{i=1}^{n} y_i)^{a+n}}{(b+\sum_{i=1}^{n+1} y_i)^{a+n+1}}\prod_{i=1}^{n+1} 1_{\{y_i>0\}}$$
