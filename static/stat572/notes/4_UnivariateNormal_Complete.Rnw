...
\begin{figure}[H]
<<echo=F, message=F, warning=F, fig.width=4.5, fig.height=3.5, fig.align='center', eval=T>>=
...
\item 95\% of the distribution is within $\pm 1.96\sigma$ of the mean
...
Replacing the last expression above into the posterior formula, we have
\bean
...
# x.ll <- seq(-5*tau.diff+mu.diff,-2*sigma,length.out = 100)
...
\eea
where $\nu>0$ represents a scalar hyperparameter that increases or decreases the prior precision for $\btheta$.
...
\bit
\item $\mu_0=0$: we don't know whether students will improve or not, on average.
...
\item $\alpha=1/2$: reflects uncertainty about how much the standard deviation of the changes is.
\item $\beta=100\alpha$ this value leads to a expected standard deviation ($\sqrt{\lambda^{-1}}$) of the changes in the scores of about 10 points, since $\sqrt{E(\lambda)}=\sqrt{\alpha/\beta}=1/10$, and so $\sigma\approx 10$ a priori.  Also, sd$(\lambda)=\sqrt{\frac{\alpha}{(100\alpha)^2}}=\sqrt{\frac{1/2}{50^2}}\approx 0.0141$ (about 14\% of the mean value for $\lambda$).
\eit
...
plot(x=mu.post.p,y=1/sqrt(lambda.post.p),
     grid(ny=8,nx=12,col=grey(0.6)),
     pch = 2, col = "cornflowerblue",cex=0.6,cex.lab=0.8,
     ylim=c(10,20), xlim=c(0,20),
     xlab=expression(paste(mu," (mean change in IQ score)")),
     ylab=expression(paste(lambda^{-1/2}," (sd change)") ))
points(x=mu.post.c,y=1/sqrt(lambda.post.c),
