---
title: 'STAT 572: Project'
author: "Geovani Bautista, Simon Lee, Shen Qu"
date: Dec, 2019
subtitle: "Chapter 10: Nonconjugate priors and Metropolis-Hastings algorithms"
output:
  pdf_document:
    toc: no
    number_sections: no
...
```{r,eval=F,include=F, height=3.5,width=7}
#### Figure 10.1 
...
```{r,eval=F,include=F, height=1.75,width=5}
#### Figure 10.2
...
\begin{algorithm}[H]
 \KwData{$Y\sim p(\theta)$}
 \KwResult{generates $\theta^{(1)},...,\theta^{(s)} \sim\text{iid }p(\theta|y)$}
 initialization\;
 Choose $\delta$ to make the approximation algorithm run efficiently\; 
 \While{not convergentcy}{
  \For{Symmetric: $J(\theta_b|\theta_a) = J(\theta_a|\theta_b)$}{
  
1. Sample $\theta^\star \sim J(\theta|\theta^{(s)})$; such as \;
 $$J(\theta^\star|\theta^{(s)}) = \text{uniform}(\theta^{(s)}-\delta,\theta^{(s)}+\delta)$$ OR 
 $$J(\theta^\star|\theta^{(s)})=\text{normal}(\theta^{(s)},\delta^2)$$

2. Compute the acceptance ratio
$$r=\frac{p(\theta^\star|y)}{p(\theta^{(s)}|y)}=\frac{p(y|\theta^\star)p(\theta^\star)}{p(y|\theta^{(s)})p(\theta^{(s)})}$$
...
\begin{algorithm}[H]
 \KwData{$Y\sim p(u,v)$}
 \KwResult{$p_0(u,v)$ such as $=p(\theta,\sigma^2|y)$}
 initialization\;

 \While{not convergentcy}{
  \For{$J_u$ and $J_v$ are separate symmetric proposal distributions for $U$ and $V$}{
  
1. update $U$\: 

  a) sample $u^\star \sim J_u(u|u^{(s)})$\; 
  b) compute $$r = \frac{p_0(u^\star,v^{(s)})}{p_0(u^{(s)},v^{(s)})}$$
  c) set 
  \eIf{the ratio $r >1$}
   {$u^{(s+1)}\longleftarrow u^\star$ with probability $\min(1,r)$}
   {$u^{(s+1)}\longleftarrow u^{(s)}$ with probability $\max(0,1-r)$}
   
2. update $V$\: 
   a) sample $v^\star \sim J_v(v|v^{(s)})$\; 
   b) compute $$r = \frac{p_0(u^{(s+1)},v^\star)}{p_0(u^{(s+1)},v^{(s)})}$$
   c) set  
  \eIf{the ratio $r >1$}
   {$v^{(s+1)}\longleftarrow v^\star$ with probability $\min(1,r)$}
   {$v^{(s+1)}\longleftarrow v^{(s)}$ with probability $\max(0,1-r)$}
   }
 }
...
\begin{algorithm}[H]
 ...
 \While{not convergentcy}{
  \For{ $J_u,J_v$ do not depend on $U$ and $V$ values in the sequence previous to the most current values (ensures it is a Markov chain) }{
  
1. Update $U$\: 

  a) Sample $u^\star \sim J_u(u|u^{(s)},v^{(s)})$\; 
  b) Compute 
  $$r = \frac{p_0(u^\star,v^{(s)})}{p_0(u^{(s)},v^{(s)})}\times \frac{J_u(u^{(s)}|u^\star,v^{(s)})}{ J_u(u^\star|u^{(s)},v^{(s)})}$$
...
2. Update $V$\: 
   a) Sample $v^\star \sim J_v(v|u^{(s+1)},v^{(s)})$\; 
   b) Compute the acceptance ratio  
   $$r = \frac{p_0(u^{(s+1)},v^\star)}{p_0(u^{(s+1)},v^{(s)})}\times \frac{J_v(v^{(s)}|u^{(s+1)},v^\star)}{ J_v(v^\star|u^{(s+1)},v^{(s)})}$$
   c) Set  
  \eIf{the ratio $r >1$}
   {$v^{(s+1)}\longleftarrow v^\star$ with probability $\min(1,r)$}
   {$v^{(s+1)}\longleftarrow v^{(s)}$ with probability $\max(0,1-r)$}
 ...
 \caption{The M-H algorithm for approximating $p_0(u,v)$ }
\end{algorithm}



\begin{algorithm}[H]
 \KwData{$Y\sim \text{multivariate normal}(\mathbf{X}\beta_n,\sigma^2\mathbf{I})$}
 \KwResult{$\{\beta^{(s)},\sigma^{2(s)},\rho^{(s)}\}$}
 initialization\;
...
1. Update $\beta$\: 
  ...
  Sample $\beta^{(s+1)} \sim\text{multivariate normal}(\beta_n,\Sigma_n)$\; 
        where $\beta_n$ and $\Sigma_n$ depend on $\sigma^{2(s)}$ and $\rho^{(s)}$\;
   
2. Update $\sigma^2$\: 

  Sample $\sigma^{2(s+1)}\sim\text{inverse-gamma}([\nu_0+n]/2,[\nu_0\sigma^2_0 +SSR_\rho]/2)$\;
        where $SSR_\rho$ depends on $\beta^{(s)}$ and $\rho^{(s)}$\; 

3. Update $\rho$\;

  a) Propose $\rho^\star \sim \text{uniform}(\rho^{(s)} -\delta,\rho^{(s)} + \delta)$\; 
  
  \If {$\rho^\star < 0$}{reassign it to be $|\rho^\star|$}
  
  \If {$\rho^\star > 1$}{reassign it to be $2-\rho^\star$}
  
  b) Compute the acceptance ratio 
  $$r = \frac{p(y|\mathbf{X},\beta^{(s+1)},\sigma^{2(s+1)},\rho^\star)p(\rho^\star)}{p(y|\mathbf{X},\beta^{(s+1)},\sigma^{2(s+1)},\rho^{(s)})p(\rho^{(s)})}$$
 

  Sample $u \sim \text{uniform}(0,1)$\; 
     
  \eIf{$u < r$}{set $\rho^{(s+1)}\longleftarrow \rho^\star$}
      {set $\rho^{(s+1)}\longleftarrow \rho^{(s)}$}
 }
 \caption{The M-H algorithm for longitudinal regression model }
\end{algorithm}
