...
\begin{align*}
p(y_{1:n}|\mu) &=\prod_{i=1}^n \mathbf{1}_{\{y_i\in\mathbb{R}\}}(\frac{\lambda}{2\pi})^{\frac12}\exp[-\frac{\lambda}{2} (y_i-\mu)^2] \\
    ...
    &\propto \exp[-\frac{n \lambda}{2} (\mu^2 - 2 \mu \bar{y})]
\end{align*}
...
p(y_{1:n}|\mu,\lambda)&=\prod_{i=1}^n \mathbf{1}_{\{y_i\in\mathbb{R}\}}(\frac{\lambda}{2\pi})^{\frac12}\exp[-\frac{\lambda}{2} (y_i-\mu)^2] \\
    &= \underbrace{\mathbf{1}_{\{y_i\in\mathbb{R}\}}(2\pi)^{-\frac{n}2}}_{h(y)}\exp[\underbrace{-\frac{\lambda}{2} \sum_{i=1}^n y_i^2+n\lambda \mu\bar{y}}_{\phi(\mu,\lambda)t(y_{1:n})} -n\underbrace{\frac{\lambda\mu^2-\ln\lambda}{2}}_{\kappa(\mu)}] \\
\end{align*}
...
These data are weather observations of many years in two locations. Before analys the data, Our prior experiences tell me that the atmospheric and geographic factors decide the precipitation.

- Independent: 

Not strictly.  From a global perspective, all the climate issues are connected and have interaction with each other. Therefore, compared to a data in a lab, the weather observations are not purely independent. If the researcher think the distance between the two cities is far enough that the interations are negligible, we can consider the data as independent. The effect of time factor may not be independent, previous years value may affects the next year, but it doesn't affect the comparison of two cities.

- Identical: 

Not sure. If the geographic factors are principal causes of weather, the distribution of snowfall in two cities may not be same. If the atomspheric factors are principal causes, the two cities' snowfall records happened in a same "rule" and are identical.
...
We use subjective prior knowledge to assign values for the parameters in the prior (a.k.a. hyperparameters), as
  ...
  + $\mu_0=300$: we don't know the snowfall value on average.
  ...
  +  $\beta=6400\alpha$ this value leads to a expected standard deviation of the changes in the snowfall of about 80 inches/year, since $\sqrt{E(\lambda)}=\sqrt{\alpha/\beta}=1/80$, and so $\sigma\approx 80$ a priori.

Check whether a prior conforms to our beliefs?

  + Draw some samples from the prior and look at them—this is probably the best general strategy.
  + I choose to look at sample hypothetical datasets $x_{1:n_v}, y_{1:n_a}$ drawn using these sampled parameter values.
  +  Plot the cdf and check various quantiles (first quartile, median, third quartile), if univariate.
  +  Plot the pdf.
  +  Look at various moments (e.g., mean, standard deviation).


```{r, echo=F,message=F,collapse=T,out.width='50%',fig.show='hold',fig.align='center'}
#hyperparameters
mu0 <- 300; nu <- 1; alpha <- 1/2; beta <- 6400*alpha
...
plot(x=mu.pr.sample,y=sqrt(1/lambda.pr.sample),
     ...
     xlab=expression(paste(mu," (mean of snowfall)")),
...
From the derivations in the previous section we have that the posterior distributions for $\vec\mu_v,\vec\lambda_v$ and for $\vec\mu_a,\vec\lambda_a$ are given by

\begin{align*}
\text{Valdez: }\vec\mu_v,\vec\lambda_v &\sim \text{Normal-Gamma}(\mu_v^\star,\nu_v^\star,\alpha_v^\star,\beta_v^\star)\\
\text{Aomori: }\vec\mu_a,\vec\lambda_a &\sim \text{Normal-Gamma}(\mu_a^\star,\nu_a^\star,\alpha_a^\star,\beta_a^\star).
\end{align*}
...
\begin{center}
\begin{tabular}{rl|rl}
\multicolumn{2}{c}{Valdez} & \multicolumn{2}{c}{Aomori}\\
\hline
...
Valdez: $\vec\mu_v,\vec\lambda_v \sim$ Normal-Gamma(`r round(mu.v.s,1)`,`r nu.v`,`r alpha.v`,`r round(beta.v,1)`)

Aomori: $\vec\mu_a,\vec\lambda_a \sim$ Normal-Gamma(`r round(mu.a.s,1)`,`r nu.a`,`r alpha.a`,`r round(beta.a,1)`)
...
This data strongly supports the hypothesis that Valdez has more snowfll. This is evidenced in both the figure and by the probability calculated, the posterior probability that the Valdez has a higher mean of snowfall is about `r round(prob.v.greater.a,3)`.
...
I tried four combination of $\mu_0$, $nu$ and $\beta$.  I did not try to change the value of $\alpha$ without obvious evidents,

\begin{center}
\begin{tabular}{c|c|c|c|c}
 &$\mu_0$ & $nu$ & $\alpha$ & $\beta$\\
\hline
set 1 &0   & 1 & 1/2 & 6400$\alpha$\\
set 2 &300 & 1 & 1/2 & 100$\alpha$ \\
set 3 &300 & 4 & 1/2 & 6400$\alpha$\\
set 4 &300 &1/4& 1/2 & 6400$\alpha$\\
\end{tabular}
\end{center}
...
Setting $\mu_0=0$ change the location of of prior distribution. But it doesn't chage the posterior probability that the Valdez has a higher mean of snowfall is about `r round(prob.v.greater.a,3)`.
...
Setting $\sigma\approx 10$ change the scale of prior distribution. But it doesn't chage the posterior probability that the Valdez has a higher mean of snowfall is about `r round(prob.v.greater.a,3)`.

```{r, echo=F,message=F,collapse=T,out.width='40%',fig.show='hold',fig.align='center'}
# hyperparameters set 3
#hyperparameters
mu0 <- 300; nu <- 4; alpha <- 1/2; beta <- 6400*alpha
...
Setting $\nu=4$ a high strenth of one observation to the information coming from the prior on $\mu_v$ and  $\mu_a$. It shows a higher stand diviation but it doesn't chage the posterior probability that the Valdez has a higher mean of snowfall is about `r round(prob.v.greater.a,3)`.


```{r, echo=F,message=F,collapse=T,out.width='40%',fig.show='hold',fig.align='center'}
# hyperparameters set 4
#hyperparameters
mu0 <- 300; nu <- 0.25; alpha <- 1/2; beta <- 6400*alpha
...
Setting $\nu=1/4$ a low strenth of one observation to the information coming from the prior on $\mu_v$ and  $\mu_a$. It shows a lower stand diviation but it doesn't chage the posterior probability that the Valdez has a higher mean of snowfall is about `r round(prob.v.greater.a,3)`.
