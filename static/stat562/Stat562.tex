...
\setlength{\columnsep}{2pt}
	...
	\subsubsection{Borel field}
Borel field($\sigma$-algebra) B be a subset of all possible subset of S.\\
1. $\emptyset\in B$  
2. if $A\in B, then\ A^c\in B$  \\
3. if $A_{1},A_{2},A_{3},...\in B, then\ \bigcup_{n=1}^{\infty}\in B$ 

	\subsubsection{Kolmogorov: probability distribution of X}
Borel field B, P is a probability set function if \\
1. $\forall A\in B,\ P(A)\ge0$;  
2. $P(S)=1$;  \\
3. $A_{1},A_{2}$,..pairwise disjoint, $\ P(\bigcup_{n=1}^{\infty}A_i)=\sum_{i=1}^{\infty}P(A_{i})$; 
...
$P(\emptyset) =0$;
$\forall A, P(A)= 1-P(A')$;
$A,B \in \mathcal{S}, P(A \cup B) = P(A) + P(B) - P(A\cap B)	$;
$A,B,C \in \mathcal{S}, P(A \cup B \cup C) = P(A) + P(B) + P(C) - P(A\cap B) - P(A\cap C) - P(B \cap C) + P(A \cap B \cap C)$

Conditional/Baye's: $P(A|B) = \frac{P(A\cap B)}{P(B)}=\frac{P(B|A)P(A)}{P(B)}$;disjoint
$P(B) = P(B|A_1)P(A_1) +..P(B|A_k)P(A_k)=\sum_{i=1}^{k} P(B|A_i)P(A_i)$ \\
\textbf{Independence}: 
\begin{tabular}{l|l}
$P(A\cap B) = P(A)P(B)$ & $P(X\le x,Y\le y)=P(X\le x)P(Y\le y)$ \\
$E(XY)=E(X)E(Y)$        & $F_{X,Y}(x,y)=F_X(x)F_Y(y)$ \\
$V(X\pm Y) =VX+VY$   & $f_{X,Y}(s,t)=f_X(s)f_Y(t)$
\end{tabular}
...
\begin{tabular}{ l|l|l }
$c\sin{x}\ge 0, 0<x<\pi/2$  &  $\int_{0}^{\frac\pi2}c\sin{x}dx=1$ & $c=1$ \\
$ce^{-|x|}\ge 0, -\infty<x<\infty$ & $c\int_{-\infty}^{0}e^{x}dx+c\int_{0}^{\infty}e^{-x}dx=1$ & $c=\frac12$
\end{tabular}
...
$V(aX+b) = a^2\sigma^2$         & $V(X\pm Y) =VX+VY\pm 2Cov(X,Y)$ \\
$\sigma_{ax+b} = |a| \cdot \sigma_{x}$ & $Cov(x,y)=E(XY)-E(X)E(Y)$
\end{tabular}
$V(X)=\sigma_{x}^2= E(X^2) - [E(X)]^2$

\begin{tabular}{ l|l|l|l }
$f_X(x)$ & $EX$ & $EX^2$ & $V(X)$ \\
$\frac1n$ & $\sum_1^nx\frac1n$ & $\sum_1^nx^2\frac1n$ &  \\
 & $\frac1n\frac{n(n+1)}2$ & $\frac1n\frac{n(n+1)(2n+1)}6$ & $\frac{n^2-1}{12}$ \\ \hline
$ax^{a-1}$ & $\int_0^1xax^{a-1}dx$ & $\int_0^1x^2ax^{a-1}dx$ & $\frac{a}{a+2}-(\frac{a}{a+1})^2$ \\
 & $\frac{a}{a+1}x^{a+1}|_0^1$ & $\frac{a}{a+2}x^{a+2}|_0^1$ & $\frac{a}{(a+2)(a+1)^2}$ \\ \hline
$\frac32(x-1)^2$ & $\int_0^2x\frac32(x-1)^2dx$ & $\int_0^2x^2\frac32(x-1)^2dx$ & $\frac85-1^2$ \\
 & $\frac32(\frac{x^4}4|_0^2..)$ & $\frac32(\frac{x^5}5|_0^2..)$ & $\frac35$
\end{tabular}

	\subsubsection{median/mode}

$F(m) \ge 0.5$ and $P(X \ge m) \geq 0.5$. For $X$ continuous, m satisfies $\int_{-\infty}^mf(x)dx=\int_m^\infty f(x)dx=\frac12$ is half the area under the density curve; For a symmetric curve, this is the point of symmetry.
$\tilde x = ( \frac{n+1}{2} )^{\text{nth}}$ {\scriptsize observation}; X is mode when $f(x)'=0$.
...
This value also satisfies another part $P(X\ge m)$

$f(x)=\frac1{\pi(1+x^2)},-\infty<x<\infty$
$F_X(x)=\int{f(x)dx}=\int\frac1{\pi(1+x^2)}=\frac{\arctan x}{\pi},\quad -\infty<x<\infty$
$P(X\le m)=\int_{-\infty}^m{f(x)dx}=\left.\frac{\arctan x}{\pi}\right|_{-\infty}^m=\frac{\arctan m+\frac{\pi}2}{\pi}=\frac12$ \\ $\therefore \arctan m=0, m=0$ also satisfies another part $P(X\ge m)$
...
\subsubsection{Inequalities/Approximation}
\textbf{Chebyshev}: 
None negative function X; Positive constant r
