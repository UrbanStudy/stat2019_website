...
From Theorem 5.4.4, $f_{X_{(j)}}(x)=\frac{n!}{(j-1)!(n-j)!}f_X(x)[F_X(x)]^{j-1}[1-F_X(x)]^{n-j}$. $f_X(x)=e^{-(x-\mu)}$, $F_X(x)=1-e^{-(x-\mu)}$, then
...
$L(\theta|\mathbf{x})$ was a increasing function of $\theta$. So to maximize $L(\theta|\mathbf{x})$, we should use the largest possible value of $\theta$. $\hat\theta_{MLE}=X_{(1)}$
...
Thus, $\hat\alpha_{MLE}=\frac{n}{n\ln\beta-\ln(\prod_{i=1}^n x_i)}=\frac{n}{n\ln X_{(n)}-(\ln X_{(1)}+..+\ln X_{(n)})}=\frac{n}{(\ln X_{(n)}-\ln X_{(1)})+..+(\ln X_{(n)}-\ln X_{(n)})}=\frac{n}{\sum_{i=1}^n(\ln X_{(n)}-\ln X_{(i)})}$
...
For $(-\sum_{i=1}^n\ln x_i)^{-1}\sim$ Inv-Gamma$(n,\theta)$ 

(Note:$-\ln x_i\sim Expo(\theta)$, $-\sum_{i=1}^n\ln x_i\sim Gamma(n,\theta)$)
...
**7.22** This exercise will prove the assertions in Example 7.2.16, and more. Let $X_1,..,X_n$ be a random sample from a $n(\theta,\sigma^2)$ population, and suppose that the prior distribution on $\theta$ is $n(\mu,\tau^2)$. Here we assume that $\sigma^2, \mu, \tau^2$ are all known.
...
where $\alpha+\frac{n-1}2,[\frac{(n-1)S^2}2+\frac1\beta]^{-1}$ are positive constants. Show that the posterior distribution of $\sigma^2$ is $IG(\alpha,\beta)$. Find the mean of this distribution, the Bayes estimator of $\sigma^2$.
