...
For $Y\sim Uni(0,c)$ and $c=\max\limits_{a\le x\le b}f(x)$, $f_Y(y)=\frac1{c}$ and $P(Y<f(X))=1$.

For $X \sim Uni(a,b)$, $X$ and $Y$ being independent, $f_{X,Y}(x,y)=\frac1{c}\frac1{b-a}$.

If $Y<f(X)$, $W=X$ means 

$$P(W\le w)=P(a\le X\le w|Y<f(X))=\frac{P(a\le X\le w,Y<f(X))}{P(Y<f(X))}=\frac{F_{X,Y}(x,y)}{1}=\int^w_a\int^{f(x)}_0f_{X,Y}(x,y) dsdt=\frac1{c(b-a)}\int_a^w f(t)dt$$
...
plot(c(0,40), c(0,0.15), type = "n",main="Geom(1/(c(b-a)))",xlab ="n", ylab ="p")
...
abline(v=2,lty=2);text(2,0, "a", col = 2, adj = c(1.5,0));abline(v=37,lty=2);text(37,0, "b", col = 2, adj = c(-.5,-1))
```

Let $f(t)\sim Geom(\frac1{c(b-a)}),x\in[a,b]$. The area of box is $c(b-a)$. $P(W\le w)$ is the probability of the shaded area.

$$c(b-a)P(W\le w)=\int^w_a\int^{f(x)}_0 dsdt=\int_a^w f(t)dt$$

Thus, as long as $c\ge\max\limits_{a\le x\le b}f(x)$, $F_W(w)=\frac1{c(b-a)}\int_a^w f(t)dt$. By the definition 1.6.3 (pdf) , this algorithm generates a random variable with pdf $f(x)$ from the uniform random variables.
...
Let $N=$ number of $(X,Y)$ pairs required for one $W$, then $N$is a $Geom(\frac1{c(b-a)}$ random ariable. Thus to generate one $W$, we expect to need $E(N)=c(b-a)$ pairs  $(X,Y)$ and in this sense minimizing $c(b-a)$will optimize the algorithm.
