...
Let $X=\frac{n-1}{\sigma^2}S^2\sim\chi^2_{n-1}$, $Y=S^2=\frac{\sigma^2}{n-1}X$, we have $f_{Y}(y)=f_X(x)|\frac{dx}{dy}|$

By given $\sigma^2$ and the pdf of $\chi^2_{n-1}$ is $f(x)=\frac{1}{2^{\frac{p}2}\Gamma(\frac{p}2)}x^{\frac{p}2-1}e^{-\frac{x}2}$

$$f_Y(S^2|\sigma^2)=f_X(\frac{n-1}{\sigma^2}S^2|\sigma^2)\left|\frac{d(\frac{n-1}{\sigma^2}S^2)}{d(S^2)}\right|=\frac{1}{2^{\frac{n-1}2}\Gamma(\frac{n-1}2)}\left(\frac{n-1}{\sigma^2}S^2\right)^{\frac{n-1}2-1}e^{-\frac{n-1}{2\sigma^2}S^2}\cdot\frac{n-1}{\sigma^2}$$
...
Consider $\frac{\sum(X_i-\bar X)^2}{k}$ as an estimator of $\sigma^2$ (under normality). Find the K that minimizes MSE

For $X_1,..X_i\sim iid\ n(\mu,\sigma^2)$, then $\frac{(n-1)S^2}{\sigma^2}\sim\chi^2_{n-1}$, $S^2=\frac{\sum(X_i-\bar X)^2}{n-1}$.

We have $E[S^2]=\sigma^2$ and $Var[\frac{(n-1)S^2}{\sigma^2}]=2{n-1}$, $Var[S^2]=\frac{2\sigma^4}{n-1}$.

Let $M=\frac{\sum(X_i-\bar X)^2}{k}=\frac{(n-1)}{k}S^2$, then

$Var[M]=Var[\frac{(n-1)}{k}S^2]=\frac{(n-1)^2}{k^2}Var[S^2]=\frac{2(n-1)\sigma^4}{k^2}$

$E[M]=E[\frac{(n-1)}{k}S^2]=\frac{(n-1)}{k}E[S^2]=\frac{n-1}{k}\sigma^2$

$Bias[M]=E[M]-\sigma^2=\frac{n-k-1}{k}\sigma^2$

$MSE[M]=Var[M]+Bias^2[M]=\frac{2(n-1)}{k^2}\sigma^4+\frac{(n-k-1)^2}{k^2}\sigma^4=\frac{n^2+k^2-2nk+2k-1}{k^2}\sigma^4$

Set derivtive $MSE'[M]=\frac{(2k-2n+2)k^2-2k(n^2+k^2-2nk+2k-1)}{(k^2)^2}=\frac{2(nk-k+1-n^2)}{k^3}=0$, get $k=n+1$.

Therefore, when $k=n+1$, $\min{MSE[M]}=\frac2{n+1}\sigma^4$
