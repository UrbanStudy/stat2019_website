...
Let $a(\theta)=$ the number of elements in $\mathcal{A}(\theta)$ and $b(\theta)=$ the number of elements in $\frac{f(\mathbf{x}|\theta)}{f(\mathbf{y}|\theta)}$. Set $\mathcal{A}(\theta)=\{i:xi\le\theta\},\mathcal{B}(\theta)=\{i:yi\le\theta\}$, then 
...
The pdf of R is $h(r|\theta)=n(n-1)r^{n-2}(1-r),0<r<1$. $h(r|\theta)\sim Beta(n-1,2)$. $E[R]=(n−1)/(n+1)$. Thus $R−E[R]=X_{(n)}−X_{(1)}−(n−1)/(n+1)\not\equiv0$, $E[R−ER]=0$.

For $T(\mathbf{X})=(X_{(1)},X_{(n)})$, $g[T(\mathbf{X})]\not\equiv0$ and $Eg[T(\mathbf{X})]=Eg[X_{(n)},X_{(1)}]=0$ for all $\theta$.

Therefore, $T(\mathbf{X})=(X_{(1)},X_{(n)})$ is not complete. An ancillary statistic is not a complete sufficient statistic.

 ---

**6.13** Suppose $X_1$ and $X_2$ are iid observations from the pdf $f(x|\alpha)=\alpha x^{\alpha-1}\mathrm{e}^{-{x^\alpha}}, x>o,\alpha>O$. Show that $(\log X_1)/(\log X_2)$ is an ancillary statistic.
