...
By 5.6.3 the exponential-uniform transformation, $-\ln{U}=-\theta\ln x_i\sim Expo(\theta)$; $-\theta\sum_{i=1}^n\ln x_i\sim Gamma(n,\theta)$; $(-\theta\sum_{i=1}^n\ln x_i)^{-1}\sim Inv-Gamma(n,\theta)$.

For the pdf of Inv-Gamma$(\alpha,\beta)$ is $f_{X}(x)=\frac{\beta^{\alpha}}{\Gamma(\alpha)}x^{-\alpha-1}e^{-\frac\beta{x}},x>0$, $E[x^n]=\frac{\beta^n}{(\alpha-1)\cdots(\alpha-n)}$.
