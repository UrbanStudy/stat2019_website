...
So $g(T)=0\quad\forall\mu$, then P(g(T)=0)=1, $X_{(1)}$ is a complete statistic.
...
$S^2_{\mu}$ is a function of only $Z_1,..,Z_n$ and be free of $\mu$. S^2 is an ancillary statistic. Therefore, $S^2$ and  $X_{(1)}$ are independent by Basu's Theorem.
...
$L(\theta|\mathbf{x})$ was a increasing function of $\theta$. So to maximize $L(\theta|\mathbf{x})$, we should use the largest possible value of $\theta$. $\hat\theta=X_{(1)}$
...
By given $F(\mathbf{x}|\alpha,\beta)$, $$f(\mathbf{x}|\alpha,\beta)=\begin{cases}0&\text{if }x<0\\\frac{\alpha}{\beta^\alpha}x^{\alpha-1}&\text{if }0\le x\le\beta\\0&\text{if } x>\beta\end{cases}$$
...
$$f(\mathbf{x}|\theta)=\prod_{i=1}^n\theta x_i^{\theta-1}=\theta^n(\prod_{i=1}^n x_i)^{\theta-1}$$
...
For $(-\sum_{i=1}^n\ln x_i)^{-1}\sim Inv-Gamma(n,\theta)$ ($-\ln x_i\sim Expo(\theta)$, $-\sum_{i=1}^n\ln x_i\sim Gamma(n,\theta)$)
