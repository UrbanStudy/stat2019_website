...
$f_X(x)=\frac1{b-a},\int_a^bf_X(x)dx=1$, $f_Y(y)=\frac1{c},\int_0^cf_Y(y)dy=1$. Set $w=b$, then we have $P(Y<f(X))=\int^b_a\int^{c}_0 dydx={c(b-a)}$.

$F_W(w)=F(X|Y)$.

$$P(W\le w)= P(a\le X\le w|Y<f(X))=\frac{P(a\le X\le w,Y<f(X))}{P(Y<f(X))}=\frac{\int^w_a\int^{f(x)}_0 dydx}{\int^b_a\int^{c}_0 dydx}=\frac{\int^w_af(x)dx}{\int^b_acdx}=\frac1{c(b-a)}\int_a^w f(x)dx$$
...
points(c(0:20),dgeom(c(0:20), 1/7.5, log = FALSE))
lines(c(0:20), dgeom(c(0:20), 1/7.5, log = FALSE), col = 2)
```

Let $N=$ number of $(X,Y)$ pairs required for one $W$, then $ $is a g$Gome \frac1{c(b-a)} $random
 ariable. Thus to generate one Y$W$we expect to need E$ (N)  (b-a)$pairs  $UX Y,$ and in
t is sense minimizing c$c(b-a)$will optimize the algorithm.
