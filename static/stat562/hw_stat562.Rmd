...
(b). Use Basu's Theorem to show that $X_{(1)}$ and $S^2$ are independent.
 ...
 $X\sim$ location $Expo(\mu)$, $\frac{f(\mathbf {x}|\mu)}{f(\mathbf {y}|\mu)}=\frac{\prod\limits_{i=1}^n(ne^{n\mu}e^{-nx_i})}{\prod\limits_{i=1}^n(ne^{n\mu}e^{-ny_i})}=\frac{\mathrm{e}^{\sum_{i=1}^ny_i}I_{(\mu,\infty)}(\min x_i)}{\mathrm{e}^{\sum_{i=1}^nx_i}I_{(\mu,\infty)}(\min y_i)}$
...
$f(\mathbf{x}|\theta)$ is a decreasing function of $X_i$, $X_{(1)}=\min_iX_i$.
