...
plot(c(0,20), c(0,0.15), type = "n",main="Geom(1/(c(b-a)))",xlab ="n", ylab ="p")
points(c(0:20),dgeom(c(0:20), 1/7.5, log = FALSE))
lines(c(0:20), dgeom(c(0:20), 1/7.5, log = FALSE), col = 2)
abline(v=2.5*(5-2),lty=2);text(7.5,0, "c(b-a)", col = 2, adj = c(-.1,0))
```

Let $N=$ number of $(X,Y)$ pairs required for one $W$, then $N$is a $Geom(\frac1{c(b-a)}$ random ariable. Thus to generate one $W$, we expect to need $E(N)  (b-a)$ pairs  $(X,Y)$ and in this sense minimizing $c(b-a)$will optimize the algorithm.
