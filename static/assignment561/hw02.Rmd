...
(b) If A and B are independent, they cannot be mutually exclusive.
 ...
 > Acrroding the definition of statistically independent, When A and B are independent, $P(A\cap{B})=P(A)P(B)$. For P(A)>0 and P(B)>0, P(A\cap{B})>0, which means $A\cap B\ne\emptyset$. Therefore, A and B does not satisfy the definition of **paiwise disjoint**
...
$$\lim_{x\to\infty}F(x)=\frac12+\frac1\pi(\frac\pi2)=1$$
 ...
 > This function satisfies the condition a. Differentiating Fx(x) gives
...
$$F_Y(y)=\begin{cases}\frac{1-\epsilon}{1+e^{-y}}，& y<0\\\epsilon+\frac{1-\epsilon}{1+e^{-y}}，& y\ge0\end{cases} $$
 ...
 > No jump occurs when F(y) approaches 0 from the right. And $0<\epsilon<1$. This function satisfies the condition a and c. Differentiating Fx(x) gives
