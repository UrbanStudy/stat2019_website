...
$$\because Y=d\tan X,\quad 0<x<\pi/2\quad \therefore X=\arctan\frac Yd$$

<!--
$$\because f_X(x)=x,\ 0<x<\pi/2,\quad \therefore F_X(x)=\int f_X(x)=\int x=\frac{x^2}2 $$
$$F_Y(y)=P(Y\le y)=P(d\tan X\le y)=P(X\le\arctan\frac Yd)$$
$$=F_X(\arctan\frac yd)=\frac12(\arctan\frac yd)^2,\ 0<\arctan\frac yd<\pi/2$$ -->

 > $Y=d\tan X$ is a monotone inecreasing function. We can use shortcut.

$$f_Y(y)=f_X(x)|\frac{dx}{dy}|=\arctan\frac yd|\frac{d(\arctan\frac yd)}{dy}|=\arctan(\frac yd)\frac{1}{[(\frac yd)^2+1]d}=?=\frac{2}{\pi[(\frac yd)^2+1]d}$$

 > This is the Cauchy distribution restricted to (0,1), and the mean is infinite.

$$E[Y]=\int_0^{\frac\pi2}d\tan x\sim\infty$$
...
$$F_X(x)=\int{f(x)dx}=\int3x^2=x^3$$

$$F_M(m)=P(M\le m)=P(X\le m)=F_X(m)=m^3=\frac12$$
...
$$F_X(x)=\int{f(x)dx}=\int\frac1{\pi(1+x^2)}=\left.\frac{\arctan x}{\pi}\right|_{-\infty}^\infty=\frac1\pi(\frac\pi2+\frac\pi2)=1$$
