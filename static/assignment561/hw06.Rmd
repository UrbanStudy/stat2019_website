...
According to the L’Hˆopital’s rule, $M_{aX+b}(t)=e^{bt}M_X(at)$
$$\therefore M_{pY}(t)=(\frac{p}{1-(1-p)e^{pt}})^r$$
...
$$\therefore \lim_{p\rightarrow0}\frac{p}{1-(1-p)e^{pt}}=\lim_{p\rightarrow0}\frac{1}{-(1-p)'e^{pt}-(1-p)te^{pt}}=\lim_{p\rightarrow0}\frac{1}{e^{pt}-(1-p)te^{pt}}=\frac1{1-t}$$

$$\implies M_{pY}(t)=(\frac1{1-t})^r$$

$$Gamma (α,β)\quad f_{X}(x)={{{(x/\beta)^{\alpha-1}{\exp}(-x/\beta)}[{\beta\Gamma (\alpha)]^{-1}}}},\ x\ge0,\ M_X(t)=(\frac1{1-\beta t})^\alpha$$
...
$$\therefore f_Y(y)=f_X(y^\gamma)|\frac{dy^\gamma}{dx}|=\frac1\beta e^{-\frac{y^\gamma}{\beta}}\gamma y^{\gamma-1}, x\ge0,\beta>0, \gamma>0$$

$$EY=\int_0^{\infty}\frac\gamma\beta  yy^{\gamma-1}e^{-\frac{y^\gamma}{\beta}}dy=\gamma \int_0^{\infty}\frac{y^{\gamma}}\beta  e^{-\frac{y^\gamma}{\beta}}dy$$

This is a Weibull distribution, 
$$EY=\beta^{\frac1\gamma}\Gamma(1+\frac1\gamma),\quad VarY=\beta^{\frac2\gamma}[\Gamma(1+\frac2\gamma)-\Gamma^2(1+\frac1\gamma)]$$

Let $u=\frac{y^\gamma}{\beta}$, $dy=\frac{\gamma y^{\gamma-1}}{\beta}du$
...
$$\therefore f_Y(y)=f_X(\frac{\beta y^2}2)|\frac{d\frac{\beta y^2}2}{dx}|=\frac1\beta e^{-\frac{\beta y^2}{2\beta}}\beta y=ye^{\frac{-y^2}2}, y\ge0,\beta>0$$

This is a Rayleigh distribution with $\beta=\sqrt2$.

$$EY=\int_0^{\infty}yye^{\frac{-y^2}2}dy=\frac{\sqrt{2\pi}}{2}$$

$$EY^2=\int_0^{\infty}y^3e^{\frac{-y^2}2}dy=2$$
