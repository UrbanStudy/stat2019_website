...
# when K down to 31, the defective might not be chosed on 0.1 probability.
qhyper(0.1, 6, 94, 31, lower.tail = TRUE, log.p = FALSE)

# by function of phyper
uniroot(function(k)(phyper(0.1, 6, 94, k, lower.tail = TRUE, log.p = FALSE)-0.1), lower=0, upper=100)$root
...
```{r}
# by function of phyper
uniroot(function(k)(phyper(1, 6, 94, k, lower.tail = TRUE, log.p = FALSE)+phyper(0, 6, 94, k, lower.tail = TRUE, log.p = FALSE)-0.1), lower=0, upper=100)$root
...
```{R}
uniroot(function(x)(exp(x)-100*x-100), lower=0, upper=1000)$root
```

$$\therefore min(EX)=min(\lambda)=7$$
