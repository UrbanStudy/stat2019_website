...
$$\Gamma(\frac{2k+1}2)=(k-\frac12)\Gamma(k-\frac12)=(k-\frac12)(k-\frac32)\Gamma(k-\frac32)=...=\prod_{i=1}^k(k-i+\frac{1}2)\Gamma(\frac12)=\prod_{i=2}^{k}(i-\frac{1}2)\sqrt{\pi}$$
...
$$=1-\frac{12^9}{9!}e^{-12}=0.7576078$$

or following a Gamma process with $\alpha=10,\beta=1.5$. 

$$\frac{1}{\Gamma(10)1.5^{10}}8^{10-1}e^{-\frac{8}{1.5}}=0.7576078$$

```{r, include=FALSE}
1-ppois(9, 8/(2/3), lower.tail = TRUE, log.p = FALSE)

pgamma(8, 10, rate = 1.5, lower.tail = TRUE,log.p = FALSE)
...
$$(a-1+\frac{x}\beta)=0\implies x=\frac{1-\alpha}\beta$$
...
Let x is the number of draws. When $x=1,3,5,7..$, the probability of achieving this goal is 0. When $x=2,4,6,8..$, the probability of achieving this goal is $\frac12$. 

let $y=\frac{x}2$. For $Y\sim Geometric(\frac12)$

$$f_Y(y)=p(1-p)^{y-1}=2^{-y}=2^{-\frac{x}2}, x=2,4,6..$$

$$f_X(x)=\begin{cases}2^{-\frac{x}2}& x=2,4,6..\\ 0 & x=1,3,5.. \end{cases}$$

Because the mean and variance of Geometric function just related with p, thus

$$E[X]=(\frac{1}p)=2;\quad Var[X]=\frac{1-p}{p^2}=2$$
...
In an introductory course on mathematical statistics, the following pmfs are typically
covered: binomial, Poisson, negative binomial, and hypergeometric. The Bernoulli
and geometric pmfs are merely special cases of the binomial and negative binomial
distributions, respectively. We illustrate the mode-determining condition (1) of
the previous section by using two cases that appear to lie on opposite ends of the
computational continuum: the Poisson and hypergeometric pmfs.

The Poisson pmf arises both as a stochastic process in its own right and as an
approximation to the binomial. The Poisson pmf is given by:

$$P(X=x)=p_x=\frac{\lambda^x}{x!}e^{-\lambda}, x=0,1,2.., \lambda>0\quad (2)$$

Applying the first inequality of condition and forming the ratio :

$$\frac {p_x}{p_{x-1}}=\frac{\frac{\lambda^x}{x!}e^{-\lambda}}{\frac{\lambda^{x-1}}{(x-1)!}e^{-\lambda}}\ge1$$

$\frac{\lambda}x\ge1$ or $x\le\lambda\quad (4)$

In applying the second inequality it is apparent that x has been replaced with x+1
and the direction of the inequality has reversed; this obviates the necessity of
computations (3) and (4), immediately leading to:

$x+1\ge\lambda$ or $x\ge\lambda-1$

Since x is a non-negative integer which also must satisfy $\lambda-1\le x\le \lambda$, so $x=\lambda$ where $\lambda$ denotes the greatest integer that does not exceed $\lambda$. If the parameter $\lambda$ is itself a positive integer then, by virtue of $\lambda-1\le x\le \lambda$, the distribution is
bimodal with the two modes located at $x=\lambda-1$ and $x=\lambda$.

Therefore, $\mu\in(1,2)$ for x=1 is the unique mode.
...
**3.1** Let Z be a random variable with pdf f(z). Define $z_{\alpha}$ to be a number that satisfies this relationship:
...
#### Cauchy family
 ...
 defined in Section 3.3. This family can be extended to a location-scale family yielding pdfs of the form.
