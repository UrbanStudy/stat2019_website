...
$$\Gamma(\frac{2k+1}2)=\frac{2k-1}2\Gamma(\frac{2k-1}2)=\frac{2k-1}2\frac{2k-3}2\Gamma(\frac{2k-3}2)=...=\prod_{1}^{k}\frac{2k-1}{2}\Gamma(\frac12)=\frac{(2k)!}{4^{k}k!}\sqrt{\pi}$$
...
$$\therefore \Gamma(\frac{2k+1}2)=\begin{cases}\frac{(2k)!}{4^{k}k!}\sqrt{\pi} & when\quad k\ge0\\\frac{(-4)^{-k}(-k)!}{(-2k)!}\sqrt{\pi} &  when\quad k<0\end{cases}$$
...
$$f_X(x)=\binom{x-1}{r-1}p^r(1-p)^{x-r}=\binom{x-1}{1}\frac12^2\frac12^{x-2}=(x-1)2^{-x}, x \in r,r+1,r+2..$$

$$E[X]=\frac{r}p=4;\quad Var[X]=\frac{r(1-p)}{p^2}=4$$
...
For CDF of Geometric function, $\lim_{x\to\infty}\sum_{x=1}^\infty p(1-p)^{x-1}=1, x=1,2,3,..$

And for X is a probability function, $\lim_{x\to\infty}F(X)=1$

$$lim_{x\to\infty}F(X)=c\left[1-1+(1-p)^{a-1}\right]=c(1-p)^{a-1}=1\implies c=(1-p)^{1-a}$$
