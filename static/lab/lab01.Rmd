...
1. |  sample regression model | $Y_i=\beta_1+\beta_0X_i+\epsilon_i,\quad i=1,2,3...n$
1. |  fitted regression model (fitted value & least-squares estimators | $\hat{Y_i}=\hat{\beta_1}+\hat{\beta_0}X_i$
 (1). | slope   | $\hat{\beta_1}=\frac{S_{xy}}{S_{xx}}=\sum_{i=1}^nC_iY_i$
 (1). | The solution to thenormal equations (Intercept) |  $\hat{\beta_0}=\overline Y-\hat{\beta_1}\overline X=\frac1n{\sum_{i=1}^nY_i}-\overline X(\sum_{i=1}^n C_iY_i)=\sum_{i=1}^n(\frac1n-\overline{X}C_i)Y_i$
 1 |         | $C_i=\frac{X_i-\overline X}{S_{xx}}$
 1. | sample mean  | $\overline X=\frac1n\sum_{i=1}^n X_i$
 2. | sum of cross-products | $S_{xx}$ 
...
$$\text{According 1, 2}\quad=\frac1n\sum_{i=1}^n[\beta_0+\beta_1X_i+E(\epsilon_i)]-\overline X\sum_{i=1}^nC_i[\beta_0+\beta_1X_i+E(\epsilon_i)]=\beta_0+\beta_1\frac{\sum_{i=1}^nX_i}n-\beta_0\overline X\sum_{i=1}^nC_i-\beta_1\overline X\sum_{i=1}^nC_iX_i$$
 ...
 $$\because \sum_{i=1}^nC_i=\sum_{i=1}^n\frac{X_i-\overline X}{}$$
