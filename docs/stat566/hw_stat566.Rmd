...
```{r echo=T,message=F, results='hold', out.width='30%'}
...
$$y_{ij}=\mu+\tau_i+\beta_{blk}+\varepsilon_{ij}$$
for $i=1,2$ ; $blk=1,2$; $j=1,2,3,4$, where 

$\tau_i$ is fixed main effect of $i^{th}$ level of  Factor D;

$\beta_{blk}$ is effect of $j^{th}$ level of block;

$\varepsilon_{ij}$ is random error for the four replicates Exerimental Unit when $i^{th}$ level of Factor D and $j^{th}$ level of block are applied.

$y_{ijk}$ is response for the 4 replicate EU when  $i^{th}$ level of Factor D and  $j^{th}$ level of block are applied.
