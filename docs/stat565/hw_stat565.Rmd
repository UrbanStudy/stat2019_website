...
$$\sum_{i=1}^aQ_i=b\bar y_{..}-\frac{\bar y_{..}}kkb=0$$
...
For $τ_i$ is a constant of the $i^th$ treatment, $E{τ_i}=τ_i,V{τ_i}=0$
...
$$V[\bar y_{i..}-\bar y_{...}]=V[τ_i+\bar ε_{i..}-\bar ε_{ijk}]=V[\tau_i]+V[\frac{\sum\limits_{j=1}^b\sum\limits_{k=1}^nε_{ijk}}{bn}]+V[\frac{\sum\limits_{i=1}^a\sum\limits_{j=1}^b\sum\limits_{k=1}^nε_{ijk}}{abn}]+2Cov[τ_i,\bar ε_{i..}]-2Cov[τ_i,\bar ε_{...}]-2Cov[\bar ε_{i..},\bar ε_{...}]$$ 

For $τ_i$ and $ε_{ijk}$ are independent. $Cov\left[τ_i,\bar ε_{i..}\right]=Cov[τ_i,\bar ε_{...}]=0$

$$=0+\frac{σ^2}{bn}+\frac{σ^2}{abn}+0-0-2Cov[\frac{\sum\limits_{j=1}^b\sum\limits_{k=1}^nε_{ijk}}{bn},\frac{\sum\limits_{i=1}^a\sum\limits_{j=1}^b\sum\limits_{k=1}^nε_{ijk}}{abn}]=\frac{σ^2}{bn}+\frac{σ^2}{abn}-\frac{2}{ab^2n^2}\sum_{j=1}^b\sum_{k=1}^nCov[ε_{ijk},\sum_{i=1}^aε_{ijk}]$$ 

For $ε_{ijk}\sim iidN(0,σ^2)$, $Cov\left[ε_{ijk},\sum_{i=1}^aε_{ijk}\right]=\sigma^2$

$$=\frac{σ^2}{bn}+\frac{σ^2}{abn}-\frac{2}{ab^2n^2}\sum_{j=1}^b\sum_{k=1}^nσ^2=\frac{a-1}{abn}σ^2$$ 
