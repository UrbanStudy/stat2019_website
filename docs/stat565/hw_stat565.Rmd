...
ggplot(table_ingredients,aes(Trt,student_resid))+geom_dotplot(binaxis = "y", stackdir = "center")+theme_light()
ggplot(table_ingredients,aes(Day,student_resid))+geom_dotplot(binaxis = "y", stackdir = "center")+theme_light()
ggplot(table_ingredients,aes(Batch,student_resid))+geom_dotplot(binaxis = "y", stackdir = "center")+theme_light()
```
...
ggplot(table_LSD2,aes(Trt,y))+geom_point(aes(size=Col,shape=Row,alpha=.6))+facet_grid(.~Replicate)+theme_light()
...
$$\sum_{i=1}^aQ_i=\sum_{i=1}^a[y_{i.}-\sum_{j=1}^bn_{ij}\bar y_{.j}]=y_{..}-\sum_{j=1}^b(\bar y_{.j}\sum_{i=1}^an_{ij})=y_{..}-y_{..}\sum_{j=1}^b\sum_{i=1}^an_{ij}=0$$
...
$$SS_A=bn\sum_{i=1}^a(\bar y_{i..}-\bar y_{...})^2=bn\sum_{i=1}^a(τ_i+\bar ε_{i..}-\bar ε_{...})^2=bn\sum_{i=1}^aτ_i^2+bn\sum_{i=1}^a\bar ε_{i..}^2+bn\sum_{i=1}^a\bar ε_{...}^2+--$$ 

$$E[SS_A]=bn\sum_{i=1}^aE[τ_i^2]+bn\sum_{i=1}^aE[\bar ε_{i..}^2]+bn\sum_{i=1}^aE[\bar ε_{...}^2]+2bnCov[\sum_{i=1}^aτ_i,\sum_{i=1}^a\bar ε_{i..}]-2bnCov[\sum_{i=1}^aτ_i,\sum_{i=1}^a\bar ε_{...}]-2bnCov(\sum_{i=1}^a\bar ε_{i..},\sum_{i=1}^a\bar ε_{...})$$ 

$$=bn\sum_{i=1}^a(V[τ_i^2]-E[τ_i]^2)+bn\sum_{i=1}^aE[\bar ε_{i..}^2]+bn\sum_{i=1}^aE[\bar ε_{...}^2]+0-0-2bnCov(\sum_{i=1}^a\bar ε_{i..},\sum_{i=1}^a\bar ε_{...})$$ 

$$=abn(σ^2_τ-τ_i^2)+\frac{a}{bn}σ^2+\frac{1}{abn}σ^2-2abnσ^2=abn(σ^2_τ-τ_i^2)+\frac{a^2+1-a^2b^2n^2}{abn}σ^2$$ 