...
$P_{adj2}=0.5111>P_{adj1}=0.5141>P_0=0.435$
...
The ANOVA for RCBD shows that ingredients have significant effects on the average values of reaction time at 0.05 significant level. The P-value of 0.000488 for treatment is samll enough.

 ---

 (d) Test the significance of different batches of material at α=0.05. Report your conclusion and p value here.

The reaults of TukeyLSD test show that batches have not significant differents on the average values of reaction time at 0.05 significant level. The P-value of all the paired comparing among batches is large enough.
...
$$V[\bar y_{i..}-\bar y_{...}]=V[τ_i+\bar ε_{i..}-\bar ε_{ijk}]=V[\tau_i]+V[\frac1{bn}{\sum\limits_{j=1}^b\sum\limits_{k=1}^nε_{ijk}}]+V[\frac1{abn}{\sum\limits_{i=1}^a\sum\limits_{j=1}^b\sum\limits_{k=1}^nε_{ijk}}]+2Cov[τ_i,\bar ε_{i..}]-2Cov[τ_i,\bar ε_{...}]-2Cov[\bar ε_{i..},\bar ε_{...}]$$
