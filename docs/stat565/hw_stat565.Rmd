...
1-pf(0.8794357, 3, 5, lower.tail = TRUE, log.p = FALSE)
1-pf(0.8721288, 3, 5, lower.tail = TRUE, log.p = FALSE)
# qf(0.95, 3, 6, lower.tail = TRUE, log.p = FALSE)
...
```{r echo=FALSE, warning=FALSE, out.width='45%'}
...
```
 ...
 (d) Report the complete code along with output here.
...
$$\sum_{i=1}^aQ_i=\sum_{i=1}^a[y_{i.}-\sum_{j=1}^bn_{ij}\bar y_{.j}]=\sum_{i=1}^a\sum_{j=1}^b y_{ij}-\sum_{j=1}^b(\bar y_{.j}\sum_{i=1}^an_{ij})=y_{..}-\frac1k\sum_{j=1}^b(y_{.j}\sum_{i=1}^an_{ij})=y_{..}-\frac{y_{..}}{kr}\sum_{j=1}^b\sum_{i=1}^an_{ij}$$

$$\sum_{i=1}^aQ_i=\sum_{i=1}^a[y_{i.}-\sum_{j=1}^bn_{ij}\bar y_{.j}]=\sum_{i=1}^ar\bar y_{i.}-\sum_{j=1}^b(\bar y_{.j}\sum_{i=1}^an_{ij})=r\frac{\bar y_{..}}k-\frac1k\sum_{j=1}^b(y_{.j}\sum_{i=1}^an_{ij})=\frac{ar}k\bar y_{..}-\frac{\bar y_{..}}k\sum_{j=1}^b\sum_{i=1}^an_{ij}$$

For each treatment has $r$ replications in BIBD, $\sum_{j=1}^b\sum_{i=1}^an_{ij}=ra=kb$

$$\sum_{i=1}^aQ_i=b\bar y_{..}-\frac{y_{..}}kkb=0$$
...
$$\bar y_{i..}-\bar y_{...}=\frac1{bn}\sum_{j=1}^b\sum_{k=1}^n(μ+τ_i+β_j+(τβ)_{ij}+ε_{ijk})-\frac1{abn}\sum_{i=1}^a\sum_{j=1}^b\sum_{k=1}^n(μ+τ_i+β_j+(τβ)_{ij}+ε_{ijk})$$ 

$$=μ+τ_i+\frac1{b}\sum_{j=1}^bβ_j+\frac1{b}\sum_{j=1}^b(τβ)_{ij}+\frac1{bn}\sum_{j=1}^b\sum_{k=1}^nε_{ijk}-μ-\frac1{a}\sum_{i=1}^aτ_i-\frac1{b}\sum_{j=1}^bβ_j-\frac1{ab}\sum_{i=1}^a\sum_{j=1}^b(τβ)_{ij}-\frac1{abn}\sum_{i=1}^a\sum_{j=1}^b\sum_{k=1}^nε_{ijk}$$ 

For $\sum_{i=1}^aτ_i=0$, $\sum_{j=1}^bβ_j=0$; $\sum_{i=1}^a(τβ)_{ij}=0$, and $\sum_{j=1}^b(τβ)_{ij}=0$

$$=τ_i+\frac1{bn}\sum_{j=1}^b\sum_{k=1}^nε_{ijk}-\frac1{abn}\sum_{i=1}^a\sum_{j=1}^b\sum_{k=1}^nε_{ijk}=τ_i+\bar ε_{i..}-\bar ε_{...}$$ 

$$=τ_i+\frac1{bn}\sum_{j=1}^b\sum_{k=1}^nε_{ijk}-\frac1{abn}\sum_{i=1}^a\sum_{j=1}^b\sum_{k=1}^nε_{ijk}=τ_i+\frac1{abn}\sum_{j=1}^b\sum_{k=1}^n(aε_{ijk}-\sum_{i=1}^aε_{ijk})$$ 

 ---

$$SS_A=bn\sum_{i=1}^a(\bar y_{i..}-\bar y_{...})^2=bn\sum_{i=1}^a(τ_i+\bar ε_{i..}-\bar ε_{...})^2=bn\sum_{i=1}^a(τ_i^2+\bar ε_{i..}^2+\bar ε_{...}^2+2τ_i\bar ε_{i..}-2τ_i\bar ε_{...}-2\bar ε_{i..}\bar ε_{...})$$ 

$$=bn\sum_{i=1}^a(τ_i^2+\bar ε_{i..}^2+\bar ε_{...}^2)+2bn\sum_{i=1}^aτ_i\bar ε_{i..}-2bn\sum_{i=1}^aτ_i\bar ε_{...}-2\sum_{i=1}^a\bar ε_{i..}\bar ε_{...}$$ 

 ---

$$E[SS_A]=bn\sum_{i=1}^a\left(E[τ_i^2]+E[\bar ε_{i..}^2]+E[\bar ε_{...}^2]+2Cov[τ_i,\bar ε_{i..}]-2Cov[τ_i,\bar ε_{...}]-2Cov[\bar ε_{i..},\bar ε_{...}]\right)$$ 

For $τ_i$ and $ε_{ijk}$ are independent. $Cov\left[τ_i,\bar ε_{i..}\right]=Cov[τ_i,\bar ε_{...}]=0$

For $ε_{ijk}\sim iidN(0,σ^2)$, $Cov\left[ε_{ijk},\sum_{i=1}^aε_{ijk}\right]=\sigma^2$

$$=bn\sum_{i=1}^aτ_i^2+\frac{bn}{(bn)^2}\sum_{i=1}^a\sum_{j=1}^b\sum_{k=1}^nE[ε_{ijk}^2]+\frac{bn}{(abn)^2}\sum_{i=1}^a\sum_{i=1}^a\sum_{j=1}^b\sum_{k=1}^nE[ε_{ijk}^2]+0-0-\frac{2bn}{(bn)(abn)}\sum_{i=1}^a\sum_{j=1}^b\sum_{k=1}^nCov\left[ε_{ijk},\sum_{i=1}^aε_{ijk}\right]$$ 



$$=bn\sum_{i=1}^aτ_i^2+{a}σ^2+σ^2-2σ^2=bn\sum_{i=1}^aτ_i^2+(a-1)σ^2$$ 