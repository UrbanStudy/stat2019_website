...
$\Gamma=\sum_{i=1}^ac_i\mu_i$,$C=\sum_{i=1}^ac_i\bar y_i$,$E(C)=\Gamma$;$\sum_{i=1}^ac_i=0$;orthogonal $\sum_{i=1}^ac_id_i=0$
...
\begin{tabular}{l|l|l|l|l}
     ...
     &          &$\frac{\sum^a_{i=1}\sum^n_{j=1}y_{ij}}N$ & $\frac{\sum^n_{j=1}y_{ij}}n$ & $\frac{\sum^a_{i=1}y_{ij}}a$\\
E[f.]  &  $\mu$   &  $\mu$      & $\mu+\tau_i$ &$\mu+\beta_j$\\
V[f.]  &$\sigma^2$&$\frac{\sigma^2}{na}$&$\frac{\sigma^2}{n}$&$\frac{\sigma^2}{a}$\\\hline
E[r.]  &  $\mu$   &  $\mu$      &$\mu$ & $\mu$  \\
