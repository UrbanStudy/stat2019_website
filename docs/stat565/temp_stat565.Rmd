...
|RB-RCBD| |$\mu+\tau_{i}+\beta_j+ε_{ij}-\mu-\tau_{i}-\frac{\sum_{j=1}^b\beta_j}b -\frac{\sum_{j=1}^bε_{ij}}b$|$=\beta_j-\frac{ \sum_{j=1}^b\beta_j}b+$|$ε_{ij}-\frac{\sum_{j=1}^nε_{ij}}n$
...
$$E[y_{ij}-\bar y_{i.}]=\begin{cases}E[ε_{ij}]-\frac1{n}\sum_{j=1}^nE[ε_{ij}]=0&\text{FT-CRD}\\
E[ε_{ij}]-\frac1n \sum_{j=1}^nE[ε_{ij}]=0&\text{RT-CRD}\\
E[\beta_j]+E[ε_{ij}]-\frac1{b}\sum_{j=1}^bE[ε_{ij}]=\beta_j&\text{FB-RCBD}\\
E[\beta_j]-\frac1b \sum_{j=1}^bE[\beta_j]+E[ε_{ij}]-\frac1b \sum_{j=1}^bE[ε_{ij}]=\beta_j&\text{RB-RCBD}
\end{cases}$$

$$E[\bar y_{i.}-\bar y_{..}]=\begin{cases}E[\tau_{i}]+\frac1{n}\sum_{j=1}^nE[ε_{ij}]-\frac1{an}\sum_{i=1}^a\sum_{j=1}^nE[ε_{ij}]=\tau_{i}&\text{FT-CRD}\\
E[\tau_{i}]-\frac1{a}\sum_{i=1}^aE[\tau_{i}]+\frac1n \sum_{j=1}^nE[ε_{ij}]-\frac1{an}\sum_{i=1}^a\sum_{j=1}^nE[ε_{ij}]=\tau_{i}&\text{RT-CRD}\\
E[\tau_{i}]+\frac1{b}\sum_{j=1}^bE[ε_{ij}]-\frac1{ab}\sum_{i=1}^a\sum_{j=1}^bE[ε_{ij}]=\tau_{i}&\text{FB-RCBD}\\
E[\tau_{i}]+\frac1b \sum_{j=1}^bE[ε_{ij}]-\frac1{ab}\sum_{i=1}^a\sum_{j=1}^bE[ε_{ij}]=\tau_{i}&\text{RB-RCBD}
\end{cases}$$

$$E[y_{ij}-\bar y_{.j}]=\begin{cases}
E[\tau_{i}]+E[ε_{ij}]-\frac1{a}\sum_{j=1}^aE[ε_{ij}]=\tau_{i}&\text{FB-RCBD}\\
E[\tau_{i}]+E[ε_{ij}]-\frac1a \sum_{j=1}^aE[ε_{ij}]=\tau_{i}&\text{RB-RCBD}
\end{cases}$$

$$E[\bar y_{.j}-\bar y_{..}]=\begin{cases}
E[\beta_j]+\frac1{a}\sum_{i=1}^aE[ε_{ij}]-\frac1{ab}\sum_{i=1}^a\sum_{j=1}^bE[ε_{ij}]=\beta_j&\text{FB-RCBD}\\
E[\beta_j]-\frac1{a}\sum_{i=1}^aE[\beta_j]+\frac1a \sum_{i=1}^aE[ε_{ij}]-\frac1{ab}\sum_{i=1}^a\sum_{j=1}^bE[ε_{ij}]=\beta_j&\text{RB-RCBD}
\end{cases}$$


$$E[y_{ij}-\bar y_{i.}-\bar y_{.j}+\bar y_{..}]=E[ε_{ij}]-\frac1{a}\sum_{i=1}^aE[ε_{ij}]-\frac1{b}\sum_{j=1}^bE[ε_{ij}]+\frac1{ab}\sum_{i=1}^a\sum_{j=1}^bE[ε_{ij}]=0\quad \text{FB-RCBD}$$
...
$Var[\beta_{j}+ε_{ij}-\frac{\sum_{j=1}^nε_{ij}}n]=$,
$Var[\beta_{j}]+Var[ε_{ij}]+\frac{\sum_{j=1}^nVar[ε_{ij}]}{n^2}-\frac{2Cov[\beta_{j},ε_{ij}]}n-\frac{2Cov[\beta_{j},\sum_{j=1}^nε_{ij}]}n-\frac{2Cov[ε_{ij},\sum_{j=1}^nε_{ij}]}n$,
$=\sigma_{\beta}^2+\sigma^2+\frac{\sigma^2}n-\frac{2\sigma^2}n=\sigma_{\beta}^2+\frac{(n-1)\sigma^2}n$

RB-RCBD

$Var[\beta_{j}-\frac{\sum_{j=1}^b\beta_{j}}b+ε_{ij}-\frac{\sum_{j=1}^nε_{ij}}n]=$,
$Var[\beta_{j}]+\frac{\sum_{j=1}^bVar[\beta_{j}]}{b^2}+Var[ε_{ij}]+\frac{\sum_{j=1}^nVar[ε_{ij}]}{n^2}-\frac{2Cov[\beta_{j},\sum_{j=1}^b\beta_{j}]}b+\frac{2Cov[\beta_{j},ε_{ij}]}n-\frac{2Cov[\beta_{j},\sum_{j=1}^nε_{ij}]}n-\frac{2Cov[\sum_{j=1}^b\beta_{j},ε_{ij}]}b+\frac{2Cov[\sum_{j=1}^b\beta_{j},\sum_{j=1}^bε_{ij}]}{b^2}-\frac{2Cov[ε_{ij},\sum_{j=1}^nε_{ij}]}n$,
$=\sigma_{\beta}^2+\frac{\sigma_{\beta}^2}{b}-\frac{2\sigma_{\beta}^2}{b}+\sigma^2+\frac{\sigma^2}n-\frac{2\sigma^2}n=\frac{(\sigma_{\beta}^2+\sigma^2)(n-1)}n$
...
$=\sigma_{\tau}^2+\frac{\sigma_{\tau}^2}{n}+\frac{\sigma^2}{an}-\frac{2\sigma^2}{an}=\frac{(n+1)\sigma_{\tau}^2}{n}-\frac{\sigma^2}{an}$

RT-CRD

$Var[\tau_{i}-\frac{\sum_{i=1}^a\tau_{i}}{a}+\frac{\sum_{j=1}^nε_{ij}}n-\frac{\sum_{i=1}^a\sum_{j=1}^nε_{ij}}{an}]=$,
$Var[\tau_{i}]+\frac{\sum_{i=1}^aVar[\tau_{i}]}{a^2}+\frac{\sum_{j=1}^nVar[ε_{ij}]}{n^2}+\frac{\sum_{i=1}^a\sum_{j=1}^nVar[ε_{ij}]}{a^2n^2}-\frac{2Cov[\tau_{i},\sum_{i=1}^a\tau_{i}]}a+\frac{2Cov[\tau_{i},\sum_{j=1}^nε_{ij}]}n-\frac{2Cov[\tau_{i},\sum_{i=1}^a\sum_{j=1}^nε_{ij}]}{an}-\frac{2Cov[\sum_{i=1}^a\tau_{i},\sum_{j=1}^nε_{ij}]}{an}+\frac{2Cov[\sum_{i=1}^a\tau_{i},\sum_{i=1}^a\sum_{j=1}^nε_{ij}]}{a^2n}-\frac{2Cov[\sum_{j=1}^nε_{ij},\sum_{i=1}^a\sum_{j=1}^nε_{ij}]}{an^2}$,
$=\sigma_{\tau}^2+\frac{\sigma_{\tau}^2}{a}-\frac{2\sigma_{\tau}^2}{a}+\frac{\sigma^2}{n}+\frac{\sigma^2}{an}-\frac{2\sigma^2}{an}=\frac{(n\sigma_{\tau}^2+\sigma^2)(n-1)}{an}$
...
$=\sigma_{\tau}^2+\frac{\sigma_{\tau}^2}{b}+\frac{\sigma^2}{ab}-\frac{2\sigma^2}{ab}=\frac{(b+1)\sigma_{\tau}^2}{b}-\frac{\sigma^2}{ab}$
...
$=\sigma_{\tau}^2+\sigma^2+\frac{\sigma^2}a-\frac{2\sigma^2}a=\sigma_{\tau}^2+\frac{(a-1)\sigma^2}a$
...
$=\sigma_{\beta}^2+\frac{\sigma_{\beta}^2}{a}+\frac{\sigma^2}{ab}-\frac{2\sigma^2}{ab}=\frac{(a+1)\sigma_{\beta}^2}{a}-\frac{\sigma^2}{ab}$

RB-RCBD

$Var[\beta_{j}-\frac{\sum_{j=1}^b\beta_{j}}{b}+\frac{\sum_{i=1}^aε_{ij}}a-\frac{\sum_{i=1}^a\sum_{j=1}^bε_{ij}}{ab}]=$,
$Var[\beta_{j}]+\frac{\sum_{j=1}^bVar[\beta_{j}]}{b^2}+\frac{\sum_{i=1}^aVar[ε_{ij}]}{a^2}+\frac{\sum_{i=1}^a\sum_{j=1}^bVar[ε_{ij}]}{a^2b^2}-\frac{2Cov[\beta_{j},\sum_{j=1}^b\beta_{j}]}b+\frac{2Cov[\beta_{j},\sum_{i=1}^aε_{ij}]}a-\frac{2Cov[\beta_{j},\sum_{i=1}^a\sum_{j=1}^bε_{ij}]}{ab}-\frac{2Cov[\sum_{j=1}^b\beta_{j},\sum_{i=1}^aε_{ij}]}{ab}+\frac{2Cov[\sum_{j=1}^b\beta_{j},\sum_{i=1}^a\sum_{j=1}^bε_{ij}]}{ab^2}-\frac{2Cov[\sum_{i=1}^aε_{ij},\sum_{i=1}^a\sum_{j=1}^bε_{ij}]}{a^2b}$,
...
$Var[ε_{ij}]+\frac{\sum_{i=1}^aVar[ε_{ij}]}{a^2}+\frac{\sum_{j=1}^bVar[ε_{ij}]}{b^2}+\frac{\sum_{i=1}^a\sum_{j=1}^bVar[ε_{ij}]}{a^2b^2}-\frac{2Cov[ε_{ij},\sum_{i=1}^aε_{ij}]}{a}-\frac{2Cov[ε_{ij},\sum_{j=1}^bε_{ij}]}{b}+\frac{2Cov[ε_{ij},\sum_{i=1}^a\sum_{j=1}^bε_{ij}]}{ab}+\frac{2Cov[\sum_{i=1}^aε_{ij},\sum_{j=1}^bε_{ij}]}{ab}-\frac{2Cov[\sum_{i=1}^aε_{ij},\sum_{i=1}^a\sum_{j=1}^bε_{ij}]}{a^2b}-\frac{2Cov[\sum_{j=1}^bε_{ij},\sum_{i=1}^a\sum_{j=1}^bε_{ij}]}{ab^2}$
$=\sigma^2+\frac{\sigma^2}{a}+\frac{\sigma^2}{b}+\frac{\sigma^2}{ab}-\frac{2\sigma^2}{a}-\frac{2\sigma^2}{b}+\frac{2\sigma^2}{ab}-\frac{2\sigma^2}{ab}-\frac{2\sigma^2}{ab}-\frac{2\sigma^2}{ab}=\frac{(ab-a-b-3)\sigma^2}{ab}$
...
$$Var[y_{ij}-\bar y_{i.}]=\begin{cases}\frac{n-1}n\sigma^2&\text{FT-CRD}\\
\frac{n-1}n(σ_{\tau}^2+\sigma^2)&\text{RT-CRD}\\
\frac{b-1}b\sigma^2&\text{FB-RCBD}\\
\frac{b-1}b(σ_{\beta}^2+\sigma^2)&\text{RB-RCBD}
\end{cases}$$
