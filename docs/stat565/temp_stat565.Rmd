...
For $ε_{ij}\sim iid\ N(0,σ^2)$, $E[\varepsilon_{ij}]=0$, then
...
For $ε_{ij}\sim iid\ N(0,σ^2)$, $Var[\varepsilon_{ij}]=\sigma^2$, then

$$Var(y_{ij}-\bar y_{i.})=Var[\varepsilon_{ij}-\frac1n \sum_{j=1}^n\varepsilon_{ij}]=Var[\varepsilon_{ij}]+\frac1{n^2}\sum_{j=1}^nVar[\varepsilon_{ij}]-2Cov(\varepsilon_{ij},\sum_{j=1}^n\varepsilon_{ij})$$

$$=\sigma^2+\frac{\sigma^2}n+0=\frac{n+1}n\sigma^2$$
...
$$E[(y_{ij}-\bar y_{i.})^2]=Var[y_{ij}-\bar y_{i.}]+E[y_{ij}-\bar y_{i.}]^2=\frac{n+1}n\sigma^2$$
  ...
  (c) Let $a=3$ treatments in the above model and the following linear functions of $\mu_i=\mu+\tao_i$ are tested. $\mu_1=\mu_2$, $\frac{\mu_1+\mu_2}{2}=\mu_3$.

   (i) Prove that each of these is a contrast and they are orthogonal contrasts.

For $a=3,\mu_1=\mu_2$, $c_1=1,c_2=-1,c_3=0$

$$\sum_{i=1}^3c_i=1-1+0=0$$

For $\frac{\mu_1+\mu_2}2=\mu_3$, $d_1=1,d_2=1,d_3=-2$

$$\sum_{i=1}^3d_i=1+1-2=0$$
...
model_battery <- lm(Life~Brand, data=table_battery)
...
model_circuit <- lm(Respone~Type, data=table_circuit)
anova(model_circuit)
...
plot(TukeyHSD(aov(model_circuit), conf.level=0.99), las=1 , col="brown" )
