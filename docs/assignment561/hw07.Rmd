...
$$f(x)={h(x)}{c(\sigma^2)}e^{w_1(\sigma^2)t_1(x)}$$
...
When $\sigma$ is known, 
$$f(x|\mu)=\frac{1}{\sqrt{2\pi\sigma^2}}e^{-\frac1{2\sigma^2}(x-\mu)^2}=\frac{1}{\sqrt{2\pi\sigma^2}}e^{-\frac{x^2}{2\sigma^2}+\frac{x\mu}{\sigma^2}-\frac{\mu^2}{2\sigma^2}}=\frac{e^{-\frac{x^2}{2\sigma^2}}}{\sqrt{2\pi\sigma^2}}e^{\frac{x}{\sigma^2}\mu}e^{-\frac{\mu^2}{2\sigma^2}}$$
...
When $\alpha$ is known, 
$$f(x|\beta)=\frac{x^{\alpha-1}}{\Gamma(\alpha)\beta^{\alpha}}e^{-\frac{x}{\beta}}={h(x)}{c(\beta)}e^{w_1(\beta)t_1(x)}$$
...
When $\beta$ is known, 
$$f(x|\alpha)=e^{-\frac{x}{\beta}}\frac{1}{\Gamma(\alpha)\beta^{\alpha}}e^{(\alpha-1)\ln{x}}={h(x)}{c(\alpha)}e^{w_1(\alpha)t_1(x)}$$

$h(x)$ | $c(\alpha)$ | $w_1(\alpha)$ | $t_1(x)$
------ | -------- | --------- | ------
 $e^{-\frac{x}{\beta}}$ | $\frac{1}{\Gamma(\alpha)\beta^{\alpha}}$ | $\alpha-1$ | $\ln x$

When $\alpha$ and $\beta$ are unknown, 
$$f(x|\alpha,\beta)=\frac{1}{\Gamma(\alpha)\beta^{\alpha}}e^{(\alpha-1)\ln{x}-\frac{x}{\beta}}={h(x)}{c(\beta)}e^{w_1(\beta)t_1(x)+w_2(\alpha,\beta)t_2(x)}$$
...
When $\alpha$ is known, 
$$f(x|\beta)=\frac{x^{\alpha-1}}{B(\alpha,\beta)}(1-x)^{\beta-1}=\frac{x^{\alpha-1}}{B(\alpha,\beta)}e^{(\beta-1)\ln(1-x)}={h(x)}{c(\beta)}e^{w_1(\beta)t_1(x)}$$
...
When $\beta$ is known, 
$$f(x|\alpha)=\frac{(1-x)^{\beta-1}}{\Gamma(\alpha)\beta^{\alpha}}e^{(\alpha-1)\ln{x}}={h(x)}{c(\alpha)}e^{w_1(\alpha)t_1(x)}$$
...
When $\alpha$ and $\beta$ are unknown, 
$$f(x|\alpha,\beta)=\frac{1}{B(\alpha,\beta)}x^{\alpha-1}(1-x)^{\beta-1}=\frac{1}{B(\alpha,\beta)}e^{(\alpha-1)\ln x}e^{(\beta-1)\ln(1-x)}={h(x)}{c(\beta)}e^{w_1(\alpha)t_1(x)+w_2(\beta)t_2(x)}$$
...
When $\lambda$ is known, 

$$f(x|\lambda)=\frac{\lambda^x}{x!}e^{-\lambda}=\frac{1}{x!}e^{-\lambda}e^{\ln(\lambda)x}={h(x)}{c(p)}e^{w_1(p)t_1(x)}$$
...
$$f(x|p)=\binom{x-1}{r-1}p^r(1-p)^{x-r}=\binom{x-1}{r-1}(\frac{p}{1-p})^r(1-p)^{x}=\binom{x-1}{r-1}(\frac{p}{1-p})^re^{(1-p)\ln x}={h(x)}{c(p)}e^{w_1(p)t_1(x)}$$
...
Therefore, this function is an exponential family. The $\theta$ parameter vector lies on a line.
