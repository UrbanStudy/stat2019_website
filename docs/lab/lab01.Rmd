...
$Z_i$ is called a standard normal random variable.  
 $Z_{i}\sim iid\ N\left(0,1\right)$   

Let $Y_{1}=\sum_{i=1}^{3}(\frac{X_{i}-\mu}{\sigma})$  
(a). Comptute the $E(Y_{1})$  
$$\because X_i\ are\ independent\ variables$$
$$So\quad E(Y_1)=E(\sum_{i=1}^3Z_i)=\sum_{i=1}^3E(Z_i)=0$$
(a). Comptute the $Var(Y_{1})$  

$$And\quad Var(Y_{1})=Var(\sum_{i=1}^3Z_i)=\sum_{i=1}^3Var(Z_i)=3$$
...
> According the Center Limit Theorem,
