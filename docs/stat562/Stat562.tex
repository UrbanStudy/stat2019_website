...
$f_X(x)=\sum_{y\in\mathbf{R}}f_{X,Y}(x,y)=\int_{-\infty}^{\infty}f(x,y)dy$ 4.1.6/10 $f(x|y)=P(X=x|Y=y)=\frac{f(x,y)}{f_Y(y)}$ 4.2.1 \\
...
$E[g(X)|y]=\sum_xg(x)f(x|y)=\int_{-\infty}^{\infty}g(x)f(x|y)dx$ r.v.(y) 4.2.3 $E[g(X)h(Y)|y]=h(Y)E[g(X)|y]$\\
...
$V[X|Y]=E[(X-E[X|Y])^2|Y]=E[X^2|y]-(E[X|y])^2$ 4.2.4 | $\sigma_{ax+b} =|a|\sigma_{x}$
