...
$E(aX+b)=aE(X)+b$ | $E(X)=\int_{-\infty}^{0}F_X(t)dt+\int_{0}^{\infty}F_X(t)dt$

$E[g(x)]=\mu=\sum_{x\in D}h(x)p(x)=\int_{-\infty}^{\infty}g(x)f(x)dx$ | $E[(X-\mu)^n]=\mu_n=\sum{(x-\mu)^n}p(x)=\int{(x-\mu)^n}f(x)dx$

\textbf{Variances}

$V[X]=\sigma_{x}^2=E(X^2)-[E(X)]^2$ | $V[X|Y]=E[(X-E[X|Y])^2|Y]$ | $V[aX+b]=a^2\sigma^2$ $\sigma_{ax+b} =|a|\sigma_{x}$
...
5.2.2 $\bar X=\frac{X_1+..+X_n}{n}=\frac1n\sum_{i=1}^nX_i$ | 5.2.3 $S^2=\frac1{n-1}\sum_{i=1}^n(X_i-\bar X)^2=\frac1{n-1}\sum_{i=1}^n(X_i^2-n\bar X^2)$

5.2.5 $E\left(\sum_{i=1}^ng(X_i) \right)=nE(g(X_1))$ $Var\left(\sum_{i=1}^ng(X_i) \right)=nVar(g(X_1))$ | 5.2.6 $E\bar X=\mu$, $Var\bar X=\frac{\sigma^2}n$, $ES^2=\sigma^2$

5.2.7 $M_{\bar X}(t)=[M_X(\frac{t}n)]^n$ | 5.2.9 Convolution formula $Z=X+Y$ $f_Z(z) =\int_{-\infty}^{\infty}f_X(w)f_Y(z-w)dw$
...
\begin{tabular}{l|l|l|l}
W  Normal& $\bar X\sim N(\mu,\frac{\sigma^2}n)$ & $\bar X,S^2$ indep & $\frac{(n-1)S^2}{\sigma^2}\sim\chi^2_{n-1}$\\\hline
W/o Normal & $E[\bar X]=\mu$ & $V[\bar X]=\frac{\sigma^2}n)$ & $E[S^2]=\sigma^2$\\
\end{tabular}

$f_{\chi^2}(x)=\frac{x^{\frac{p}2-1}}{\Gamma \frac{p}2 2^{\frac{p}2}}e^{-\frac{x}2}, x>0$ | $\chi^2_{p}\sim gamma(\frac{p}2,2)$ see 4.6.8

5.3.2a | $Z\sim n(0,1)$ | $Z^2\sim \chi^2_1$ \\
5.3.2b | $X_1,..,X_n\sim \chi^2_{p_i}$ | $X_1+..+X_n\sim \chi^2_{p_1+..p_n}$\\

5.3.1 proof $\bar X_{n+1}=\frac{\sum_{i=1}^{n+1}X_{i}}{n+1}=\frac{\sum_{i=1}^{n}X_{i}+X_{n+1}}{n+1}=\frac{n\bar X_{n}+X_{n+1}}{n+1}$ $nS^2_{n+1}=(n-1)S_n^2+(\frac{n}{n+1})(X_{n+1}-\bar X_n)^2$
...
$f_{T}(x)=\frac{\Gamma(\frac{p+1}2)}{\Gamma(\frac{p}2)\sqrt{p\pi}}\left (1+\frac{x^2}{p}\right)^{-\frac{p+1}2},-\infty<x<\infty$ | $t_1=Cauchy(0,1)$

5.3.5  $X_1..X_n$,$Y_1..Y_m$ indep. $X_i\sim n(\mu_X,\sigma_X^2)$，$Y_j\sim n(\mu_Y,\sigma_Y^2)$,$\frac{S_X^2}{\sigma_X^2}\sim \chi^2$ | $\frac{S_X^2/\sigma_X^2}{S_Y^2/\sigma_Y^2} \sim F$

5.3.6 $f_{F}(x)=\frac{\Gamma(\frac{p+q}2)}{\Gamma(\frac{p}2)\Gamma (\frac{q}2)}(\frac{p}q)^{\frac{p}2}\frac{x^{\frac{p}2-1}}{(1+\frac{p}qx)^{\frac{p+q}2}}, x>0$

a. $X\sim F_(p,q)$ $\frac{1}X \sim F_(q,p)$ | b. $X\sim T_(q)$ $X^2 \sim F_(1,q)$ | c. $X\sim F_(p,q)$ $\frac{\frac{p}{q}X}{1+\frac{p}{q}X} \sim Beta(\frac{p}2,\frac{q}2)$
