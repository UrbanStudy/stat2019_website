...
$E[X]=\int_{-\infty}^{\infty}xf(x)dx=E[E(X|Y)]$ 4.4.3 $E[g(x)]=\sum_{x\in D}h(x)p(x)=\int_{-\infty}^{\infty}g(x)f(x)dx$\\
...
$E[g(X)|y]=\sum_xg(x)f(x|y)=\int_{-\infty}^{\infty}g(x)f(x|y)dx$ r.v.(y) 4.2.3 
