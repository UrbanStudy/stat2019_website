...
$$\because \int_0^1\int_0^2f(x,y)dxdy=\int_0^1\int_0^2f(x,y)dxdy=\int_0^1\int_0^2C(x+2y)dxdy=\int_0^1\left[C(\frac{x^2}2+2yx)\right]_0^2dy=\int_0^1C(2+4y)dy=\left.C(2y+2y^2)\right|_0^1=4C=1$$
...
$$F_{XY}(x,y)=P(X\le x,Y\le y)=\begin{cases}0\qquad x\le0,y\le0\\\\\int_0^x\int_0^yf(u,v)dvdu\\\\1\qquad x\ge2,y\ge1 \end{cases}=\begin{cases}\int_0^x\int_0^y\frac14(u+2v)dvdu=\frac14\int_0^x[uv+v^2]_{v=0}^{y}du=\frac14\int_0^x(yu+y^2)du=\frac14[\frac{yu^2}2+y^2u]_{u=0}^x=\frac{x^2y}8+\frac{xy^2}4&0<y<1,0<x<2\\\int_0^2\int_0^y\frac14(u+2v)dvdu=\frac14\int_0^2[uv+v^2]_{v=0}^{y}du=\frac14\int_0^2(yu+y^2)du=\frac14[\frac{yu^2}2+y^2u]_{u=0}^2=\frac{y^2}2+\frac{y}2&0<y<1,2\le x\\\int_0^x\int_0^1\frac14(u+2v)dvdu=\frac14\int_0^x[uv+v^2]_{v=0}^{1}du=\frac14\int_0^x(u+1)du=\frac14[\frac{u^2}2+u]_{u=0}^x=\frac{x^2}8+\frac{x}{4}&1\le y,0<x<2 \end{cases}$$
...
$$f_Z(z)=\begin{cases}f_X(g^{-1}(z))|\frac{d}{dz}g^{-1}(z)|=(3z^{-\frac12}-1)|-\frac32z^{-\frac32}|=\frac92z^{-2}-\frac32z^{-\frac32}\quad1<z<9 \\0\qquad elsewhere\end{cases}$$
...
The support of the distribution of (U, V ) is {(u, v) : u = 1, 2, . . . ; v = u + 1, u + 2, . . .}. This is not a cross-product set. Therefore, U and V are not independent. More simply, if we know U = u, then we know V > u.
...
$$f_{U,V}(u,v)=P(U=u,V=v)=\left\{\begin{array}{l} P(X=u+v,Y=u)=p(1-p)^{u+v-1}p(1-p)^{u-1}=p^2(1-p)^{2u+v-2} &\text{ when } X>Y\\P(X=u,Y=u)=p^2(1-p)^{2(u-1)} &\text{ when } X=Y \\P(X=u,Y=u-v)=p(1-p)^{u-1}p(1-p)^{u-v-1}=p^2(1-p)^{2u-v-2} &\text{ when } X<Y\end{array}\right\}=(\frac{p}{1-p})^{2}(1-p)^{2u}(1-p)^{|v|},u=1,2..,v=0,\pm1,\pm2,..$$
...
$$P(Y=X+1)=P(X=Y-1)=\int_{i}^{i+1}e^{1-y}dy=\left.-e^{1-y}\right|_{y=i}^{i+1}=-e^{-i}+e^{1-i}=e^{-i}(e-1)$$

(b) Find the conditional distribution of $X-4$ given $Y\ge5$.

For $f_X(x)=e^{-x}$, $F_X(x)=P(X\le x)=1-e^{-x}$ and $1-F_X(x)=P(X>x)=e^{-x}$

$$P(X-4\le x|Y\ge5)=P(X-4\le x|X+1\ge5)=\frac{P(X\le x+4)\cap P(X\ge4)}{P(X\ge4)}=\frac{P(X\le x)\cap P(X\ge4)}{P(X\ge4)}=e^{-x},\quad x\ge4$$
...
$$\left.\begin{array}\mathcal{A_0}=\{-\infty<x_1<\infty,x_2=0\}\\ \mathcal{A_1}=\{-\infty<x_1<\infty,x_2>0\} \\ \mathcal{A_2}=\{-\infty<x_1<\infty,x_2<0\} \\ \end{array}\right\}\to\mathcal{B_1}=\{0<y_1<\infty,|y_2|<1\}$$
...
$$J_{1,2}=\begin{vmatrix}\frac{\partial h_1}{\partial y_1} & \frac{\partial h_2}{\partial y_2} \\ \frac{\partial h_1}{\partial y_1} & \frac{\partial h_2}{\partial y_2} \end{vmatrix}=\frac{\partial h_1}{\partial y_1}\frac{\partial h_2}{\partial y_2}-\frac{\partial h_2}{\partial y_1}\frac{\partial h_1}{\partial y_2}=\pm\frac12y_2y_1^{-\frac12}\cdot y_1^{\frac12}[\frac12(1-y_2^2)^{-\frac12}(-2y_2)]\mp \frac12y_1^{-\frac12}(1-y_2^2)^{\frac12}y_1^{\frac12}=\mp\frac12(1-y_2^2)^{-\frac12}$$

$$\therefore f_{Y_1,Y_2}(y_1,y_2)=f_{X_1,X_2}(h_1(y_1,y_2))\cdot f_{X_1,X_2}(h_2(y_1,y_2))(|J_1|+|J_1|)=\frac1{2\pi\sigma^2}e^{-\frac{y_2^2y_1+(1-y_2^2)y_1}{2\sigma^2}}(1-y_2^2)^{-\frac12}=\frac1{2\pi\sigma^2}e^{-\frac{y_1}{2\sigma^2}}(1-y_2^2)^{-\frac12},\quad 0<y_1<\infty,|y_2|<1$$



 (b). Show that $Y_1$ and $Y_2$ are independent, and interpret this result geometrically.

The joint pdf, $f_{Y_1,Y_2}(y_1,y_2)$ factors into a function of $y_1$ and $y_2$. So $Y_1$ and $Y_2$ are independent. $Y_1$ is the square of the distance from $(X_1,X_2)$ to
the origin. $Y_2$ is the cosine of the angle between the positive $x_1$-axis and the line from $(X_1,X_2)$ to the origin. So independence says the distance from the origin is independent of the orientation (as measured by the angle).
