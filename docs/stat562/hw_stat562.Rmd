...
Now let $W=\frac{Z}{Y},Q=Y$. Then $Y=Q,Z=WQ$, and $|J| = q$. Therefore

$$f_{W,QQ}(w,q)=\frac{n(n-1)}{\theta^n}(q-wq)^{n-2}q=\frac{n(n-1)}{\theta^n}(1-w)^{n-2}q^{n-1},0<w<1,0<q<\theta$$

The joint pdf factors into functions of $w$ and $q$, and, hence, $W$ and $Q$ are independent.

 ---

**5.25** As a generalization of the previous exercise, let $X1,..,X_n$ be iid with pdf $f_X(x)=\begin{cases}\frac{a}{\theta^a}x^{a-1}&0<x<1\\0&\text{otherwise}\end{cases}$. Let $X_{(1)}<..<X_{(n)}$ be the order statistics. Show that $X_{(1)}/X_{(2)}, X_{(2)}/X_{(3)},..,$X_{(n-1)}/X_{(n)}$ $ and $X_{(n)}$ are mutually independent random variables. Find the distribution of each of them.
...
The Jacobian is $J = y_2y_3^2\cdots y_n^{n-1}$. So the joint pdf of $Y_1,..,Y_n$ is

$$f(y_1,..,y_n)=\frac{n!a^n}{\theta^{an}}(y_1\cdots y_n)^{a-1}(y_2\cdots y_n)^{a-1}\cdots y_n^{a-1}(y_2y_3^2\cdots y_n^{n-1})$$

$$=\frac{n!a^n}{\theta^{an}}y_1^{a-1}y_2^{2a-1}\cdots y_n^{na-1}, 0<u_i<1,i=1,..,n-1,0<y_n<\theta$$

We see that $f(y_1,..,y_n)$ factors so $Y_1,..,Y_n$ are mutually independent. To get the pdf of $Y_1$, integrate out the other variables and obtain that $f_{Y_1}(y=a$. Thus $f_{Y_1}(y_1)=ay^{a−1}_1,0<y_1<1$. 
Similarly, for $i=2,..,n−1$, we obtain $f_{Y_i}(y_i)=iay_i^{ia−1},0<y_i<1$. 

From Theorem 5.4.4, $f_{X_{(j)}}(x)=\frac{n!}{(j-1)!(n-j)!}f_X(x)[F_X(x)]^{j-1}[1-F_X(x)]^{n-j}$

Therefore, the pdf of $Y_n$ is $f_{Y_n}(y_n)=\frac{na}{\theta^{an}}y_n^{na-1}, 0<y_n<\theta$. It can be checked that the product of these marginal pdfs is the joint pdf given above.
