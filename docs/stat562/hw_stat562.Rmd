...
7.12 Xi   iid Bernoulli( ), 0       1/2.
a. method of moments:
EX =   =1nXiXi = ¯X ) ˜  = ¯X .
MLE: In Example 7.2.7, we showed that L( |x) is increasing for     ¯x and is decreasing
for     ¯x. Remember that 0       1/2 in this exercise. Therefore, when ¯X   1/2, ¯X is
the MLE of  , because ¯X is the overall maximum of L( |x). When ¯X > 1/2, L( |x) is an
increasing function of   on [0, 1/2] and obtains its maximum at the upper bound of   which
is 1/2. So the MLE is ˆ  = min
¯X , 1/2
.
b. The MSE of ˜  is MSE(˜ ) = Var ˜  + bias(˜ )2 = ( (1 −  )/n) + 02 =  (1 −  )/n. There is no
simple formula for MSE(ˆ ), but an expression is
MSE(ˆ ) = E(ˆ  −  )2 =
Xny=0
(ˆ  −  )2ny y(1 −  )n−y=[Xn/2]y=0  yn−  2n
y y(1 −  )n−y +Xn
y=[n/2]+112 −   2  ny
 
 y(1 −  )n−y,
where Y =P
i Xi   binomial(n,  ) and [n/2] = n/2, if n is even, and [n/2] = (n − 1)/2, if n is odd.
...
Therefore MSE(ˆ ) < MSE(˜ ) for every   in 0 <     1/2. (Note: MSE(ˆ ) = MSE(˜ ) = 0 at
  = 0.)