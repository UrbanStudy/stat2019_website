...
From Theorem 5.4.4, $f_{X_{(j)}}(x)=\frac{n!}{(j-1)!(n-j)!}f_X(x)[F_X(x)]^{j-1}[1-F_X(x)]^{n-j}$, $F_X(x)=1-e^{-(x-\mu)}$. Then,
...
$$\frac{f(\mathbf {x}|\mu)}{f(\mathbf {y}|\mu)}=\frac{\prod\limits_{i=1}^n(ne^{n\mu}e^{-nx_i})}{\prod\limits_{i=1}^n(ne^{n\mu}e^{-ny_i})}=\frac{\mathrm{e}^{\sum_{i=1}^ny_i}I_{(\mu,\infty)}(\min x_i)}{\mathrm{e}^{\sum_{i=1}^nx_i}I_{(\mu,\infty)}(\min y_i)}$$
...
$f(\mathbf{x}|\theta)$ is a decreasing function of $X$
