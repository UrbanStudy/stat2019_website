...
For $S^2=\frac{\sum(X_i-\bar X)^2}{n-1}$ Let $M=\frac{\sum(X_i-\bar X)^2}{k}=\frac{(n-1)}{k}S^2=\frac{\sigma^2}{k}\frac{(n-1)S^2}{\sigma^2}$, $\frac{k}{\sigma^2}M=\frac{(n-1)S^2}{\sigma^2}$.

For $X_1,..X_i\sim iid\ n(\mu,\sigma^2)$, $\frac{(n-1)S^2}{\sigma^2}\sim\chi^2_{n-1}$, 
...
When $k=n+1$, $\min{MSE[M]}=\frac2{n+1}$
