...
From Example 5.6.7 (Beta random variable generation-I), to generate $Y\sim beta(a, b)$. If both a and b are integers, use the direct transformation method (5.6.5). Set $U_j$ are iid $uniform(O,1)$ random variables, then

$$Y=\frac{\sum_{j=1}^a\ln U_j}{\sum_{j=1}^{a+b}\ln U_j}\sim Beta(a,b)$$

If both a and b are not integers, To generate $Y\sim beta(a, b)$: a. Generate $(U,V)$ independent $uniform(0,1)$. b. If $U<\frac1cf_Y(V)$, set $Y=V$; otherwise, return to step (a). Which means $F_Y(y)=F(V|U)$
...
$f_X(x)=\frac1{b-a},\int_a^bf_X(x)dx=1$, $f_Y(y)=\frac1{c},\int_0^cf_Y(y)dy=1$. Set $w=b$, then we have $P(Y<f(X))=f_X(x)f_Y(y)=\frac1{c(b-a)}$.

$F_W(w)=F(X|Y)$.

$$P(W\le w)= P(a\le X\le w|Y<f(X))=\frac{P(a\le X\le w,Y<f(X))}{P(Y<f(X))}=\frac{\int^w_a\int^{f(x)}_0 dydx}{c(b-a)}=\frac{\int^w_af(x)dx}{c(b-a)}=\frac1{c(b-a)}\int_a^w f(x)dx$$
...
As long as $c\ge\max\limits_{a\le x\le b}f(x)$, $F_W(w)=\frac1{c(b-a)}\int_a^w f(t)dt$. By the definition 1.6.3 (pdf) , this algorithm generates a random variable with pdf $f(x)$ from the uniform random variables.

So we can calculate the beta probabilities from the uniform probabilities, which suggests that we can generate the beta random variable 

```{r echo=FALSE, out.width='40%',collapse=T}
...
plot(c(0,1), c(0,3), type = "n",main="beta(2,5)",xlab ="x", ylab ="y")
...
abline(v=(2-1)/(2+5-2),lty=2);text(0.2,0, "1/b", col = 2, adj = c(-.1,0));abline(v=1/2,lty=2);text(0.5,0, "1/a", col = 2, adj = c(-.1,0))
...
plot(c(0,10), c(0,0.5), type = "n",main="Geom(1/2.5)",xlab ="n", ylab ="p")
points(c(0:20),dgeom(c(0:20), 1/2.5, log = FALSE))
lines(c(0:20), dgeom(c(0:20), 1/2.5, log = FALSE), col = 2)
```
...
$$-\frac{(\bar x-\theta)^2}{2\frac{\sigma^2}n}-\frac{(\theta-\mu)^2}{2\tau^2}=
-\frac{(\tau^2+\frac{\sigma^2}n)\theta^2-2(\tau^2\bar x+\frac{\sigma^2}n\mu)\theta+\tau^2\bar x^2+\frac{\sigma^2}n\mu^2}
{2\tau^2\frac{\sigma^2}n}$$

$$=-\frac{(\tau^2+\frac{\sigma^2}n)^2\theta^2-2(\tau^2\bar x+\frac{\sigma^2}n\mu)(\tau^2+\frac{\sigma^2}n)\theta+\tau^2(\tau^2+\frac{\sigma^2}n)\bar x^2+\frac{\sigma^2}n(\tau^2+\frac{\sigma^2}n)\mu^2}
{2\tau^2\frac{\sigma^2}n(\tau^2+\frac{\sigma^2}n)}$$

$$=-\frac{(\tau^2+\frac{\sigma^2}n)^2\theta^2-2(\tau^2\bar x+\frac{\sigma^2}n\mu)(\tau^2+\frac{\sigma^2}n)\theta
+(\tau^4\bar x^2+\frac{\sigma^4}{n^2}\mu^2+2\tau^2\frac{\sigma^2}n\bar x\mu)(-2\tau^2\frac{\sigma^2}n\bar x\mu+\tau^2\frac{\sigma^2}n\bar x^2+\tau^2\frac{\sigma^2}n\mu^2)}
{2\tau^2\frac{\sigma^2}n(\tau^2+\frac{\sigma^2}n)}$$
...
From Theorem 7.2.10 (Invariance property of MLEs), If $\hat\theta$ is the $MLE$ of $\theta$, then for any function $\tau(\theta)$, the $MLE$ of $\tau(\theta)$ is $\tau(\hat\theta)$.

$$\pi(\theta|\bar x,\sigma^2,\mu,\tau^2)\sim N\left(\frac{\tau^2\bar x+\frac{\sigma^2}n\mu}{\tau^2+\frac{\sigma^2}n},\frac{2\tau^2\frac{\sigma^2}n}{\tau^2+\frac{\sigma^2}n}\right)$$
...
By the pdf of $\chi^2_{n-1}$, $\frac{1}{2^{\frac{p}2}\Gamma(\frac{p}2)}x^{\frac{p}2-1}e^{-\frac{x}2}$
