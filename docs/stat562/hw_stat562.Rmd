...
polygon(c(0,seq(0,0.5,0.05),0.5) ,c(0,dbeta(seq(0,0.5,0.05),2,5, ncp = 0, log = FALSE),0) ,density=10, angle=45,lty =3)
abline(h=dbeta((2-1)/(2+5-2),2,5, ncp = 0, log = FALSE),lty=3);text(0,2.5, "c", col = 2, adj = c(0,0))
abline(v=1/2,lty=2);text(0.5,0, "y", col = 2, adj = c(-.1,0))
```
...
Let $N=$ number of $(X,Y)$ pairs required for one $W$, then $N$ is a $Geom(\frac1{c(b-a)}$ random variable. Thus to generate one $W$, we expect to need $E(N)=c(b-a)$ pairs of $(X,Y)$ and in this sense minimizing $c(b-a)$ will optimize the algorithm.
