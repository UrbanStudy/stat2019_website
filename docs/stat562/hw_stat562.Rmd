...
For $X_1,..X_i\sim iid\ n(\mu,\sigma^2)$, then $\frac{(n-1)S^2}{\sigma^2}\sim\chi^2_{n-1}$ and $S^2=\frac{\sum(X_i-\bar X)^2}{n-1}$ 

Let $M=\frac{\sum(X_i-\bar X)^2}{k}=\frac{(n-1)}{k}S^2=\frac{\sigma^2}{k}\frac{(n-1)S^2}{\sigma^2}$, then $\frac{k}{\sigma^2}M=\frac{(n-1)S^2}{\sigma^2}$.

$Var[\frac{(n-1)S^2}{\sigma^2}]=\frac{k^2}{\sigma^4}Var[M]=2(n-1)\implies Var[M]=\frac{2(n-1)\sigma^4}{k^2}$

$E[S^2]=\frac{k}{n-1}E[M]=\sigma^2\implies E[M]=\frac{n-1}{k}\sigma^2$
