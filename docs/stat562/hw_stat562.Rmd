...
$$\frac{f(\mathbf{x}|\theta)}{f(\mathbf{y}|\theta)}=\frac{\prod_{i=1}^nf(x_i)}{\prod_{i=1}^nf(y_i)}=\frac{\prod_{i=1}^n[\mathrm{e}^{-(x_i-\theta)}I_{(\theta,\infty)}(x_i)]}{\prod_{i=1}^n[\mathrm{e}^{-(y_i-\theta)}I_{(\theta,\infty)}(y_i)]}=\frac{\mathrm{e}^{\sum_{i=1}^ny_i}I_{(\theta,\infty)}(x_i)}{\mathrm{e}^{\sum_{i=1}^nx_i}I_{(\theta,\infty)}(y_i)}$$
...
(a). Show that $X_{(1)}=\min_iX_i$ is a complete sufficient statistic.
 ...
 $X\sim$ location $Expo(\mu)$,
 
$$\frac{f(\mathbf{x}|\theta)}{f(\mathbf{y}|\theta)}=\frac{\prod\limits_{i=1}^nf(x_i)}{\prod\limits_{i=1}^nf(y_i)}=\frac{\prod\limits_{i=1}^n[\mathrm{e}^{-(x_i-\theta)}I_{(\theta,\infty)}(x_i)]}{\prod\limits_{i=1}^n[\mathrm{e}^{-(y_i-\theta)}I_{(\theta,\infty)}(y_i)]}=\frac{\mathrm{e}^{\sum\limits_{i=1}^ny_i}I_{(\theta,\infty)}(x_i)}{\mathrm{e}^{\sum\limits_{i=1}^nx_i}I_{(\theta,\infty)}(y_i)}$$

If and only if $\min\{x_1,..,x_n\} =\min\{y_1,..,y_n\}$, the ratio $\frac{f(\mathbf{x}|\theta)}{f(\mathbf{y}|\theta)}$ for every two sample points x and y is constant as a function of $\theta$. Therefore, $T(X)=\min\{X_1,..,X_n\}$ is a minimal sufficient statistic.
 
 (2) Proof complete
 
$f_X(x)=e^{-(x-\mu)}$, $F_X(x)=1-e^{-(x-\mu)}$.
From Theorem 5.4.4, $f_{X_{(j)}}(x)=\frac{n!}{(j-1)!(n-j)!}f_X(x)[F_X(x)]^{j-1}[1-F_X(x)]^{n-j}$, then
...
So $g(T)=0,\forall\mu$, then $P(g(T)=0)=1$, $X_{(1)}$ is a complete statistic.
...
Therefore, $S^2$ and  $X_{(1)}$ are independent by Basu's Theorem.
...
$\prod_{i=1}^n x_i^{-2}$ is free of $\theta$. By the Factorial Theorem, $X_{(1)}$ is a sufficient statistic for $\theta$
...
By given $F(X_i|\alpha,\beta)$, $f(X_i|\alpha,\beta)=\begin{cases}0&\text{if }x<0\\\frac{\alpha}{\beta^\alpha}x^{\alpha-1}&\text{if }0\le x\le\beta\\0&\text{if } x>\beta\end{cases}$
...
By 6.2.6 Facotrization Theorem, $(\prod_{i=1}^n x_i,x_{(n)})$ are sufficient statistic.
...
$\forall\alpha$, $L(\alpha,\beta|\mathbf{x})=\begin{cases}0&\text{when}\beta<x_{(n)}\\\text{monotonically decreasing}&\text{when}\beta>x_{(n)}\end{cases}$. Thus, $\hat\beta_{MLE}=X_{(n)}$.
...
Thus, $$\hat\alpha_{MLE}=\frac{n}{n\ln\hat\beta_{MLE}-\ln\prod\limits_{i=1}^n x_i}=\frac{n}{n\ln X_{(n)}-[\ln X_{(1)}+..+\ln X_{(n)}]}=\frac{n}{[\ln X_{(n)}-\ln X_{(1)}]+..+[\ln X_{(n)}-\ln X_{(n)}]}=\frac{n}{\sum\limits_{i=1}^n[\ln X_{(n)}-\ln X_{(i)}]}$$
...
By 2.1.10 Probability integral transformation, $F_X(\mathbf{x}|\theta)=\int_{-\infty}^x\theta x^{\theta-1}dx=\left.x^\theta\right|_0^x=1-x^\theta \sim Uni(0,1)$. 

By 5.6.3 the exponential-uniform transformation, $F^{-1}_X(\mathbf{x}|\theta)=-\ln x\sim Expo(\theta)$, $-\sum_{i-1}^n\ln x\sim Gamma(n,\theta)$, $(-\sum_{i=1}^n\ln x_i)^{-1}\sim$ Inv-Gamma$(n,\theta)$

the pdf of Inv-Gamma$(\alpha,\beta)$: $f_{X}(x)=\frac{\beta^{\alpha}}{\Gamma(\alpha)}x^{-\alpha-1}e^{-\frac\beta{x}},x>0$, $E[x^n]=\frac{\beta^n}{(\alpha-1)\cdots(\alpha-n)}$. Thus,
...
where $\alpha,\beta$ are positive constants. Show that the posterior distribution of $\sigma^2$ is $IG(\alpha+\frac{n-1}2,[\frac{(n-1)S^2}2+\frac1\beta]^{-1})$. Find the mean of this distribution, the Bayes estimator of $\sigma^2$.
