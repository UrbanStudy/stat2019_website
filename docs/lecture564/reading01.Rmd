...
#### Covariance
    ...
    -|x           |   y   |$\varepsilon$| x,y |$\hat\beta_1$|$\hat\beta_0$
|---|---|---|---|---|---|---|---
...
$\hat\beta_1$ |$N\left(\beta_1,\frac{\sigma^2}{S_{xx}}\right)$|$\frac{\hat\beta_0-\beta_0}{\sqrt{\frac{\sigma^2}{S_{xx}}}}$
$\hat{β}$       | $N\left(β,\sigma^2(X'X)^{-1}\right)$
$\hat{y}$       | $N\left(\hat\beta_0+\hat\beta_1x_0,\sigma^2(\frac1n+\frac{(x-\bar x)^2}{S_{xx}})\right)$
...
$$\frac{\hat y_0|x_0-(\hat\beta_0+\hat\beta_1x_0)}{se(\hat y_0|x_0)}=\frac{\hat y_0-\bar y_0}{se(\hat y_0)}\sim t_{df=n-2}$$
 ...
 item |$\hat\beta_0$| $\hat\beta_1$ | $\hatβ_j$ | Predict$\hat y_0$
|---|---|---|---|---
se |$\sqrt{MSE(\frac1n+\frac{\bar x^2}{S_{xx}})}$|$\sqrt{\frac{MSE}{S{xx}}}$| $\sqrt{C_{jj}MSE}$ | $\sqrt{MSE\left[\frac1k+\frac1n+\frac{(x_0-\bar x)^2}{S_{xx}}\right]}$
T Test| $\frac{\hat{\beta_0}}{se(\hat{\beta_0})}$|$\frac{\hat{\beta_1}}{se(\hat{\beta_1})}$| $\frac{\hat{β_j}-c}{se(β_j)}$ | $\frac{\hat y_0-\bar y_0}{se(\hat y_0)}$
CI | $\hat{\beta_0}\pm t_{\alpha/2}se(\hat{\beta_0})$| $\hat{\beta_1}\pm t_{\alpha/2}se(\hat{\beta_1})$| | $\hat y_0\pm t_{\alpha/2,n-2}se(\hat y_0)$
...
$se(\hat{\beta_0})=\sqrt{Var(\hat\beta_0)}=\sqrt{\hat\sigma^2(\frac1n+\frac{\bar x^2}{S_{xx}})}=\sqrt{\dfrac{\hat{\sigma}^2}{n-2}}=\sqrt{\dfrac{MSE}{n}}$
...
the $i^{th}$ | $\frac{e_i}{\sqrt{MSE}}$ | $\frac{e_i}{\sqrt{MSE(1-h_{ii})}}$ | $\frac{e_i}{1-h_{ii}}$
...
$σ^2 E(y)$        |$y′=\sqrt y $ (square root; Poisson data)
$σ^2 E(y)[1−E(y)]$|$y′=\sin^{−1}(\sqrt y)$ (arcsin; binomial proportions $0≤y_i≤1$)
$σ^2 [E(y)]^2$    |$y′=\ln(y)$ (log)
$σ^2 [E(y)]^3$    |$y′=y^{−1/2} $(reciprocal square root)
$σ^2 [E(y)]^4$    |$y′=y^{−1} $(reciprocal)
