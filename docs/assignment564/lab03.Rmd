...
* Firstly, the estimated intercept (69.104) is not zero on 95% confident level by p-value ($1.71\times e^{-05}$).

* Secondly, the 99% confidence interval (32.996~105.213) for true intercept doesn't contain zero. There are strong relationship between weight and blood pressure.

* Thirdly, the MSE value of model with intercept (75.36) is about half of the MSE with zero intercept (159), which shows the former is better fitted.

* Fourthly, the R-suqare value of model with intercept is 0.5983, while the R-suqare value of zero-intercept model is 0.9929. We can explain that the much larger SSR in zero-intercept model contributs most of the additional R-suqare value. The sum square of total variation with intercept ($2693.6+1808.6=4502.2$) is only 1 percent of the SST with zero intercept ($551834+3968=555802$). It doesn't mean the later is better.

* Finally, the overlayed fitted models on the scatterplot show significant difference between two models. The assumption of through origin with zero blood presure for zero weight seems arbitrary and unreasonable.

* Overall, the model with intercept is recommended because adding non-zero intercept improves the model with smaller MSE. 

<!--Either the blood presure or weight has a reasonable range. The assumption of through origin with zero blood presure for zero weight is unreasonable and is extrapolation of the trend.
...
$$Var(\mathbf{\hatβ_1})=σ^2C_{22}=\frac{\sigma^2}{S_{xx}}$$
