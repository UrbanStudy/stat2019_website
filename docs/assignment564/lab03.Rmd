...
$$
\mathbf{|X'X|}^{-1}=\frac1{n\sum_{i=1}^nx_i^2-(\sum_{i=1}^nx_i)^2}\begin{bmatrix} x_1^2+x_2^2+\cdots+x_n^2  & -(x_{1}+x_{2}+\cdots+x_{n}) \\ -(x_{1}+x_{2}+\cdots+x_{n}) & n \end{bmatrix}_{2\times2}=\frac{\begin{bmatrix} \sum_{i=1}^nx_i^2 & -(\sum_{i=1}^nx_i) \\ -(\sum_{i=1}^nx_i) & n \end{bmatrix}_{2\times2}}{n\sum_{i=1}^nx_i^2-(\sum_{i=1}^nx_i)^2}
$$
...
$$Var(\mathbf{\hatβ})=σ^2(\mathbf{X'X})^{-1}=\frac{\sigma^2}{n\sum_{i=1}^nx_i^2-(\sum_{i=1}^nx_i)^2}\begin{bmatrix} \sum_{i=1}^nx_i^2 & -(\sum_{i=1}^nx_i) \\ -(\sum_{i=1}^nx_i) & n \end{bmatrix}_{2\times2}$$
From this result, show that 
(i)  $Var(\hatβ_0)= σ^2 [1/n+ x^2/S_{xx}]$  

$$Var(\mathbf{\hatβ_0})=σ^2C_{11}=\frac{\sigma^2\sum_{i=1}^nx_i^2}{n\sum_{i=1}^nx_i^2-(\sum_{i=1}^nx_i)^2}=\sigma^2\frac{\sum_{i=1}^nx_i^2-(n\bar x_i)^2+(n\bar x_i)^2}{n\sum_{i=1}^nx_i^2-(n\bar x_i)^2}$$

$$=\frac{\sigma^2}{n}+\sigma^2\frac{n^2\bar x_i^2}{n\sum_{i=1}^nx_i^2-n^2\bar x_i^2}=\frac{\sigma^2}{n}+\frac{\sigma^2n\bar x_i^2}{\sum_{i=1}^nx_i^2-n\bar x_i^2}=\sigma^2(\frac{1}{n}+\frac{(\sum_{i=1}^nx_i)^2}{S_{xx}})$$

(ii)  $Var(\hatβ_1)=σ^2/S_{xx}$

$$Var(\mathbf{\hatβ_1})=σ^2C_{22}=\frac{\sigma^2n}{n\sum_{i=1}^nx_i^2-(\sum_{i=1}^nx_i)^2}=\sigma^2\frac{n}{n\sum_{i=1}^nx_i^2-n^2\bar x_i^2}=\frac{\sigma^2}{\sum_{i=1}^nx_i^2-n\bar x_i^2}=\frac{\sigma^2}{S_{xx}}$$

(iii)  $Cov(β_0,β_1)=-(\bar x σ^2)/S_{xx}$

$$Cov(\mathbf{\hatβ_0,\hatβ_1})=σ^2(C_{12})=\frac{\sigma^2(-\sum_{i=1}^nx_i)}{n\sum_{i=1}^nx_i^2-(\sum_{i=1}^nx_i)^2}=\frac{-\sigma^2(n\bar x_i)}{n\sum_{i=1}^nx_i^2-n^2\bar x_i^2}=\frac{-\sigma^2\bar x_i}{\sum_{i=1}^nx_i^2-n\bar x_i^2}=\frac{-\sigma^2\bar x_i}{S_{xx}}$$