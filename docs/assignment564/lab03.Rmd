...
anova(model_wei_bp, model_wei_bp_origin)
...
* Secondly, the estimated intercept (69.104) is confident on 95% confident level by p-value($1.71\times e^{-05}$).
* Thirdly, the R-suqare value of model with intercept is 0.5983, means only 59.83% observations can be explained by this model. Meanwhile, the R-suqare value of model with zero intercept is 0.9929, which can explain more observation. We can find the sum square of total variation with intercept ($2693.6+1808.6=4502.2$) is about 1 percent of the value with zero intercept ($551834+3968=555802$), which contributs most of the R-suqare value.
* Fourthly, the SSE value of  model with intercept (1808.573) is about half of the SSE with zero intercept (3967.682), which shows the former is better fitted.
