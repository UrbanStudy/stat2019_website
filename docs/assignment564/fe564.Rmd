...
```{r, eval=T, include = F}
vif(model_wf_full_log)
ols_vif_tol(model_wf_full_log)

vif(lm(log(y) ~ X2 + X3 + X4 + X5 + X6 + X7 + X8 + X9, data=table_wf))

vif(lm(log(y) ~ X1 + X2 + X3 + X5 + X6 + X7 + X8 + X9, data=table_wf))
...
```{r, eval=T, include = F}
plot(model_wf_full_log,pch=16,col="blue")

avPlots(model_wf_full_log)
ols_plot_diagnostics(model_wf_full_log)
ols_plot_added_variable(model_wf_full_log)
ols_plot_dfbetas(model_wf_full_log)

ols_plot_resid_hist(model_wf_full_log)

ols_plot_resid_qq(model_wf_full_log)

shapiro.test(rstudent(model_wf_full_log)) #If p-value is bigger, then no problem of non-normality #

table_wf_resi <- table_wf %>% mutate(student_residual=rstudent(model_wf_full_log))
ggpairs(data=table_wf_resi[c(1:11)])
...
step(model_wf_none, direction= "forward", scope=(~ X1 + X2 + X3 + X4 + X5 + X6 + X7 + X8 + X9), k=2)
...
step(model_wf_full_log, direction = "backward")
...
model_wf_subset <- regsubsets(log(y) ~ X1 + X2 + X3 +X4 + X5 + X6 + X7 + X8 + X9, data=table_wf, nbest=10 ) # nbest is the number of models from each size #
...
model_wf_final <- lm(y ~ X3 + X2, data=table_wf)
...
```{r, eval=T, include = T}
# Lack of Fit F Test
ols_pure_error_anova(lm(y~X7, data = table_wf))

MPV::PRESS(model_wf_final)
ols_press(model_wf_final)
```
...
1-((MPV::PRESS(model_wf_full))/(var(table_wf$y)*(nrow(table_wf)-1)))
# prediction power of 1
1-((MPV::PRESS(model_wf_full_log))/(var(table_wf$y)*(nrow(table_wf)-1)))
