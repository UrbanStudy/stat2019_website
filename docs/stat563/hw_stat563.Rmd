...
$f_{Y_{(1)},Y_i}(y_1,y_i)=f_{Y_{(1)}}(y_1)f_{Y_i}(y_i)=n\nu^ne^{−ny_{(1)}}\nu e^{-y_i}=n\nu^{n+1}e^{−(ny_{(1)}+y_i)}$

Let $W_1=Y_{(1)}=n\nu^ne^{−ny}$ and $W_i=Y_i−Y_{(1)}$, then $Y_{(1)}=w_1, Y_i=w_1+w_i$. $|J|=\begin{vmatrix}1 & 0 \\ 1 & 1 \end{vmatrix}=1$

$$f_{W_1,W_i}(w_1,w_i)=f_{Y_{(1)},Y_i}(w_1,w_1+w_i)|J|=n\nu^{n+1}e^{−(nw_1+w_1+w_i)}=n\nu^{n+1}e^{−(n+1)w_1}e^{−w_i}$$

$f_{W_1}(w)=n\nu^{n+1}e^{−(n+1)w}, w>\ln\nu$, $f_{W_i}(w)=e^{-w}$, $W_1, W_2$ are independent.

$W_i=Y_i-Y_{(1)}=e^{-w_i}\sim Expo(1), i=2,..,n$. 

$T=\sum_{i=1}^n(Y_i-Y_{(1)})=Y_1-Y_{(1)}+\sum_{i=2}^n(Y_i-Y_{(1)})=\sum_{i=2}^n(Y_i-Y_{(1)})=\sum_{i=2}^nW_i\sim Gam(n−1, 1)$
