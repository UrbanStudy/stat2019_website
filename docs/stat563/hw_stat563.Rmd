...
$$Var[\bar X]=E[\bar X-\mu]^2=E[\frac1n\sum_iX_i-\mu]^2=\frac1{n^2}E[\sum_i(X_i-\mu)^2+2\sum_{i>j}(X_i-\mu)(X_j-\mu)]$$

$$=\frac1{n^2}[n\sigma^2+\frac{2n(n-1)}{2}\rho\sigma^2]=\frac{\sigma^2}{n}+\frac{n-1}{n}\rho\sigma^2$$
...
where the series can be summed using (1.5.4), the partial sum of the geometric series, or using Mathematica.

 ---
 
(c) The correlation structure in part (b) arises in an autoregressive $AR(l)$ model, where we assume that $X_{i+1}=\rho X_i+\delta_i$, with $\delta_i\sim iid\ n(0,1)$. If $|\rho_i|<1$ and we define $\sigma^2=1/(1-\rho^2)$, show that $Corr(X_1,X_i) =\rho^{|i-1|}$.
...
$$Corr(X_1,X_i)=\frac{\rho^{i−1}EX_1^2−\rho^{i−1}(EX_1)^2}{VarX_1VarX_i}
=\frac{\rho^{i−1}\sigma^2}
