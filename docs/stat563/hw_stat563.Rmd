...
Let $Y_i=\ln X_i, i=1,..,n$. 

Sets $Z_1=\min_i Y_i$, Z2,..,Zn equal to the remaining $Y_i$s with same order. 

Let $W_1=Z_1$ and $W_i=Z_i−Z_1,i=2,..,n$. 

$W_i$s are independent with $W_1$, $f_{W_1}(w)=n\nu^ne^{−nw}, w>\ln\nu $, and $W_i\sim Expo(1), i=2,..,n$. Now T =Pn i=2, $W_i\sim Gam(n−1,1)$, and, hence,
...
$$\sum_{i=1}^n\ln x_i=-\frac{n}{\hat\mu_{MLE}},\sum_{j=1}^m\ln y_j=-\frac{m}{\hat\theta_{MLE}}$$

$$T=\frac{\sum\ln X_i}{\sum\ln X_i+\sum\ln Y_i}=\frac{-\frac{n}{\hat\mu_{MLE}}}{-\frac{n}{\hat\mu_{MLE}}-\frac{m}{\hat\theta_{MLE}}}=\frac{n\hat\theta_{MLE}}{n\hat\theta_{MLE}+m\hat\mu_{MLE}}$$
...
The test $\Lambda\le c_0=0.076546$,$\Lambda\ge c_1=3.637798$ is UMP at $0.10$ level.
...
. So we have a test that is most powerful $\forall\theta_1\notin\Omega_0$, so it is UMP.
