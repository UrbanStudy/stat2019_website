...
From part (a), the logistic location family of pdfs $\{g(t|\theta):\theta\in\Theta\}$ of $T$ has an MLR.

for testing $H_0:\theta\le0$ versus $H_1:\theta>0, $T is a sufficient statistic for $\theta$.

From **Theorem 8.3.17 (Karlin-Rubin)**, if and only if $T>t_0$ is a UMP level $\alpha$ test, where $\alpha= P_{\theta_0}(T>t_0)$,

then for any $t_0$, the test rejects $H_0$  .
...
$=(\theta+1-k)^n+\int_\theta^kn[(\theta+1-y_1)^{n-1}-(1-y_1)^{n-1}]dy_1=(\theta+1-k)^n-(\theta+1-k)^n+1+(1-k)^n-(1-\theta)^{n}=1+\alpha-(1-\theta)^n$
