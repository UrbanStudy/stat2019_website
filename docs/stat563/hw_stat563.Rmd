...
When $\theta+1\le k\le1$, all $Y_1$ and $Y_n$ locate outside the rejection region. The power function$=P(\text{reject} H_0|H_1\text{true})=P(Y_1\ge k\cup Y_n\ge 1|\theta>0)=0$

When $0< k\le\theta$, all $Y_1$ and $Y_n$ locate inside the rejection region. $\beta(\theta)=P(\text{reject} H_0|H_1\text{true})=P(Y_1\ge k\cup Y_n\ge 1|\theta>0)=1$
...
The power function $=P(Y_1\ge k\cup Y_n\ge 1|\theta>0)=\int_k^{\theta+1}(\theta+1-y_1)^{n-1}dy_1=(\theta+1-k)^n$
...
The power function$=P(k\le Y_1\le\theta+1|\theta>0)+P(\theta\le Y_1\le k,1\le Y_n\le\theta+1 |\theta>0)=\int_k^{\theta+1}(\theta+1-y_1)^{n-1}dy_1+\int_\theta^k\int_1^{\theta+1}n(n-1)(y_n-y_1)^{n-2}dy_ndy_1$ $=(\theta+1-k)^n+\int_\theta^kn[(\theta+1-y_1)^{n-1}-(1-y_1)^{n-1}]dy_1=(\theta+1-k)^n-(\theta+1-k)^n+1+(1-k)^n-(1-\theta)^{n}=1+\alpha-(1-\theta)^n$
...
Form **Corollary 8.3.13**, $(Y_1,Y_n)$ is a sufficient statistic for $\theta$ and $g(y|\theta)$ is the pdf of $Y$ corresponding to $\theta_0,\theta_1$.  if it satisfies 
$y_1\in S_1$, $y_n\in S_n$ if $f(y_1,y_n|\theta_1)>kf(y_1,y_n|\theta_0)$ and $y_1\in S_1^c$, $y_n\in S_n^c$ if $f(y_1,y_n|\theta_1)<kf(y_1,y_n|\theta_0)$ for some $k\ge 0$, where $\alpha=P_{\theta_0}(y_1\in S_1,y_n\in S_n)$

Therefore, the test based on $(Y_1,Y_n)$ with rejection regions $Y_1\ge k$ and $Y_n\ge 1$ is a UMP level a test.
