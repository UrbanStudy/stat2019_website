...
$$\Lambda=(\frac{\hat\sigma_X^2}{\hat\sigma_0^2})^{n/2}\cdot(\frac{\hat\sigma_Y^2}{\lambda_0\hat\sigma_0^2})^{m/2}=[\frac{n}{m+n}+\frac{m}{m+n}\frac{\sum_{i=1}^mY_i^2/(\lambda_0\sigma_X^2m)}{\sum_{i=1}^nX_i^2/(\sigma_X^2n)}]^{-n/2}\cdot[\frac{m}{m+n}+\frac{n}{m+n}\frac{\sum_{i=1}^nX_i^2/(\sigma_X^2n)}{\sum_{i=1}^mY_i^2/(\lambda_0\sigma_X^2m)}]^{-m/2}$$
$$=[\frac{n}{m+n}+\frac{m}{m+n}F]^{-n/2}\cdot[\frac{m}{m+n}+\frac{n}{m+n}F^{-1}]^{-m/2}$$

The rejection region is
$$\left\{ (x,y):(\frac{n}{m+n}+\frac{m}{m+n}F)^{-n/2}\cdot(\frac{m}{m+n}+\frac{n}{m+n}F^{-1})^{-m/2}<c_{\alpha} \right\}$$

where c is chosen to satisfy
$$P\left[ (x,y):(\frac{n}{m+n}+\frac{m}{m+n}F)^{-n/2}\cdot(\frac{m}{m+n}+\frac{n}{m+n}F^{-1})^{-m/2}<c_{\alpha} \right]=\alpha$$

 ---

(c) Find a $1-\alpha$ confidence interval for A.

To ease notation, let $a=m/(n+m)$ and $b=a\sum_{i=1}^my^2_i/\sum_{i=1}^nx^2_i$. From the duality of hypothesis tests and confidence sets, the set

$$c(\lambda)=\left\{\lambda:(a+b/\lambda)^{-n/2}\cdot(1-a+a(1-a)\lambda/b)^{-m/2}\ge c_{\alpha} \right\}$$

is a $1-\alpha$ confidence set for $\lambda$

We now must establish that this set is indeed an interval. 

To do this, we establish that the function on the left hand side of the inequality has only an interior maximum. 

That is, it looks like an upside-down bowl. Furthermore, it is straightforward to establish that the function is zero at both  = 0 and  = 1. These facts imply that the set of values for which the function is greater than or equal to c must be an interval. 

We make some further simplifications. If we multiply both sides of the inequality by [(1 − a)/b]m/2,
we need be concerned with only the behavior of the function

Moreover, since we are most interested in the sign of the derivative of h, this is the same as the sign of the derivative of log h, which is much easier to work with. We have

The sign of the derivative is given by the expression in square brackets, a parabola. 

It is easy to see that for  0, the parabola changes sign from positive to negative. 

Since this is the sign change of the derivative, the function must increase then decrease. 

Hence, the function is an upside-down bowl, and the set is an interval.
...
(a) Find the $1-\alpha$ confidence set for $\alpha$ that is obtained by inverting the LRT of $H0:a=a_0$ versus $H1:a\neq a_0$ based on a sample $X_1,..,X_n$ from a $n(\theta,a\theta)$ family, where $\theta$ is unknown.

These densities have already appeared in Exercise 8.8, where LRT statistics were calculated for testing $H_0:a=1$.


Using the result of Exercise 8.8(a), the restricted MLE of  (when $a=a_0$) is

$$\hat\theta_0=-\frac12a_0+\sqrt{a_0^2+4\sum_{i=1}^nx^2/n}$$

and the unrestricted MLEs are

$\hat\theta=\bar x$ and $\hat a=\frac{1}{n\bar x}\sum_{i=1}^n(x_i-\bar x)^2=\hat\sigma^2/\bar x$

The LRT statistic is

$$\Lambda=\frac{(1/a_0\hat\theta_0)^{n/2}e^{-\sum_{i=1}^n(x_i-\hat\theta_0)^2/(2a_0\hat\theta_0)}}{(1/\hat a\hat\theta)^{n/2}e^{-\sum_{i=1}^n(x_i-\hat\theta)^2/(2\hat a\hat\theta)}}=(\frac1{2\pi a_0\hat\theta_0})^{n/2}e^{-\sum_{i=1}^n(x_i-\hat\theta_0)^2/(2a_0\hat\theta_0)}$$
...
Using the results of Exercise 8.8b, the restricted MLE (for $a=a_0$) is found by solving


yielding the MLE

$$\hat\theta_R=\bar x+\frac1{2a_0}\sqrt{\bar x+4a_0(\hat\sigma^2+x^2)}$$

The unrestricted MLEs are

$\hat\theta=\bar x$ and $\hat a=\frac{1}{n\bar x^2}\sum_{i=1}^n(x_i-\bar x)^2=\hat\sigma^2/\bar x^2$
