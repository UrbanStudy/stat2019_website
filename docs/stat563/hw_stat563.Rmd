...
$$\begin{cases}P(T\le c_1)+P(T\ge c_2)=\alpha=0.10\\c_1^{n-1}(1-c_1)^{m-1}=c_2^{n-1}(1-c_2)^{m-1}\end{cases}$$
The test $\Lambda\le c_1,\Lambda\ge c_2$ is most powerful $\forall\theta\neq\mu$, so it is UMP at $0.10$ level.
...
**8.33** Let $X_1,..,X_n$ be a random sample from the $uniform(0,\theta+1)$ distribution. $T_0$ test $H_0:\theta=0$ versus $H_1:\theta>0$, use the test reject $H_0$ if $Y_n\ge1$ or $Y_1\ge k$, where k is a constant, $Y_1= \min\{X_1,..,X_n\}, Y_n= \max\{X_1,..,X_n\}$.

(a) Determine k so that the test will have size $\alpha$.

From theorems 5.4.4 and 5.6.6, the marginal pdf of $Y_{1}$, the joint pdf of $(Y_1,Y_n) are$

$$f_{Y_{1}}(y_1|\theta)=\frac{n!}{(1-1)!(n-1)!}f_{Y_{1}}(y_1|\theta)[F_{Y_{1}}(y_1|\theta)]^{1-1}[1-F_{Y_{1}}(y_1|\theta)]^{n-1}=n[1-(y_1-\theta)]^{n-1},\ \theta<y_1<\theta+1$$

$$f_{Y_{1},Y_{n}}(y_1,y_n|\theta)=\frac{n!}{(1-1)!(n-1-1)!(n-n)!}f_{Y_{1}}(y_1|\theta)f_{Y_{n}}(y_n|\theta)[F_{Y_{1}}(y_1|\theta)]^{1-1}[F_{Y_{n}}(y_n|\theta)-F_{Y_{1}}(y_1|\theta)]^{n-1-1}[1-F_{Y_{n}}(y_n|\theta)]^{n-n}=n(n-1)(y_1-y_n)^{n-2},\ \theta<y_1<y_n<\theta+1$$

Under $H_0:\theta=0$, $P(Y_n\ge1)=0$, 

$$\alpha=P(Y_1\ge k|0)=\int_k^1n[1-(y_1-\theta)]^{n-1}dy_1=(1-k)^n$$
Thus, use $k=1-\alpha^{1/n}$ to have a size $\alpha$ test.

(b) Find an expression for the power function of the test in part (a) .

For $k<\theta$, $\beta(\theta)=1$.

For $k-1<\theta\le0$, 

$$\beta(\theta)=\int_k^{\theta+1}[1-(y_1-\theta)]^{n-1}dy_1=(\theta+1-k)^n$$

For $k-1\ge\theta$, $\beta(\theta)=0$.

For $k\ge\theta>0$, 

$$\beta(\theta)=\int_k^{\theta+1}[1-(y_1-\theta)]^{n-1}dy_1+\int_\theta^k\int_1^{\theta+1}n(n-1)(y_1-y_n)^{n-2}dy_ndy_1=\alpha+1-(1-\theta)^n$$
...
For $1\ge\theta$, the ratio of pdfs is
...
$$\begin{cases}\text{reject}& \text{if }\ \frac{f_{Y_{1},Y_{n}}(y_1,y_n|\theta)}{f_{Y_{1},Y_{n}}(y_1,y_n|0)}>1 \\\text{accept}& \text{if }\ \frac{f_{Y_{1},Y_{n}}(y_1,y_n|\theta)}{f_{Y_{1},Y_{n}}(y_1,y_n|0)}<1 \end{cases}$$

For $\theta\ge k$, use $k'=0$,

$$\begin{cases}\text{reject}& \text{if }\ \frac{f_{Y_{1},Y_{n}}(y_1,y_n|\theta)}{f_{Y_{1},Y_{n}}(y_1,y_n|0)}>0 \\\text{accept}& \text{if }\ \frac{f_{Y_{1},Y_{n}}(y_1,y_n|\theta)}{f_{Y_{1},Y_{n}}(y_1,y_n|0)}<0 \end{cases}$$

(d) Find values of n and k so that the UMP .10 level test will have power at least .8 if $\theta>1$.

For $\theta\gek=1-\alpha^{1/n}$, $\beta(\theta)=1$. So these conditions are satisfied for any n.
